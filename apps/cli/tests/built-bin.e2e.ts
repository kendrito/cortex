...
import { startMockLlmServer } from '@cortex/llm-mock-server'
...
const cortexBin = join(repoRoot, 'apps/cli/lib/bin.js')
...
async function runBuiltBin(
  ...
  const result = await execa(process.execPath, [cortexBin, ...args], {
    input: '',
    timeout: 25_000,
    killSignal: 'SIGKILL',
    reject: false,
  ...
  if (result.timedOut) {
    throw new Error(`cortex built bin did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
  }
...
async function waitForFile(file: string): Promise<void> {
  ...
  while (!existsSync(file)) {
    if (Date.now() >= deadline) throw new Error(`cortex profile lifecycle marker did not appear: ${file}`)
 ...
 * cortex.profile.bundles, no cortex-base — proving out-of-box composition machinery without
 * booting the entire product tree.
 */
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  const home = mkdtempSync(join(tmpdir(), 'cortex-profile-lifecycle-'))
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.CORTEX_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'cortex-profile-lifecycle',
    private: true,
    dependencies: {},
    cortex: { profile: { bundles: ['cortex-lifecycle-bundle'] } },
  }, undefined, 2))
  // Hand-place the "installed" bundle where profile resolution finds it.
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  const linkTarget = join(profileDir, 'node_modules', 'cortex-lifecycle-bundle')
  mkdirSync(join(profileDir, 'node_modules'), { recursive: true })
  try {
...
function startProfileLifecycle(fixture: ProfileLifecycleFixture, args: readonly string[] = []) {
  return execa(process.execPath, [cortexBin, '--profile', 'lifecycle', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    env: {
      CORTEX_HOME: fixture.home,
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'cortex-profile-environment-probe',
    private: true,
    dependencies: {},
    cortex: { profile: { bundles: ['@cortex/base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@cortex/cmdline` and `commander` through the profile module
 * fallback, exactly as an installed out-of-tree bundle does.
 */
function createStartupFixture(): StartupFixture {
  const home = mkdtempSync(join(tmpdir(), 'cortex-profile-startup-'))
  ...
  const bundleDir = join(profileDir, 'node_modules', 'cortex-startup-bundle')
  mkdirSync(bundleDir, { recursive: true })
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    "import { Command } from 'commander'",
    "import { parseCmdline } from '@cortex/cmdline'",
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.CORTEX_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'cortex-startup-bundle',
    version: '0.0.0',
    type: 'module',
    cortex: { bundle: { patch: './cordis.patch.yml' } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'cortex-profile-startup',
    private: true,
    dependencies: {},
    cortex: { profile: { bundles: ['cortex-startup-bundle'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  return {
    home,
...
function startStartupProfile(fixture: StartupFixture, args: readonly string[]) {
  return execa(process.execPath, [cortexBin, '--profile', 'startup', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    timeout: 25_000,
...
describe.skipIf(!existsSync(cortexBin))('cortex BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('cortex --profile web')
    expect(help.stdout).toContain('cortex plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    const home = mkdtempSync(join(tmpdir(), 'cortex-app-help-'))
    try {
      ...
      expect(web.stdout).toContain('Usage: cortex --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('cortex web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('cortex web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: cortex --profile headless')
  ...
  it('does not load a project environment for --version', async () => {
    const project = mkdtempSync(join(tmpdir(), 'cortex-version-project-'))
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'cortex-missing-profile-'))
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { CORTEX_HOME: home })
      expect(result.code).toBe(1)
      expect(result.stderr).toContain('profile "nope" does not exist')
      expect(result.stderr).toContain('cortex plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  it('uses the launching endpoint and managed credential through the published entry', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'cortex-home-environment-'))
    const project = mkdtempSync(join(tmpdir(), 'cortex-home-project-'))
    writeFileSync(join(home, '.credentials.yaml'), `CORTEX_API_KEY: ${apiKey}\n`, { mode: 0o600 })
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: cortex exited 13 with no diagnostic instead of settling
    // ([Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-03-hmr-initial-scan-boot-deadlock.md)).
    const home = mkdtempSync(join(tmpdir(), 'cortex-invalid-patch-'))
    try {
      const result = await runBuiltBin(['--profile', 'web', '--patch', invalidProvider], {
        CORTEX_HOME: home,
        CORTEX_API_KEY: 'keyless-invalid-config',
        CORTEX_TELEMETRY_DISABLED: '1',
      })
      expect(result.code).toBe(1)
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($CORTEX_HOME/cordis.patch.yml) is live too
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `cortex plugin --profile x add .` from a plugin checkout must install THAT
    // checkout — pnpm's cwd is the profile directory, so an un-anchored `.`
    // would self-link the profile.
    const home = mkdtempSync(join(tmpdir(), 'cortex-plugin-anchor-'))
    const checkout = mkdtempSync(join(tmpdir(), 'cortex-plugin-checkout-'))
    try {
      ...
      const result = await execa(process.execPath, [cortexBin, 'plugin', '--profile', 'anchor', 'add', '.'], {
        cwd: checkout,
        input: '',
        timeout: 60_000,
        killSignal: 'SIGKILL',
      ...
      const manifest = JSON.parse(readFileSync(join(home, 'profiles', 'anchor', 'package.json'), 'utf8')) as {
        ...
        cortex: { profile: { bundles: string[] } }
      }
      expect(Object.keys(manifest.dependencies)).toEqual(['anchored-bundle'])
      expect(manifest.cortex.profile.bundles).toContain('anchored-bundle')
    } finally {
      rmSync(home, { recursive: true, force: true })
  ...
  it('activates a dependency that gained cortex.bundle in a later update', async () => {
    ...
    // version declares cortex.bundle. Simulated without a registry: hand-place
    // the installed package, flip its manifest, and run a benign pnpm verb.
    const home = mkdtempSync(join(tmpdir(), 'cortex-plugin-update-'))
    try {
      const profileDir = join(home, 'profiles', 'up')
      const installed = join(profileDir, 'node_modules', 'late-bundle')
      mkdirSync(installed, { recursive: true })
      writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
        name: 'cortex-profile-up',
        private: true,
        dependencies: { 'late-bundle': 'file:./late-bundle' },
        cortex: { profile: { bundles: ['@cortex/base'] } },
      }))
      writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
      // v1: no cortex manifest — a plain dependency.
      writeFileSync(join(installed, 'package.json'), JSON.stringify({ name: 'late-bundle', version: '1.0.0' }))
      const first = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { CORTEX_HOME: home })
      expect(first.code).toBe(0)
      let manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { cortex: { profile: { bundles: string[] } } }
      expect(manifest.cortex.profile.bundles).toEqual(['@cortex/base'])
      // v2: the installed package now declares cortex.bundle (an update landed).
      writeFileSync(join(installed, 'package.json'), JSON.stringify({
        name: 'late-bundle', version: '2.0.0', cortex: { bundle: { patch: './cordis.patch.yml' } },
      }))
      writeFileSync(join(installed, 'cordis.patch.yml'), '[]\n')
      const second = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { CORTEX_HOME: home })
      expect(second.code).toBe(0)
      manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { cortex: { profile: { bundles: string[] } } }
      expect(manifest.cortex.profile.bundles).toEqual(['@cortex/base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    let home: string
    beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'cortex-dump-bin-')) })
    afterEach(() => { rmSync(home, { recursive: true, force: true }) })

    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { CORTEX_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@cortex/agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @cortex/base')
      expect(stdout).toContain("name: '@cortex/host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@cortex/headless'")
      expect(stdout).not.toMatch(/name: '@cortex\/host-/)
      expect(stdout).not.toContain("name: '@cortex/web-app'")
      expect(stdout).not.toMatch(/name: '@cortex\/client-/)
    }, 30_000)

    it('composes the profile user layer and a --patch overlay in order', async () => {
      // Auto-init the web profile first, then write its user layer.
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { CORTEX_HOME: home })
