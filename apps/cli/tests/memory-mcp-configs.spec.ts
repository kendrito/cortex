...
import { boot, loadOverlayPatches } from '@cortex/app-boot'
import SystemPrompt from '@cortex/system-prompt'
import ToolRuntime from '@cortex/tools'
import * as McpClient from '@cortex/mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@cortex/mcp-client')
    ...
    expect(source).not.toContain('CORTEX_API_KEY')
  })
