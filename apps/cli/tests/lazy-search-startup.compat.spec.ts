 ...
 * web profile (cortex-base + cortex-web-app bundle patches, auto-initialized).
...
function runBuiltWeb(cwd: string): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolveRun, rejectRun) => {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      CORTEX_API_KEY: 'cortex-cli-smoke-dummy-key',
      CORTEX_HOME: join(cwd, '.cortex'),
    }
    delete env.CORTEX_BASE_URL
    ...
    child.stdout.on('data', (chunk: string) => {
      stdout += chunk
      if (!settled && /cortex web: http:\/\/127\.0\.0\.1:\d+/u.test(stdout)) {
        settled = true
        child.kill('SIGTERM')
      }
    })
...
describe.skipIf(!requireBuiltArtifacts)('built CLI lazy-search startup', () => {
  it('boots and disposes the shipped composition with full-text search off by default', async () => {
    ...
    try {
      ...
      expect(result.stdout).toMatch(/cortex web: http:\/\/127\.0\.0\.1:\d+/u)
