 ...
 * the REAL shipped bundle layers (cortex-base + cortex-web-app resolved from the
...
import { evaluate } from '@cortex/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@cortex/app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL cortex-base/cortex-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'cortex-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@cortex/base', '@cortex/web-app'])
    const profile = loadProfile('cortex', 'web', anchor, home)
    ...
    for (const name of ['@cortex/pwsh-sandbox', '@cortex/tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'cortex-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@cortex/base'])
    const profile = loadProfile('cortex', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
