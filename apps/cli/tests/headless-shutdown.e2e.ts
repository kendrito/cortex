...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@cortex/loader-smoke'
...
markers = [b"cortex-test: never-dispose ready", b"cortex-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'cortex-headless-shutdown-'))
  try {
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      ...
      private: true,
      dependencies: {},
      cortex: { profile: { bundles: ['@cortex/base', '@cortex/headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        CORTEX_API_KEY: 'keyless-shutdown-no-call',
        CORTEX_TELEMETRY_DISABLED: '1',
        CORTEX_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`cortex headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`cortex headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('cortex: observing at ')
    expect(output).toContain('cortex-test: never-dispose ready')
    expect(output).toContain('cortex-test: never-dispose started')
