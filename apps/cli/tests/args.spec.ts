...
import { parseCortexArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseCortexArgs(argv, '1.2.3')
...
describe('parseCortexArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@cortex/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@cortex/cordis'] })
