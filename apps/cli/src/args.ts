/**
 * Commander adapter for the `cortex` command line.
 *
 ...
 * `@cortex/cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `cortex --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `cortex --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @cortex/cortex/args
 */
...
/** The resolved `cortex` invocation. Help, version, and errors exit inside {@link parseCortexArgs}. */
export type CortexInvocation = ProfileInvocation | DumpConfigInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  cortex --profile web                          boot the web profile (same as: cortex web)
  cortex --profile headless "run the tests"     answer one task, print the result, and exit
  cortex --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  cortex --profile tui --resume <session>       arguments after the launcher flags reach the app
  cortex --profile web --help                   the web app's own flags and help
  cortex plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): CortexInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseCortexArgs(argv: readonly string[], version: string): CortexInvocation {
  let resolved: CortexInvocation | undefined
  ...
  program
    ...
    .description('cortex: boot a Cortex profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `cortex -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: cortex --profile <name> --help)')
    .option('--profile <name>', 'the profile under $CORTEX_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `cortex -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: cortex web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('cortex: no invocation resolved')
  return resolved
}
