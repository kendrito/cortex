/**
 * Shared profile boot for every `cortex` surface: resolve the profile, stack its
 * patch layers (bundle layers in `cortex.profile.bundles` order, the profile's
 ...
 * @module @cortex/cortex/profile-boot
 */

import { writeFileSync } from 'node:fs'
import { join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { FiberState, type Context } from '@cortex/cordis'
...
import { CORTEX_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@cortex/launch-environment'
...
/**
 * The home-level user patch layer (`$CORTEX_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$CORTEX_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveCortexHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this cortex installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the CORTEX_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# cortex profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's cortex.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `CORTEX_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$CORTEX_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `cortex.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$CORTEX_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  // The writable root the roster appends is `cortex-agent-presets`' own, so a
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.CORTEX_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(CORTEX_LAUNCH_ENVIRONMENT_KEY, options.environment)
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@cortex/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@cortex/cordis-plugin-hmr', config: { root: [] } })
      }
