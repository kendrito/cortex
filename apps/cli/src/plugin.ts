/**
 * `cortex plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `cortex.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `cortex.bundle` joins the layer stack; a
 ...
 * gained its `cortex.bundle` declaration in a newer version.
 * @module @cortex/cortex/plugin
 */
 ...
 * @returns true when the package manifest declares `cortex.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.cortex?.bundle?.patch !== undefined
}

/**
 * Reconcile `cortex.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `cortex.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.cortex?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no cortex.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (cortex-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.cortex = { ...after.cortex, profile: { ...after.cortex?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `cortex` was invoked from.
...
/**
 * Run one `cortex plugin` invocation: init if needed, forward to pnpm, reconcile.
