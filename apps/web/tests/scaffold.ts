...
// Boots the REAL web composition — the cortex-base and cortex-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $CORTEX_SNAPSHOT:
// replay (default, keyless: installs cortex-llm-replay into the open llm seam
// in providers mode), record (real adapter + key, harvests fixtures from live
// session memory), refresh (keyless replay that rewrites goldens). A first-run
// option leaves the seam exactly as the shipped composition boots it — no
// route at all — while masking the process credential, without making a model
// call.
//
// Composition divergences from `cortex web`, all deliberate, all via include
...
// built dist; ordinary keyless modes fill the open llm seam post-boot with
// installLlmReplay on the settled root ctx (the plugin-row path discards the
// ReplayHandle; the direct install keeps assertConsumed for the teardown
// fixture-consumption check). The shipped composition mounts no first-party
// adapter row — llm-pi-ai boots dormant, its routes come from settings — so
...
import Include, { type PatchOptions } from '@cortex/cordis-plugin-include'
import Group from '@cortex/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@cortex/acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@cortex/app-boot'
...
import type { ReplayHandle } from '@cortex/llm-replay'
import { installLlmReplay, parseSessionLog } from '@cortex/llm-replay'
import SessionStore, {
  packChunkRuns,
...
// } from '@cortex/client-ui-settings-models'
...
/** Snapshot mode for the lane, from $CORTEX_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.CORTEX_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`CORTEX_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the cortex-base and cortex-web-app bundle patches over the empty profile root. */
...
// mode, never catch-all: the shipped tree registers no route of its own, so a
...
const REPLAY_PROVIDERS = [{
  id: 'cortex-official',
  name: 'Cortex',
  models: [{ id: 'cortex-v4-flash', name: 'Cortex-V4-Flash', contextWindow: 128_000 }],
}]
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($CORTEX_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the installed cortex-llm-replay
   * adapter in replay/refresh modes; ignored in record mode (the real adapter
   * answers). Omit for scenarios issuing no model calls — a stray stream then
   * fails loud (no replay adapter is installed, and the shipped tree registers
   * no route of its own).
   */
  ...
  cordisTools?: boolean
  /**
   * Boot the shipped tree with no llm route at all — neither a replay adapter
   * nor the route-only stand-in — while masking the process environment's
   * CORTEX_API_KEY for this scaffold lifetime. This is the keyless first-run
   * configuration lane: exactly what a fresh install composes before any
   * provider is configured.
   */
  cortexMissingCredential?: boolean
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  if (mode === 'record') {
    ...
    if (process.env.CORTEX_API_KEY === undefined || process.env.CORTEX_API_KEY.length === 0) {
      throw new Error('web e2e record mode needs CORTEX_API_KEY (env or repo-root .env)')
    }
  }
  if (mode === 'record' && options.cortexMissingCredential === true) {
    throw new Error('cortexMissingCredential is a keyless replay/refresh option')
  }
  const maskCortexCredential = mode !== 'record' && options.cortexMissingCredential === true
  const originalCortexCredential = process.env.CORTEX_API_KEY
  let credentialEnvironmentRestored = false
  const restoreCredentialEnvironment = (): void => {
    if (credentialEnvironmentRestored || !maskCortexCredential) return
    credentialEnvironmentRestored = true
    if (originalCortexCredential === undefined) {
      Reflect.deleteProperty(process.env, 'CORTEX_API_KEY')
    } else {
      process.env.CORTEX_API_KEY = originalCortexCredential
    }
  }
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'cortex-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $CORTEX_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.cortex document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.cortex-home')
  ...
  // session is created. Without this a developer's real ~/.cortex/skills silently
  // enters replay requests and goldens while CI sees none. `CORTEX_HOME` follows
  ...
  const skillRootEnvironment = {
    CORTEX_HOME: harnessHome,
    CORTEX_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    CORTEX_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'cortex-web-e2e-sessions-'))
  } catch (error) {
  ...
  if (maskCortexCredential) Reflect.deleteProperty(process.env, 'CORTEX_API_KEY')

  // The include patch set — the same layer stack the profile boot composes
  // (bundle patches in cortex.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.cortex/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $CORTEX_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.cortex-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.cortex, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        cortexHome: join(workspaceCwd, '.cortex-home'),
    ...
    // (apps/web IS @cortex/web-frontend); only the URL line is silenced.
    ...
    { id: 'settings', config: { cortexHome: harnessHome } },
    { id: 'credentials', config: { cortexHome: harnessHome } },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@cortex/tool-cordis' },
      ] }]
      : [],
  ]
  ...
  try {
    ...
    ctx.provide('cortexHomePath', cortexHomePath)
    ...
    // `@cortex/cordis-plugin-group` by name.
    ...
    // Fill the open llm seam on the settled root ctx: the shipped tree boots
    // with llm-pi-ai dormant, so keyless modes own every route here. The
    // first-run lane leaves the seam empty, has no replay fixture, and never
    // streams. The direct install, unlike a plugin row, returns the
    // ReplayHandle for the teardown consumption check.
    ...
    } else if (mode !== 'record' && options.cortexMissingCredential !== true) {
      // No fixture would leave the tree with ZERO provider routes — a state no
      // configured product composition has, and one the composer refuses to
      // type into. Register the same routes
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run CORTEX_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
