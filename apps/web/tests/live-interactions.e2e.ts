...
// The model adapter is cortex-llm-replay with override sidecars: `hang` (+ a
...
import { deriveReplayScript, parseSessionLog } from '@cortex/llm-replay'
import type { ReplayOverrideDoc } from '@cortex/llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(buildOverride?: (sidecarHome: string) => ReplayOverrideDoc): Promise<void> {
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'cortex-web-e2e-sidecar-'))
