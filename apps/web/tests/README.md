...
the deliberate composition divergences from `cortex web` — are documented in
...
## Do not import `@cortex/client-*` here
...
`@cortex/goal/remote`, so the Host build phase ended up waiting on an
...
namespace, acknowledgement field, version, and asserted product copy.

Two kinds of Client import stand. `assembled-boot.ts` drives the shell itself, so
it imports `AppWebEntry` from `@cortex/client-web` and the boot-manifest
type from `@cortex/client-modules/client`: booting the real shell is what
...
`@cortex/client-runtime/client` because `client/runtime` is reachable
