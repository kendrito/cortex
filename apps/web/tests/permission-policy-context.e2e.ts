...
import { canonicalPath } from '@cortex/sandbox'
...
const PROMPTS = [
  ...
  'Does the Cortex file sandbox currently restrict file operations? Answer directly in one sentence. Do not call tools.',
...
function runtimeContexts(events: readonly SessionEvent[]): string[] {
  return events.flatMap((event) => {
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@cortex/system-prompt') return []
...
describe('web e2e: current sandbox policy reaches the model before tools', () => {
  ...
  it.skipIf(MODE === 'record')('records cache-safe current policy before the corresponding model behavior', async () => {
    ...
    expect(systems[0]).not.toContain('Current Cortex file policy:')
    ...
    expect(contexts[0]).toContain('Current Cortex file policy: read-only. Any available operation enforced by the Cortex file sandbox cannot modify files in the standing mode.')
    ...
    expect(contexts[1]).toContain('Current Cortex file policy: danger-full-access. The Cortex file sandbox does not restrict file modifications by available operations.')
    ...
    expect(contexts[2]).toContain(`Current Cortex file policy: workspace-write. Any available operation enforced by the Cortex file sandbox may modify files under the session workspace: ${JSON.stringify(canonicalPath(sessionWorkspace))}. Some platform temporary areas may also be writable.`)
    ...
    expect(contexts[3]).toContain('Current Cortex file policy: read-only.')
