...
import { parseSessionLog } from '@cortex/llm-replay'
import type { SessionEvent, SessionId } from '@cortex/session'
import type {} from '@cortex/goal'
import {
...
const PROMPT = 'Do two turns, and in each turn output the file structure of one random package. Note that once you finish a turn, output the content directly and stop; our system will open another turn for you, and you just do a similar one there.'
...
const PACKAGE_FILES: Readonly<Record<string, string>> = {
  ...
  'packages/client/ui-conversation/package.json': '{"name":"@cortex/client-ui-conversation"}\n',
  ...
  'packages/context/session-reference/package.json': '{"name":"@cortex/session-reference"}\n',
  ...
  'packages/llm/token-meter/package.json': '{"name":"@cortex/token-meter"}\n',
  ...
  'packages/skill/skill-filesystem/package.json': '{"name":"@cortex/skill-filesystem"}\n',
