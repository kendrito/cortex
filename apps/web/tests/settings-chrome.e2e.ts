...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('uses the persisted dark preference while plugins are still loading', async () => {
    ...
    const darkCube = initialDialog.getByRole('button', { name: 'Dark', exact: true })
    ...
    const systemCube = restoredDialog.getByRole('button', { name: 'System', exact: true })
  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    const readState = async (target: Page = page): Promise<ThemeState> => await target.evaluate(() => {
      ...
      return {
        ...
        legacy: localStorage.getItem('cortex.theme'),
        themeColor: metas[0]?.content ?? null,
        themeColorCount: metas.length,
        token: computed.getPropertyValue('--cortex-alias-bg-base').trim(),
      }
    })
    ...
    const secondPage = await newEnglishPage(browser)
    ...
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
    const systemCube = page.getByRole('dialog', { name: 'Settings' }).getByRole('button', { name: 'System', exact: true })
    await systemCube.click()
    await expect.poll(() => systemCube.getAttribute('aria-pressed'), { timeout: 5_000 }).toBe('true')
    ...
    await page.getByRole('dialog', { name: 'Settings' }).getByRole('button', { name: 'Light', exact: true }).click()
    await expect.poll(async () => (await readState()).attr, { timeout: 5_000 }).toBe(false)
    expectThemeColorSynchronized(await readState())
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    expect(await page.evaluate(() => localStorage.getItem('cortex.conversation.busyEnter'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
    ...
    await reloaded.getByRole('button', { name: 'Steer', exact: true }).waitFor({ timeout: 10_000 })
    ...
    try {
      ...
      await secondPage.getByRole('dialog', { name: 'Settings' })
        .getByRole('button', { name: 'Steer', exact: true }).waitFor({ timeout: 10_000 })
      expect(await secondPage.evaluate(() => localStorage.getItem('cortex.conversation.busyEnter'))).toBeNull()
      expect(secondTripwire.pageErrors).toEqual([])
      expect(secondTripwire.warnings).toEqual([])
    } finally {
      await secondPage.close()
    ...
    await reloaded.getByRole('button', { name: 'Steer', exact: true }).click()
    await page.getByRole('menuitem', { name: 'Queue', exact: true }).click()
    await reloaded.getByRole('button', { name: 'Queue', exact: true }).waitFor({ timeout: 10_000 })
    expect(await page.evaluate(() => localStorage.getItem('cortex.conversation.busyEnter'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/ui-conversation:\n\s+busyEnter: queue/)
    await page.keyboard.press('Escape')
  ...
  it('offers English as the only settings language and keeps it across reload', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-settings-language'))
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: 'Settings' })
    await dialog.waitFor({ timeout: 10_000 })
    // The Language selector pill shows the active locale's own name.
    const selector = dialog.getByRole('button', { name: 'English', exact: true })
    expect(await selector.getAttribute('aria-haspopup')).toBe('menu')
    await selector.click()
    // One shipped locale: the menu offers English and nothing else.
    await expect.poll(() => page.getByRole('menuitem').allTextContents(), { timeout: 5_000 }).toEqual(['English'])
    await page.getByRole('menuitem', { name: 'English', exact: true }).click()
    await expect.poll(() => selector.getAttribute('aria-expanded'), { timeout: 5_000 }).toBe('false')
    // Re-selecting the active locale writes nothing: no explicit Host
    // preference and no localStorage — the browser-derived locale stands.
    expect(await page.evaluate(() => localStorage.getItem('cortex.locale'))).toBeNull()
    expect(await readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8').catch(() => ''))
      .not.toMatch(/locale:\n\s+preference:/)
    await page.keyboard.press('Escape')

    // Reload: the settings surface is still English.
    const warningStart = tripwire.warnings.length
    await page.reload({ waitUntil: 'load' })
    await page.waitForSelector('[class*="frame"]', { timeout: 30_000 })
    acknowledgeReloadConnectionLoss(tripwire, warningStart)
    ...
    await page.getByRole('dialog', { name: 'Settings' })
      .getByRole('button', { name: 'English', exact: true }).waitFor({ timeout: 10_000 })
    expect(await page.evaluate(() => localStorage.getItem('cortex.locale'))).toBeNull()
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  }, 90_000)
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    try {
      ...
      expect(await enPage.evaluate(() => localStorage.getItem('cortex.locale'))).toBeNull()
