/** Published cortex web + pnpm dev:web → browser HMR, with no page reload. */
...
import type { Fiber } from '@cortex/cordis'
import LocalSubprocessRuntime from '@cortex/subprocess-local'
import type { SubprocessHandle, SubprocessSpawnSpec } from '@cortex/subprocess'
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  const world = await mkdtemp(join(tmpdir(), 'cortex-web-hmr-world-'))
  ...
  if (!existsSync(binPath)) throw new Error('HMR browser test needs the built cortex bin; run pnpm run build first')
  ...
  try {
    ...
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      ...
      world,
      {
        CORTEX_API_KEY: 'keyless-hmr-no-call',
        CORTEX_HOME: join(world, '.cortex'),
      },
    ))
    const baseUrl = await waitForOutput(host, /cortex web: (http:\/\/[^\s]+)/, 'built cortex web')
    ...
    const pageIdentity = await page.evaluate(() => {
      ...
      Object.defineProperty(window, '__cortexHmrPageIdentity', { value: identity })
      return identity
    })

    await writeFile(sourcePath, updatedSource)
    await page.getByText(newText, { exact: true }).waitFor({ timeout: 30_000 })
    expect(await page.evaluate(() => (window as Window & { __cortexHmrPageIdentity?: string }).__cortexHmrPageIdentity))
      .toBe(pageIdentity)
