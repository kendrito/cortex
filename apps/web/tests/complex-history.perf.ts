...
import type { ReplayEntry, ReplayOverrideDoc } from '@cortex/llm-replay'
...
const LIVE_PROMPT = [
  LIVE_PROMPT_MARKER,
  'Analyze the following project context and return a concise diagnostic.',
  ...Array.from(
    { length: 48 },
    (_, index) =>
      `Context ${String(index + 1).padStart(2, '0')}: the user is checking incremental rendering performance in a long session.`
...
function appendRequestHeader(session: Session, turn: number, step: number): void {
  session.append('request/header', {
    header: {
      config: { provider: 'cortex-official', model: 'cortex-v4-flash' },
...
function appendAssistant(
  ...
  session.append('assistant/message', {
    ...
    message: createAssistantMessage({
      content: text(body),
      source: { provider: 'cortex-official', model: 'cortex-v4-flash' },
    }),
    usage: {
...
function comparisonPrompt(index: number): string {
  ...
  return (`${LONG_CONTINUATION_USER_PREFIX}_${String(index)} `
    + `Continue analyzing incremental issue ${String(index)} of this long session while keeping scroll position and input responsive. `
...
function soakTurn(index: number): ConversationTurnSpec {
  ...
  return {
    prompt: (`${SOAK_USER_PREFIX}_${suffix} `
      + `Continuous conversation round ${String(index)}: check incremental rendering and preserved state. `
...
async function startMutationProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Reflect.set(globalThis, '__cortexPerfMutationProbe', probe)
  })
}

async function stopMutationProbe(page: Page): Promise<MutationProbeResult> {
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__cortexPerfMutationProbe') as
    ...
    Reflect.deleteProperty(globalThis, '__cortexPerfMutationProbe')
...
async function startUserRenderProbe(
  ...
  await page.evaluate((expectedMarker) => {
    ...
    Reflect.set(globalThis, '__cortexPerfUserRenderProbe', probe)
  }, marker)
  await send.evaluate((button) => {
    button.addEventListener('click', (event) => {
      const probe = Reflect.get(globalThis, '__cortexPerfUserRenderProbe') as
...
async function stopUserRenderProbe(
  ...
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__cortexPerfUserRenderProbe') as
      ...
      | undefined
    Reflect.deleteProperty(globalThis, '__cortexPerfUserRenderProbe')
    if (
...
async function launchPerformanceWorld(
  ...
  try {
    if (options.replay === undefined) {
      ...
      replayDir = await mkdtemp(join(tmpdir(), 'cortex-web-perf-replay-'))
