...
// disclosure — as a persistent command row. The shipped composition mounts no
// telemetry backend, so the golden pins the `Session sharing is not
// configured.` sentence; the per-status sentences are pinned by the package
// unit tests.
...
describe('web e2e: /feedback command acknowledgement', () => {
  ...
  it.skipIf(MODE === 'record')('records feedback and renders the acknowledgement with session id and sharing status', async () => {
    ...
    // recorded session and discloses that no sharing backend is mounted.
