...
describe('web e2e: Full access confirmation', () => {
  ...
  beforeAll(async () => {
    ...
    const executablePath = process.env.CORTEX_PLAYWRIGHT_EXECUTABLE_PATH
    browser = await chromium.launch(executablePath === undefined ? {} : { executablePath })
    // The golden pins the actual registered dictionary rather than a
    // test-local translation callback.
