...
it('isolates replay skill discovery from every ambient host root', async () => {
  const ambient = await mkdtemp(join(tmpdir(), 'cortex-web-ambient-skills-'))
  const cortexHome = join(ambient, 'cortex-home')
  const agentsHome = join(ambient, 'agents-home')
  const bundled = join(ambient, 'bundled')
  await Promise.all([
    writeSkill(join(cortexHome, 'skills'), 'ambient-cortex'),
  ...
  const originalAgentsHome = process.env.CORTEX_AGENTS_HOME
  const originalBundled = process.env.CORTEX_BUNDLED_SKILL_DIR
  process.env.CORTEX_HOME = cortexHome
  process.env.CORTEX_AGENTS_HOME = agentsHome
  ...
  try {
    ...
    try {
      ...
      expect(names).not.toContain('ambient-cortex')
    ...
    try {
      ...
      if (originalCortexHome === undefined) delete process.env.CORTEX_HOME
      else process.env.CORTEX_HOME = originalCortexHome
      if (originalAgentsHome === undefined) delete process.env.CORTEX_AGENTS_HOME
      else process.env.CORTEX_AGENTS_HOME = originalAgentsHome
      if (originalBundled === undefined) delete process.env.CORTEX_BUNDLED_SKILL_DIR
      else process.env.CORTEX_BUNDLED_SKILL_DIR = originalBundled
