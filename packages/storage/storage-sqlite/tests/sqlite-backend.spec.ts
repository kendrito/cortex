...
import Storage, { storageBackendServiceKey } from '@cortex/storage'
import type { KvUnitDescriptor } from '@cortex/storage'
...
async function freshDbPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-storage-sqlite-'))
  dirs.push(dir)
