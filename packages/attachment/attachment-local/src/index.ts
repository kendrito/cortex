/** Local durable attachment backend rooted below `CORTEX_HOME`. @module @cortex/attachment-local */
...
export class LocalAttachmentStore extends AttachmentStore {
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx)
    this.root = resolve(join(resolveCortexHome(config.cortexHome), 'attachments', 'v1'))
