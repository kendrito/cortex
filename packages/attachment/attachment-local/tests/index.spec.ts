...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const cortexHome = await mkdtemp(join(tmpdir(), 'cortex-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { cortexHome })
      ...
      await rm(cortexHome, { recursive: true, force: true })
    }
  })

  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const cortexHome = await mkdtemp(join(tmpdir(), 'cortex-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { cortexHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { cortexHome, maxImageBytes: 1 })
