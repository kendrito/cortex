...
import type { StorageBackend } from '@cortex/storage'
import { DomainFacility } from '@cortex/storage-domain'
import type { DomainChanged } from '@cortex/storage-domain'
import SessionStore, { SessionId } from '@cortex/session'
import type { SessionHeader } from '@cortex/session'
...
async function makeDir(name: string): Promise<string> {
  base ??= await realpath(await mkdtemp(join(tmpdir(), 'cortex-workspace-')))
