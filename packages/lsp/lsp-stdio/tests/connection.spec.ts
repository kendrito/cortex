...
import { LspConnection } from '@cortex/lsp-stdio'
import type { ConnectionWriter } from '@cortex/lsp-stdio/src/connection.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit CORTEX_* env entries to the child', async () => {
    // A configured CORTEX_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'CORTEX_LSP_TEST_FACT', CORTEX_LSP_TEST_FACT: 'managed' })
