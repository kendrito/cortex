...
import SessionTitleService, { foldSessionTitle } from '@cortex/session-title'
...
describe('session title persistence round trips', () => {
  it('round-trips through a remounted JSONL backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-title-jsonl-'))
    roots.push(root)
  ...
  it('round-trips through a remounted SQLite backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-title-sqlite-'))
    roots.push(root)
