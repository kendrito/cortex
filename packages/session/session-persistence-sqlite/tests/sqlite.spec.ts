...
import type { Session, SessionEvent, SurfaceEvent, SurfaceEventType } from '@cortex/session'
import SqliteSessionPersistence, { SCHEMA_VERSION } from '@cortex/session-persistence-sqlite'
import {
  openDatabase,
  rowToEvent,
  rowToMeta,
...
async function freshDbPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-sqlite-'))
  dirs.push(dir)
...
runCoordinatorContract('sqlite', async (): Promise<CoordinatorFixture> => {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-sqlite-coord-'))
