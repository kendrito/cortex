import { MessageId, createUserMessage, createMessage } from '@cortex/llm'
...
import type { Session, SessionEvent, SessionHeader } from '@cortex/session'
...
async function freshRoot(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-jsonl-'))
  dirs.push(dir)
  return dir
}
...
runCoordinatorContract('jsonl-none', async (): Promise<CoordinatorFixture> => {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-jsonl-coord-'))
  return {
    mount: async (ctx) => {
      const fiber = await ctx.plugin(JsonlSessionPersistence, { root: dir, compression: 'none' })
      return fiber
...
describe('JsonlSessionPersistence: format helpers', () => {
  ...
  it('projectKey normalizes project paths into bounded readable names', () => {
    expect(projectKey('/Users/qyj/work/cortex')).toBe('--Users-qyj-work-cortex--')
    expect(projectKey('/a/b-c')).toBe(projectKey('/a-b/c'))
    expect(projectKey('C:\\work\\agent')).toBe('--C-work-agent--')
    expect(projectKey('/Größe/~agent')).toBe('--Gr~00F6~00DFe-~007Eagent--')
