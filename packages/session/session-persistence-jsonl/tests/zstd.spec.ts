...
async function freshRoot(prefix = 'cortex-jsonl-zstd-'): Promise<string> {
...
runPersistenceContract('jsonl-zstd', async () => {
  const root = await mkdtemp(join(tmpdir(), 'cortex-jsonl-zstd-contract-'))
  const ctx = new Context()
  await ctx.plugin(SessionStore)
...
runCoordinatorContract('jsonl-zstd', async (): Promise<CoordinatorFixture> => {
  const root = await mkdtemp(join(tmpdir(), 'cortex-jsonl-zstd-coordinator-'))
  return {
...
describe('JsonlSessionPersistence: encoding selection', () => {
  it('rejects roots owned by the opposite encoding in both directions', async () => {
    const rawRoot = await freshRoot('cortex-jsonl-raw-mismatch-')
    ...
    const zstdRoot = await freshRoot('cortex-jsonl-zstd-mismatch-')
