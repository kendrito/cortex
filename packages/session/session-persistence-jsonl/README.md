...
The JSONL durable session-persistence backend — a concrete `SessionPersistence` (the `cortex-session-persistence` seam). Each session has one append-only logical JSONL log, stored as `.jsonl.zstd` by default or raw `.jsonl` when compression is disabled.
