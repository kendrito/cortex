/**
 * @cortex/headless — one-shot direct Agent driver. The bundle patch
 * rides over cortex-base without Host, HTTP, or browser plugins; this runner
...
import type { ModelSelectionRef } from '@cortex/agent'
...
import type {} from '@cortex/cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`cortex: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`cortex: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
