 ...
 * @module @cortex/headless/startup
 */
...
function headlessCommand(): Command {
  return new Command()
    ...
    .addHelpText('after', `
Examples:
  cortex --profile headless "run the tests"     answer one task and exit
`)
}
...
export function apply(ctx: Context): void {
  ...
  program.action(() => {
    ...
    if (task.trim() === '') program.error('error: a task is required, for example: cortex --profile headless "run the tests"')
