import { createAssistantMessage, createUserMessage } from '@cortex/llm'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { Context, type Fiber } from '@cortex/cordis'
import { DatabaseSync } from 'node:sqlite'
import { chmod, mkdtemp, rm, stat, writeFile } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join } from 'node:path'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@cortex/session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@cortex/session'
import SessionPersistence, { SessionPersistenceRevision } from '@cortex/session-persistence'
import type { SessionPersistenceSnapshot } from '@cortex/session-persistence'
import SqliteSessionPersistence from '@cortex/session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@cortex/session-query-sqlite'
import {
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'cortex-session-search-'))
