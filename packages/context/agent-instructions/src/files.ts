 ...
 *
 * @module @cortex/agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@cortex/fs'
import { assertNever } from '@cortex/llm'
import { cortexHomeDisplay } from '@cortex/home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.cortexHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.cortexHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.cortexHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(cortexHome: string): string {
  return `${cortexHomeDisplay(cortexHome)}/AGENTS.md`
}
