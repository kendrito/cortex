...
import * as workspaceContext from '@cortex/agent-instructions'
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@cortex/llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@cortex/session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@cortex/agent'
import AgentLoop from '@cortex/agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@cortex/fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import ToolRuntime, { defineContentToolFixture } from '@cortex/tools'
import type {
  ToolExecution,
  ToolExecutionToken,
} from '@cortex/tools'
...
} from '@cortex/agent-instructions'
import {
...
async function tempRepo(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'cortex-workspace-context-'))
}
...
describe('workspace context instruction discovery', () => {
  it('treats ENOTDIR while probing a host candidate as confirmed absence', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, cortexHome: homeFile })
  ...
  it('loads user-global first, then every root-to-cwd candidate in precedence order', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd, cortexHome: home })
  ...
  it('re-reads content after a same-version, same-size rewrite', async () => {
    ...
    try {
      ...
      expect(await loadBaselineInstructions({ cwd, cortexHome: home, maxBytes: 65536 })).toBeUndefined()

      const leaf = join(cwd, 'AGENTS.md')
      await write(leaf, 'first')
      const first = await loadBaselineInstructions({ cwd, cortexHome: home, maxBytes: 65536 })
      expect(first?.text).toContain('first')
      const again = await loadBaselineInstructions({ cwd, cortexHome: home, maxBytes: 65536 })
      ...
      const second = await loadBaselineInstructions({ cwd, cortexHome: home, maxBytes: 65536 })
  ...
  it.skipIf(process.platform === 'win32')('skips a file that becomes unreadable after discovery without failing the request', async () => {
    ...
    try {
      ...
      const loaded = await loadBaselineInstructions({ cwd, cortexHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file to its target content', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd: root, cortexHome: home })
      const loaded = await loadBaselineInstructions({ cwd: root, cortexHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file through ctx.fs to its target content', async () => {
    ...
    try {
      ...
      await mountWorkspaceContext(ctx, { cortexHome: home, maxBytes: 65536 })
  ...
  it('disables baseline loading when the byte budget is zero', async () => {
    ...
    try {
      ...
      await expect(loadBaselineInstructions({ cwd: root, cortexHome: home, maxBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({ cwd: root, cortexHome: home, maxBytes: 65536, maxSourceBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({
        cwd: root, cortexHome: home, maxBytes: 65536, maxSourceBytes: Infinity,
  ...
  it('defaults cortexHome and uses cwd itself as root when no project marker exists', async () => {
    const root = await tempRepo()
    const emptyHome = await tempRepo()
    // Isolate the default-home fallback: blank CORTEX_HOME is treated as unset, and
    // the home dirs point at an empty dir so the default ~/.cortex holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.cortex/AGENTS.md would otherwise leak in.
    vi.stubEnv('CORTEX_HOME', '')
  ...
  it('honors CORTEX_HOME when cortexHome is not configured explicitly', async () => {
    const root = await tempRepo()
    const envHome = await tempRepo()
    try {
      await write(join(envHome, 'AGENTS.md'), 'env global rule')
      vi.stubEnv('CORTEX_HOME', envHome)

      const files = await discoverBaselineInstructionFiles({ cwd: root })

      expect(files).toEqual([{ absolutePath: join(envHome, 'AGENTS.md'), displayPath: '$CORTEX_HOME/AGENTS.md' }])
    } finally {
      vi.unstubAllEnvs()
      await rm(root, { recursive: true, force: true })
  ...
  it('labels the default Cortex home as ~/.cortex when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.cortex/AGENTS.md'), 'global default rule')

      // A set CORTEX_HOME would override the homedir default and relabel the home.
      vi.stubEnv('CORTEX_HOME', '')
      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@cortex/agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root })

      expect(files.map(file => file.displayPath)).toEqual(['~/.cortex/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.cortex home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.cortex/AGENTS.md'), 'global tilde rule')

      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@cortex/agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, cortexHome: '~/.cortex' })

      expect(files).toEqual([{ absolutePath: join(home, '.cortex/AGENTS.md'), displayPath: '~/.cortex/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
  ...
  it('deduplicates user-global instructions when cortexHome points at the project root', async () => {
    const root = await tempRepo()
    try {
      await mkdir(join(root, '.git'), { recursive: true })
      await write(join(root, 'AGENTS.md'), 'same file')

      const files = await discoverBaselineInstructionFiles({ cwd: root, cortexHome: root })

      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$CORTEX_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context rendering', () => {
  ...
  it('keeps compact truncation notices within budget when a multibyte display path is cut', () => {
    const rendered = renderWorkspaceContext([
      { absolutePath: '/repo/Zürich/AGENTS.md', displayPath: 'Zürich/AGENTS.md', content: 'x'.repeat(1000) },
    ], { maxBytes: 51 })
...
describe('workspace context request injection', () => {
  ...
  it('preserves a visible baseline when its source is unavailable during resume', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { cortexHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
      await composeBaselinePrefix(ctx, original)
  ...
  it('does not promote an unchanged budget-omitted baseline file during resume', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cortexHome: home, maxBytes: 700 })
  ...
  it('recomposes the baseline when candidate precedence changes between resumes', async () => {
    ...
    try {
      ...
      await write(join(root, 'CLAUDE.md'), 'claude rule')
      await mountWorkspaceContext(originalCtx, { cortexHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it('reuses an inserted but unadmitted baseline after session recovery and plugin reload', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      const fiber = await mountWorkspaceContext(ctx, { cortexHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it.each([
    ...
    try {
      ...
      await resumedCtx.plugin(workspaceContext, { cortexHome: home, maxBytes })
      const resumed = stubAgent(root, [...original.session.events])
  ...
  it('reports a changed user-global instruction through the visible-scope reconcile path', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { cortexHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('retains a visible baseline after a plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      const fiber = await mountFileToolsAndWorkspaceContext(ctx, { cortexHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('restores a compacted baseline on a hot plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await ctx.plugin(LocalFileSystem, { cwd: '/' })
      const fiber = await ctx.plugin(workspaceContext, { cortexHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
      const baseline = baselineEvents(agent)[0]
  ...
  it('deduplicates one AGENTS.md that is both user-global and the project-root candidate', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cortexHome: root, maxBytes: 65536 })
      const agent = stubAgent(root)
  ...
  it.each([10, 120])('does not expose state markers when baseline content is omitted at %i bytes', async (maxBytes) => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cortexHome: home, maxBytes })
      const agent = stubAgent(root)
  ...
  it('keeps the direct provider API usable without an operation signal', async () => {
    ...
    try {
      ...
      const rendered = await loadBaselineInstructions({ cwd: root, cortexHome: home, maxBytes: 65536 }, fs)
  ...
  it('rejects a provider-sized instruction file before reading content', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { cortexHome: home, maxBytes: 65536, maxSourceBytes: 4 })
  ...
  it('does not inject anything when maxBytes is zero', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cortexHome: home, maxBytes: 0 })
      const agent = stubAgent(root)
  ...
  it('does not inject an empty agent-instructions message when maxBytes is negative', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { cortexHome: home, maxBytes: -1 })
      const agent = stubAgent(root)
  ...
  it('labels a custom cortexHome as CORTEX_HOME instead of pretending it is ~/.cortex', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['$CORTEX_HOME/AGENTS.md'])
    } finally {
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
  ...
  it('does not repeat a candidate metadata probe during one discovery and read pass', async () => {
    ...
    try {
      ...
      const isolated = await import('@cortex/agent-instructions')
      await isolated.loadBaselineInstructions({ cwd: root, cortexHome: home, maxBytes: 65536 })
      observedStats.clear()
      await isolated.loadBaselineInstructions({ cwd: root, cortexHome: home, maxBytes: 65536 })
  ...
  it('skips an unavailable host candidate but still loads its available sibling', async () => {
    ...
    try {
      ...
      const rendered = await isolated.loadBaselineInstructions({ cwd: root, cortexHome: home, maxBytes: 65536 })
...
describe('dynamic nested workspace context injection', () => {
  ...
  it('skips visible baseline scopes when baseline scopes are excluded from reconciliation', async () => {
    ...
    try {
      ...
      agent.session.append('user/message', authoritative, { surfaceOp: 'append' })
      const resolved = resolveConfig({ cortexHome: home, maxBytes: 65536, localInstructionFileCandidates: [] })
      const cache: InstructionVersionCache = new WeakMap()
  ...
  it('does not mark omitted nested files as pending-loaded', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/sub/file.txt'), 'subtree file')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { cortexHome: home, maxBytes: 700 })
      const agent = stubAgent(root)
      await ctx.tools.execute({
        signal: testToolSignal,
  ...
  it('does not attach nested instructions when the byte budget is disabled', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/deep/file.txt'), 'hello')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { cortexHome: home, maxBytes: 0 })
  ...
  it('retries a nested instruction touch when only a truncated budget notice was rendered', async () => {
    ...
    try {
      ...
      fs.entries.set(join(root, 'pkg/file.txt'), { type: 'file', content: 'hello' })
      await ctx.plugin(ToolFs)
      await ctx.plugin(workspaceContext, { cortexHome: home, maxBytes: 20 })
      const agent = stubAgent(root)
...
describe('workspace context inbox synchronization', () => {
  ...
  it('holds back a dynamic change a one-byte positive render budget cannot represent', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { cortexHome: home, maxBytes: 1 })
      const agent = stubAgent(root)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
