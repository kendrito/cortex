...
import { CompactionId, compactCheckpointSource } from '@cortex/compaction'
import { createUserMessage, CallId , createMessage, createToolResultMessage } from '@cortex/llm'
import SessionStore, { Session, SessionId } from '@cortex/session'
import SessionQueryEngine from '@cortex/session-query'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    const sessionId = SessionId('unicode/naïve"/slash\\/line\n')
    const uri = encodeSessionReferenceUri(sessionId)
    expect(decodeSessionReferenceUri(uri)).toBe(sessionId)

    const mention = formatSessionReferenceMention({ sessionId, label: 'café]session' })
    const parsed = parseSessionReferenceText(`compare ${mention} and ${uri}`)
    expect(parsed.text).toBe(`compare @café]session and @${sessionId}`)
    expect(parsed.references).toEqual([
      { sessionId, label: 'café]session' },
    ...
    expect(parseSessionReferenceText('what is a cortex-session: URI?')).toEqual({
      text: 'what is a cortex-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see cortex-session:%%%')).toEqual({
      text: 'see cortex-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see cortex-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](cortex-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `cortex-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('cortex-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
