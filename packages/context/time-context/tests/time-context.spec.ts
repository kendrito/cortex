...
import { createUserMessage, CallId, LlmAdapter } from '@cortex/llm'
import type { GenerateOptions, StreamChunk } from '@cortex/llm'
import { Session, SessionId, type SessionEvent } from '@cortex/session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@cortex/agent'
import { defineContentToolFixture } from '@cortex/tools'
import AgentLoop from '@cortex/agent-loop'
import { mountAgentLoopTestDependencies } from '@cortex/agent-loop-testkit'
import * as timeContext from '@cortex/time-context'
import type { Config } from '@cortex/time-context'
...
describe('durable step context', () => {
  it('records turn, step, zoned time, and the preceding model-visible message baseline', async () => {
    const { ctx } = await mount({ timeZone: 'Asia/Singapore' })
    const session = Session.create(SessionId('first'))
    openMessageTurn(session, 1, 'Asia/Singapore')
    vi.setSystemTime(BASE + 90_061_000)

    await fire(ctx, sessionAgent(session), 1, 1)

    expect(contextTexts(session)).toEqual([
      'Time sampled while preparing turn 1, step 1: 2026-07-15T09:01:01+08:00[Asia/Singapore]\n'
    ...
    expect(event.data.source).toEqual({
      ...
      sections: [{
        name: 'time-context',
        text: 'Time sampled while preparing turn 1, step 1: 2026-07-15T09:01:01+08:00[Asia/Singapore]\n'
          + 'Browser time zone for this request: Asia/Singapore. Interpret otherwise-unqualified dates and times in this zone.\n'
          + 'Elapsed since the preceding model-visible message: 1d 1h 1m 1s.',
      }],
    })
  ...
  it('formats in one browser zone and falls back when steering supplies mixed zones', async () => {
    ...
    openMessageTurn(mixed, 1, 'Asia/Singapore')
    ...
    expect(contextTexts(mixed)[0]).toContain(
      ...
      + 'Browser time zone for this request: mixed ["America/New_York","Asia/Singapore"]. '
...
describe('configuration and lifecycle', () => {
  it('defaults to the process system zone and retains the zone resolved at plugin load', async () => {
    ...
    expect(contextTexts(session)[0]).toContain('2026-07-14T08:00:00+08:00[Asia/Singapore]')
  })
