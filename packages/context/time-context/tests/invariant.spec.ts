...
import * as TimeInvariant from '@cortex/time-context/invariant'
...
describe('time-context invariants', () => {
  ...
  it('requires browser-zone policy and timestamp to match current-turn request provenance', async () => {
    const ctx = await setup()
    const policy = 'Browser time zone for this request: Asia/Singapore. '
      + 'Interpret otherwise-unqualified dates and times in this zone.'
    expect(() => {
      ctx.emit('session/event', preparing(1, 1, 'Asia/Singapore'), event(reading(
        '1',
        '1',
        'model-visible message',
        '2026-07-14T08:00:00+08:00[Asia/Singapore]',
        policy,
      ), SECOND + 456))
    }).not.toThrow()
    expect(() => {
  ...
  it('rejects one corrupt zone even when another zone would classify the turn as mixed', async () => {
    const ctx = await setup()
    const session = preparing(1, 1, 'Asia/Singapore')
    session.append('user/message', createUserMessage({
    ...
    expect(() => {
      ctx.emit('session/event', session, event(reading(
        ...
        'model-visible message',
        '2026-07-14T00:00:00+00:00[UTC]',
        'Browser time zone for this request: mixed ["Asia/Singapore","Not/A_Real_Zone"]. '
