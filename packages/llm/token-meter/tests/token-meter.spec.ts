...
import { createUserMessage, CallId, createMessage } from '@cortex/llm'
import type { ContentBlock, Message, TokenUsage } from '@cortex/llm'
import SessionStore, { Session, SessionId, canonicalHeader } from '@cortex/session'
import type { EpochHeader, SessionEvent } from '@cortex/session'
import TokenMeter from '@cortex/token-meter'
import type { TokenMeasurement, TokenMeterConfig } from '@cortex/token-meter'
...
describe('TokenMeter pricing', () => {
  ...
  it('prices header, tools, and surface when no reusable usage exists', () => {
    ...
    appendHeader(session, header('cortex-v4-flash', {
      system: 'system',
...
describe('replay anchors and surface folds', () => {
  ...
  it('uses disjoint provider usage and signed durable-output rewrites', () => {
    ...
    appendSuccessfulCall(session, header('cortex-v4-flash'), {
      providerText: 'short',
  ...
  it('selects a heuristic anchor when provider usage would undercut its scale', () => {
    ...
    const requestHeader = header('cortex-v4-flash', { system })
  ...
  it('uses an estimated anchor when provider usage is absent', () => {
    ...
    appendSuccessfulCall(session, header('cortex-v4-flash', { system: 's' }), {
  ...
  it('distinguishes an explicit empty source-event list from an absent legacy list', () => {
    ...
    appendSuccessfulCall(explicit, header('cortex-v4-flash'), {
      durableText: 'listener injected text',
      providerText: '',
      usage: USAGE,
      provenance: 'empty',
    })
    appendSuccessfulCall(legacy, header('cortex-v4-flash'), {
      durableText: 'listener injected text',
      providerText: '',
      usage: USAGE,
      provenance: 'absent',
  ...
  it('invalidates usage for any canonical envelope change or explicit override', () => {
    ...
    const anchoredHeader = header('cortex-v4-flash', { system: 'one' })
    ...
    expect(service.measure(session, header('cortex-v4-flash', { system: 'two' })).baseline.kind)
      .toBe('estimated')
    expect(service.measure(session, header('cortex-v4-pro', { system: 'one' })).baseline.kind)
      .toBe('estimated')
  ...
  it('replays seeded append and replace operations with signed deltas', () => {
    ...
    appendSuccessfulCall(original, header('cortex-v4-flash'), {
      usage: USAGE,
...
describe('malformed replay and listener lifecycle', () => {
  ...
  it('rejects an assistant without its step boundary transactionally', () => {
    ...
    session.append('assistant/message', {
      ...
      message: createMessage({
        ...
        source: {
          kind: 'model',
          ...{ provider: 'mock', model: 'cortex-v4-flash' },
        },
      }),
  ...
  it('clears completed step boundaries and rejects overlapping or late step events', () => {
    ...
    appendHeader(late, header('cortex-v4-flash'))
  ...
  it('rejects repeated and non-earlier assistant source-event references', () => {
    ...
    appendHeader(duplicate, header('cortex-v4-flash'))
    ...
    appendHeader(future, header('cortex-v4-flash'))
