 ...
 *
 * @module cortex-llm-pi-ai/replay
 */

import { LlmError } from '@cortex/llm'
import type { Message, ModelMessageSource } from '@cortex/llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'cortex-foreign',
    provider: source?.provider ?? 'cortex-foreign',
    model: source?.model ?? 'cortex-foreign',
    usage: emptyPiUsage(),
