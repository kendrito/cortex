...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  root = await mkdtemp(join(tmpdir(), 'cortex-pi-composition-'))
  ...
  await writeFile(configPath, [
    ...
    '- id: settings',
    "  name: '@cortex/settings-file'",
    '  config:',
    `    path: ${JSON.stringify(settingsPath)}`,
    '    debounceMs: 10',
    '- id: credentials',
    "  name: '@cortex/credentials-local'",
    '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@cortex/settings-file', FileSettingsProvider],
    ['@cortex/credentials-local', LocalCredentialProvider],
    ['@cortex/llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
