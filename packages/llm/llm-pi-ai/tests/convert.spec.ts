...
import type { AttachmentStore } from '@cortex/attachment'
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, LlmError, createMessage } from '@cortex/llm'
import type { ContentBlock, StreamChunk } from '@cortex/llm'
...
describe('toPiContext', () => {
  ...
  it('omits empty tools and absent system prompt', () => {
    const context = toPiContext({ provider: 'cerebras', model: 'm', messages: [], tools: [] })
  ...
  it('preserves provider and model for foreign assistant messages without replay state', () => {
    const context = toPiContext({
      ...
      messages: [createMessage({
        ...
        source: { kind: 'model', provider: 'cerebras', model: 'old-model' },
      })],
    })
  ...
  it('replays all native block kinds when optional metadata is absent', () => {
    ...
    const context = toPiContext({
      ...
      messages: [createMessage({
        ...
        source: {
          kind: 'model',
          ...{ provider: 'cerebras', model: 'gemma-4-31b', replayState: state },
        },
      })],
    })
  ...
  it.each([
    ...
    ['model', { ...validReplay, model: 'gpt-oss-120b' }],
    ...
    try {
      toPiContext({
        ...
        messages: [createMessage({
          ...
          source: {
            kind: 'model',
            ...{ provider: 'cerebras', model: 'gemma-4-31b', replayState },
          },
        })],
      })
      expect.fail('expected invalid replay state')
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('classifies a completed stop with no content as an EMPTY_RESPONSE error', () => {
    expect(mapStopReason(assistant({ stopReason: 'stop' }))).toEqual({
      kind: 'error',
      failure: {
        message: 'model "gemma-4-31b" returned a completed response with no content',
  ...
  it('uses the resolved context window for silent and length-stop overflows', () => {
    ...
    expect(mapStopReason(silent, 100)).toEqual({
      kind: 'error',
      failure: {
        message: 'pi-ai detected context overflow for model "gemma-4-31b"',
