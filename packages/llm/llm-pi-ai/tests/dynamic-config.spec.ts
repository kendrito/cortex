...
import { LocalCredentialProvider } from '@cortex/credentials-local'
...
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-pi-dynamic-'))
  cleanups.push(() => rm(dir, { recursive: true, force: true }))
  return dir
}

/** Real dynamic composition mirroring the cortex twin's harness. */
...
describe('request-level dynamic profiles', () => {
  it('mounts bare and dormant, then registers routes the moment settings supply providers', async () => {
    ...
    // Dormant ≠ invisible: every offered catalog provider is configurable
    // before any route exists, each addressed inside the providers dict.
    const directory = ctx.llm.listConfigurableProviders()
    expect(directory.map(entry => entry.provider)).toEqual(offeredCatalogRoutes())
    ...
    await ctx.settings.update(NS, {
      providers: { cerebras: { apiKeyEnv: 'PI_DYNAMIC_KEY', baseURL: server.url } },
    })
    expect(ctx.llm.listProviders().map(provider => provider.id)).toEqual(['cerebras'])
    await expect(ctx.llm.listModels('cerebras')).resolves.not.toHaveLength(0)

    const result = await assemble(ctx, { provider: 'cerebras', model: 'gemma-4-31b', messages: [] })
  ...
  it('adds a provider route from settings and drops it when the user layer resets', async () => {
    ...
    await ctx.settings.update(NS, {
      providers: { cerebras: { apiKeyEnv: 'PI_LIVE_KEY', baseURL: server.url } },
    })
    expect(ctx.llm.listProviders().map(provider => provider.id)).toEqual(['openai', 'cerebras'])
    ...
    await ctx.settings.replace(NS, {})
    expect(ctx.llm.listProviders().map(provider => provider.id)).toEqual(['openai'])
    const removed = await assemble(ctx, { provider: 'cerebras', model: 'gemma-4-31b', messages: [] })
  ...
  it('rotates the per-request credential referenced by apiKeyEnv', async () => {
    ...
    await assemble(ctx, { provider: 'cerebras', model: 'gemma-4-31b', messages: [] })
