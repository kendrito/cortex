...
import type { TerminalSendOperation } from '@cortex/terminal'
...
async function harness(
  ...
  const root = mkdtempSync(join(tmpdir(), 'cortex-pty-local-'))
  roots.push(root)
...
describe('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    const previous = process.env.CORTEX_TEST_SECRET
    ...
    try {
      ...
      expect(created.motd).toContain('cortex> ')
      ...
      if (previous === undefined) delete process.env.CORTEX_TEST_SECRET
      else process.env.CORTEX_TEST_SECRET = previous
    }
  }, 10_000)
  ...
  it('quiesces a disowned same-session descendant after the shell exits naturally', async () => {
    ...
    try {
      const background = ctx.terminals.startSend(agent, created.sessionId, {
        text: `sh -c 'trap "" TERM; printf "%s" "$$" > "$1"; sleep 60' cortex "${pidFile}" & disown`,
        submit: true,
      })
