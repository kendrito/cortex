...
import AgentRegistry, { Inbox, type Agent } from '@cortex/agent'
import SandboxProvider from '@cortex/sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@cortex/sandbox'
import SandboxPolicyService, { setSandboxMode } from '@cortex/sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@cortex/terminal'
import { BashTerminalBackend } from '@cortex/terminal-bash'
import * as ptyLocal from '@cortex/terminal-bash'
import type { ResolvedConfig } from '@cortex/terminal-bash/src/config.ts'
import type { LocalPtySession } from '@cortex/terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'cortex> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        CORTEX_SHELL: '1', CORTEX_SESSION_ID: 'agent', CORTEX_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07cortex> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
    ...
    expect(session.motd).toBe('cortex> ')
