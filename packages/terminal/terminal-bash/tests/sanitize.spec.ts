...
import { normalizeTerminalText, TerminalSanitizer } from '@cortex/terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07cortex> ')).toEqual({ text: 'cortex> ', prompt: true, promptTail: 'cortex> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('cortex> ')).toEqual({ text: 'cortex> ', prompt: false, promptTail: 'cortex> ' })
  })
