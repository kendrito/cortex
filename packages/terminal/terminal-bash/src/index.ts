 ...
 * @module @cortex/terminal-bash
 */

import { Context } from '@cortex/cordis'
import type { Agent } from '@cortex/agent'
import type { Session, SessionEvent } from '@cortex/session'
import { TerminalBackendCleanupError } from '@cortex/terminal'
import type { TerminalBackend, TerminalBackendSpawnSpec } from '@cortex/terminal'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@cortex/subprocess'
import type { SandboxExecutionPolicy } from '@cortex/sandbox'
import { effectiveSandboxMode } from '@cortex/sandbox-policy'
...
function childEnvironment(spec: TerminalBackendSpawnSpec): Record<string, string> {
  ...
  return {
    ...
    CORTEX_SHELL: '1',
    CORTEX_SESSION_ID: spec.owner.id,
    CORTEX_PTY_SESSION_ID: spec.sessionId,
  }
}
