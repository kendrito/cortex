...
The six generated schemas are listed in the [`cortex-tool-terminal` catalog section](../../../docs/tool-catalog.md#cortextool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `@cortex/jobs` and its model-facing controller.
