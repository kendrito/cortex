...
Session itself owns immutable, surface-valid log storage in every composition: it takes one lossless JSON snapshot of each candidate, validates complete cited source-event coverage and positional replacement, restricts `tool/result` replacement to one current result's `content`, deep-freezes the accepted record, and exposes the log through immutable array snapshots. The `cortex-session` invariant companion checks the remaining cross-record rules that Session does not own.
...
| Companion | Checks |
|---|---|
| `cortex-session`, `cortex-agent`, `cortex-scope`, `cortex-agent-loop` | Session enclosure and call/result trace, agent-status transitions, inbox FIFO conservation, scoped subjects, and model-request reconstruction. |
| `cortex-llm`, `cortex-llm-retry`, `cortex-tools`, `cortex-system-prompt` | Stream grammar, durable retry position and bounds, tool-pipeline stages and frozen results, and authoritative prompt-assembly data. |
| `cortex-compaction`, `cortex-hook-protocol`, `cortex-sandbox-policy` | Durable compaction and hook pairing, compaction metadata, and sandbox-mode vocabulary. |
| `cortex-fs`, `cortex-subagent`, `cortex-workflow` | Filesystem event identity, provider/child pairing, and workflow/agent lifecycle identity. |
| `cortex-goal`, `cortex-goal-round-driver` | Durable goal source/content agreement, revision and lifecycle transitions, timestamps, sequential admitted rounds, and reconstructed continuation prompts. |
| `cortex-permission-presets`, `cortex-user-approval` | Active-preset references and approval asked/decided audit pairing. |
| `cortex-jobs`, `cortex-tool-todo` | Task snapshot lifecycle/ownership fields and durable whole-list todo structure. |
| `cortex-time-context` | Durable clock readings agree with the session's open turn and next pre-step position and elapsed baseline; rendered time parses and does not postdate its event. |
...
ctx.plugin(InvariantRegistry, {
  ...
  package_blocklist: ['^@cortex/agent-loop$'],
})
