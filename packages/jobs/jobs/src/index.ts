 ...
 * process-local registry lives in `@cortex/jobs-local`.
 * @module @cortex/jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@cortex/jobs is the abstract job registry seam; load an implementation such as @cortex/jobs-local instead')
    }
    super(ctx, 'jobs')
  }
