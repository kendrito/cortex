...
import { JobId } from '@cortex/jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@cortex/jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @cortex/tool-jobs in its composition)')
  })
