 ...
 * @module @cortex/jobs-local
 */

import { Context } from '@cortex/cordis'
import z from '@cortex/schemastery'
import type { Agent } from '@cortex/agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@cortex/scope'
import type { ScopeLayer } from '@cortex/scope'
import { deadline, timeoutOf } from '@cortex/timeout'
import { JobRegistry, JobId } from '@cortex/jobs'
import type {
 ...
 * `@cortex/jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @cortex/tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @cortex/agent)')
    }
