...
import { CodeView, type CodeViewInjected } from './CodeView.tsx'
import type { ClientContext } from '@cortex/client-runtime/client'
// Type-only: pulls the locale plugin's Context merge (ctx.locale).
import type {} from '@cortex/client-locale/client'
// Type-only: the `conversation.view` SlotMap key for the embedded editor tab.
 ...
 * to this one is NOT constrained: cortex.client.inject edges are informational
...
export function apply(ctx: ClientContext): void {
  ...
  // The embedded editor rides the conversation view ring: one tab per session,
  // always single-rooted at that session's own directory.
  ctx.slots.inject('conversation.view', () => ctx.slots.register({
    ...
    locale: NS,
    label: () => t('view.code'),
    inject: (sessionId): CodeViewInjected => ({
