...
import type { ISession, SessionId, WorkspaceId } from '@cortex/client-runtime/client'
import type { PropsRenderSlots } from '@cortex/client-ui-slots'
...
import { apply, inject } from '@cortex/client-ui-workspace/client'
...
describe('session rename through the assembled browser', () => {
  it('renames via the row menu: binding.session.rename fires, the dialog closes, the row re-labels from the list', async () => {
    ...
    fireEvent.change(input, { target: { value: '  Fork  experiment notes  ' } })
    ...
    await waitFor(() => { expect(rename).toHaveBeenCalledWith('Fork  experiment notes') })
    ...
    await runtime.sessions.updateSummary(SID, { displayTitle: 'Fork experiment notes', title: 'Fork experiment notes' })
    await view.findByText('Fork experiment notes')
