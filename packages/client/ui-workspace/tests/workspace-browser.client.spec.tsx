...
describe('WorkspaceBrowser', () => {
  ...
  it('expands the target group before starting a session from its + button', () => {
  ...
  it('auto-expands the Ungrouped bucket for a loose current session; its header has no menu and its + button is inert', () => {
    const startSession = vi.fn()
    mount({
  ...
  it('shows only the current blank session as the localized New Session, excluded from search', () => {
    ...
    rerender(b, {})
    expect(screen.getAllByText('New Session')).toHaveLength(1)
    // Search excludes blank rows entirely — neither the canonical New Session
    // title nor the stored display title participates in matching.
    fireEvent.change(screen.getByPlaceholderText('Search sessions...'), { target: { value: 'new session' } })
    expect(screen.queryByText('New Session')).toBeNull()
    fireEvent.change(screen.getByPlaceholderText('Search sessions...'), { target: { value: 'alpha-blank' } })
    expect(screen.queryByText('New Session')).toBeNull()
    expect(screen.queryByText('alpha-blank')).toBeNull()
  })
