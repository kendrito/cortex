...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('counts the themed scrollbar inside the shell trailing inset', () => {
    expect(root?.get('--cortex-session-list-edge-inset')).toBe('var(--cortex-sidebar-inline-padding)')
    expect(root?.get('--cortex-session-list-scrollbar-width')).toBe('8px')
    expect(root?.get('--cortex-session-list-scrollbar-offset')).toBe('2px')
    expect(root?.get('padding-right')).toBe('var(--cortex-session-list-edge-inset)')
    expect(listArea?.get('margin-left')).toBe('-4px')
    expect(listArea?.get('padding-left')).toBe('4px')
    expect(listArea?.get('margin-right')).toBe('calc(-1 * var(--cortex-session-list-edge-inset))')
    expect(declarations('.fade')?.get('right')).toBe('var(--cortex-session-list-edge-inset)')
    expect(list?.get('margin-right')).toBe('var(--cortex-session-list-scrollbar-offset)')
    expect(list?.get('margin-left')).toBe('-4px')
    expect(list?.get('padding-left')).toBe('4px')
    expect(list?.get('padding-right')).toBe([
      'calc(',
      'var(--cortex-session-list-edge-inset)',
      '- var(--cortex-session-list-scrollbar-width)',
      '- var(--cortex-session-list-scrollbar-offset)',
      ')',
    ].join(' '))
  ...
  it('keeps the compact fade, overflow control, search field, and row heights', () => {
    ...
    expect(rowDeclarations('.sessionRow.selected')?.get('background'))
      .toBe('var(--cortex-alias-interactive-bg-hover)')
  })
