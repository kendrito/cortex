...
import { apply, inject } from '@cortex/client-ui-settings-general/client'
...
describe('ui-settings-general apply', () => {
  ...
  it('the nav label is a locale-following thunk resolved at read time, never re-registered', async () => {
    const b = await bench()
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const versions = SEATS.map(([name]) => b.slots.getVersion(name))
    // The label is not baked into the registration: it is a thunk over the
    // bound translator, resolved by whoever reads the entry.
    expect(generalEntry(b.slots)!.options.label).toBeTypeOf('function')
    expect(resolveSlotLabel(generalEntry(b.slots)!.options.label)).toBe('General')
    // A locale-face revision bump (another namespace registering its
    // dictionary) reaches outlets through their subscription alone.
    b.locale.register('probe', 'en', {})
