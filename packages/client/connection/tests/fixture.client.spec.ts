...
describe('createFixtureApi', () => {
  ...
  it('searches current message text with literal unicode61-style token phrases', async () => {
    ...
    const reasoningOnly = await api.sessions.search(req({ query: 'collapsible block' }), signal)
  ...
  it('serves grouped models and keeps a selection for later history and fixture requests', async () => {
    ...
    expect(catalog.result.value.groups.map(group => group.name)).toEqual(['Cortex', 'OpenAI'])
    expect(catalog.result.value.groups[0]?.models.map(model => model.id))
      .toEqual(['cortex-v4-flash', 'cortex-v4-pro'])
  ...
  it('serves configured Cortex readiness and keeps credential values write-only', async () => {
    const api = createFixtureApi()
    ...
    const initial = await api.credentials.describe(req({ refs: ['CORTEX_API_KEY', 'TEST_API_KEY'] }))
    if (!initial.result.ok) throw new Error('credential describe failed')
    expect(initial.result.value.credentials).toEqual({
      CORTEX_API_KEY: { configured: true, source: 'file', writable: true },
  ...
  it('timing hooks: history delay + one-shot failure, silent append, and breakStreams end open generators', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 10))
    hooks.appendSilent('fx-alpha', 'silent dropped frame')
    hooks.appendUser('fx-alpha', 'normal live append')
    ...
    await vi.waitFor(() => {
      expect(seen.some(f => f.type === 'session/event' && JSON.stringify(f.event.data).includes('normal live append'))).toBe(true)
    ...
    expect(seen.some(f => f.type === 'session/event' && JSON.stringify(f.event.data).includes('silent dropped frame'))).toBe(false)
    ...
    expect(JSON.stringify(repull.result.value.events)).toContain('silent dropped frame')
