...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@cortex/llm/message'
import { CallId } from '@cortex/llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@cortex/attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@cortex/session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@cortex/commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@cortex/commands/types'
import { deriveEventMessage, foldSurface } from '@cortex/session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@cortex/host-apiproxy/api'
 ...
 * `--cortex-*` tokens, a bold run, column-aligned table rows that must scroll
 ...
 * presenter's shape (see formatGrepOutput in cortex-tool-fs-search): a
 ...
 * formatGlobOutput in cortex-tool-fs-search).
 */
...
const WEB_SEARCH_RESULT: Omit<Extract<ToolResultView, { card: 'web'; kind: 'search' }>, 'card' | 'kind'> = {
  answer: 'Cortex is a plugin-based agent harness on vendored Cordis where **every capability is a plugin**.',
  sources: [
    {
      url: 'https://github.com/local/cortex',
      title: 'Cortex — plugin-based agent harness',
...
function buildAlphaLog(): SessionEvent[] {
  ...
  push({
    ...
    data: { provider: 'cortex-official', model: 'cortex-v4-flash', contextWindow: 128_000 },
  })
  for (let turn = 0; turn < 60; turn++) {
    ...
    if (withReasoning) blocks.push({ type: 'reasoning', text: `Reasoning ${turn}: a collapsible block of reasoning content.` })
  ...
  toolTurn(70, 'web_search', '{"query":"cortex harness architecture"}', 'Search results for cortex harness architecture.')
  toolTurn(71, 'web_fetch', '{"url":"https://example.invalid"}', '# Harness architecture\n\nEverything is a plugin.')
  ...
  push({
    ...
    surfaceOp: 'append',
    data: { turn: 72, step: 0, message: assistantMessage(text('Entry 1: the first. Entry 2: the second. Entry 3: this one is cut off half-')) },
  })
...
/** Fixture-local mirror of the goal projection value (cortex-goal's GoalProjection shape). */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const modelSelections = new Map<SessionId, ModelSelection>(sessions.map(session => [
    session.sessionId,
    { provider: 'cortex-official', model: 'cortex-v4-flash' },
  ]))
  ...
  const fixtureCredentials = new Map<string, true>([
    ...
    // Cortex route so unrelated GUI journeys do not enter first-run setup.
    ['CORTEX_API_KEY', true],
  ])
  /**
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@cortex/tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@cortex/tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@cortex/tool-read'\n" }],
  ])
  ...
  const directoryTree = new Map<string, string[]>([
    ...
    [`${FIXTURE_HOME}/Documents`, [
      'project', 'cortex-iOS', 'cortex-android', 'cortex-platform',
      'cortex-web', 'cortex', 'cortex-app', 'cortex-landing-blog',
    ]],
  ])
  ...
  const fixtureQuestions: Extract<MuxFrame, { type: 'question/requested' }>['questions'] = [
    {
      id: 'harness-profile',
      header: 'Preference',
      question: 'Which kind of Agent/Harness candidate are you hiring for?',
      options: [
        { label: 'Delivery-focused (Recommended)', description: 'Values building runtime, tool executor, sandbox, trace and production debugging directly.' },
        { label: 'Research-leaning', description: 'Values agent understanding, training/eval thinking and long-term growth.' },
        { label: 'Balanced', description: 'Requires both engineering ability and agent insight, at a higher bar.' },
      ],
    },
    {
      id: 'work-mode',
      header: 'Approach',
      question: 'Which working style should candidates lead with?',
      options: [
        { label: 'Prototype first (Recommended)', description: 'Validate key assumptions fast with something runnable.' },
        { label: 'Design first', description: 'Settle scope, protocol and risk before implementing.' },
      ],
    },
    {
      id: 'signals',
      ...
      detail: 'Pick against the current hiring goal; skipping means no preference.',
      multiSelect: true,
      options: [
        { label: 'System design' },
        { label: 'Code quality' },
  ...
  const timingHooks = {
    ...
    beginModelRetry(id: string): void {
      ...
      append(sessionId, { type: 'assistant/chunk', data: { turn, step: 1, chunk: { type: 'text-delta', index: 0, text: 'half-written reply that should be withdrawn' } } })
    },
    ...
    scheduleModelRetry(id: string, retry = 1, delayMs = 450): void {
      ...
      if (!scenario.stepStarted) {
        ...
        append(sessionId, { type: 'assistant/chunk', data: { turn: scenario.turn, step: 1, chunk: { type: 'text-delta', index: 0, text: `Attempt ${String(retry)} reply that should be withdrawn` } } })
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      create: async (request) => {
        ...
        modelSelections.set(created.sessionId, { provider: 'cortex-official', model: 'cortex-v4-flash' })
        attachedSessions += 1
      ...
      prompt: (request) => {
        ...
        const selection = modelSelections.get(id) ?? { provider: 'cortex-official', model: 'cortex-v4-flash' }
        ...
        startReply(
          ...
          userText === 'render markdown'
            ...
            : userText === 'report model'
              ...
              : `Echo: ${userText}. Fixture streaming reply, for verifying typewriter growth and final swap.`,
        )
        return ok(request, { accepted: true as const })
      },
      attachment: (request) => {
        ...
        if (approvalPending) {
          conn.push({
            ...
            payload: {
              ...
              toolName: 'dangerous_tool', reason: 'fixture standing approval (answer approve/deny to dismiss)',
            },
          })
        }
        if (questionPending) {
    ...
    settings: {
      // Only the resolved Cortex address needed by first-run readiness is
      ...
      describe: request => ok(request, {
        ...
        namespaces: [{
          ns: 'llm-pi-ai',
          schema: {},
          value: { apiKeyEnv: 'CORTEX_API_KEY' },
          applies: 'live',
    ...
    llm: {
      providers: request => ok(request, {
        providers: [
          { provider: 'cortex-official', displayName: 'Cortex', settingsNs: 'llm-pi-ai', settingsPath: [], active: true },
