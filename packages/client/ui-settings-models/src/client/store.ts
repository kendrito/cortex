...
import { getPath, hasPath, nodeAtPath, rehydrateSchema } from '@cortex/client-schema-form'
 ...
 * @param provider - provider route id (e.g. `anthropic`, `acme-gateway`).
 * @returns the derived reference name (e.g. `ACME_GATEWAY_API_KEY`).
 */
 ...
 * Cortex route — the one route the prompt can offer a key field for — decide
...
export function onboardingReadiness(state: ModelsSettingsState): OnboardingReadiness {
  ...
  const row = state.rows.find(candidate =>
    candidate.entry.provider === 'cortex-official'
