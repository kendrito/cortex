 ...
 *
 * The section paints in both themes, and a `--cortex-*` name the theme does not
...
describe('ModelsSection theme styles', () => {
  it('names only theme variables the token sheet defines', () => {
    // A `--cortex-*` name the sheet never declares is not a near miss: it silently
    ...
    const named = [...css.matchAll(/var\((--cortex-[a-z0-9-]+)/g)].map(match => match[1])
  ...
  it('separates the row card from the editor it expands into', () => {
    ...
    expect(block('.editor')).toContain('background: var(--cortex-alias-bg-module-platform)')
    expect(block('.rowCard')).toContain('border: 1px solid var(--cortex-alias-border-l2)')
  ...
  it('never falls back to a literal colour', () => {
    ...
    expect(css).not.toMatch(/var\(--cortex-[a-z0-9-]+\s*,\s*(?:#|rgb|rgba|hsl|hsla)/)
  })
})
