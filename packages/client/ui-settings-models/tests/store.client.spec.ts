...
import type { RpcResponse } from '@cortex/api-remotes/client'
...
const NAMESPACES = [
  {
    ns: 'llm-acme',
    schema: {},
    value: { apiKeyEnv: 'CORTEX_API_KEY', baseURL: 'https://base' },
...
describe('ModelsSettingsStore', () => {
  it('joins rows with configured, removable, and credential state', async () => {
    ...
    expect(seenRefs).toEqual([['CORTEX_API_KEY', 'OPENAI_API_KEY']])
    const byProvider = new Map(state.rows.map(row => [row.entry.provider, row]))
    expect(byProvider.get('cortex-official')).toMatchObject({
      configured: true,
      removable: false,
