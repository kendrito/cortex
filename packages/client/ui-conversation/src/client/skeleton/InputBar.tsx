...
import { AttachmentRail, DropOverlay, ImageLightbox } from '@cortex/client-ui-attachment'
import type { AttachmentRailItem } from '@cortex/client-ui-attachment'
// Type-only: the `plan` projection key merge (the TodoDock posture — the
// composer reads a host-computed value; the domain owns the key).
import type {} from '@cortex/plan-mode/client'
// Type-only: the `goal` projection key merge (hint disambiguation).
import type {} from '@cortex/goal/client'
...
export function InputBar({
  ...
  // Intake pre-check (Cortex Chat semantics): an addition that would break
  ...
  const intakeImages = useCallback((files: readonly File[]): void => {
    ...
    const rejected = ((): string | null => {
      if (imageLimits !== undefined) {
        // Format precedes limits (Cortex Chat's filter order): a batch with
  ...
  // Whole-page file-drop intake (Cortex Chat behavior): the listeners live
