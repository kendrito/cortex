 ...
 * --cortex-alias-scrollbar-* token design-platform.css defines has a consumer,
...
const TOKEN_PREFIX = '--cortex-alias-scrollbar-'
/** Prefix of the rebindable indirection scrollbar.css owns. */
const INDIRECTION_PREFIX = '--cortex-scrollbar-'
...
const ELEVATED_REBIND = new Map([
  ['--cortex-scrollbar-thumb', '--cortex-alias-scrollbar-bg-l2'],
  ['--cortex-scrollbar-thumb-hover', '--cortex-alias-scrollbar-hover-l2'],
 ...
 * @returns every `--cortex-*` token the sheet's rendering declarations depend on.
 */
function tokensRendered(rules: CssRule[]): Set<string> {
  ...
  while (pending.length > 0) {
    for (const name of varReferences(pending.pop()!)) {
      if (name.startsWith('--cortex-')) reached.add(name)
 ...
 * so something a scrollbar can sit against. `--cortex-alias-button-*`,
 * `--cortex-alias-interactive-*`, and `--cortex-alias-markdown-*` reach the same dark
 ...
 */
const SURFACE_TOKEN_PATTERN = /^--cortex-(?:alias-bg-|specific-)/
...
function elevatedRungs(): Set<string> {
  ...
  const rungs = new Set([resolve('--cortex-alias-bg-layer-2'), resolve('--cortex-alias-bg-layer-3')])
    ...
    for (const rule of platformRules) {
      for (const [property, value] of rule.declarations) {
        ...
        for (const reference of varReferences(value)) {
          expect(reference, `${property}: ${value}`).toMatch(/^--cortex-static-/)
        }
      }
    }
  })
...
describe('scrollbar.css width variable', () => {
  ...
  it('every reader of the width variable outside ui-theme references a defined variable', () => {
    ...
    // (`right: var(--cortex-scrollbar-width)`); a rename in scrollbar.css without
...
describe('elevated surface rebinds', () => {
  ...
  it('rebinds the pair to one target: the l2 elevation pair, or transparent', () => {
    ...
    // `thumb-hover: var(--cortex-alias-scrollbar-hover-l2)` — which repaints the
    ...
    // A shape check admits `color-mix(…, var(--cortex-alias-scrollbar-bg-l2) 85%,
  ...
  it('resolves the elevated surface set from the palette ladder', () => {
    ...
    // defined. `--cortex-specific-tip` is the regression that proved the point: it
    ...
    expect(elevatedSurfaces).toContain('--cortex-alias-bg-layer-2')
    expect(elevatedSurfaces).toContain('--cortex-alias-bg-layer-3')
    expect(elevatedSurfaces).toContain('--cortex-specific-menu')
    expect(elevatedSurfaces).toContain('--cortex-specific-input-major')
    expect(elevatedSurfaces).toContain('--cortex-specific-tip')
    // Base surfaces stay out, or every scroll container would be in scope and
    // the check would say nothing.
    expect(elevatedSurfaces).not.toContain('--cortex-alias-bg-base')
    expect(elevatedSurfaces).not.toContain('--cortex-alias-bg-layer-1')
  })
  ...
  it('every sheet that scrolls on an elevated surface rebinds', () => {
    ...
    // `--cortex-alias-bg-*` and `--cortex-specific-*` name a surface, so a floating
