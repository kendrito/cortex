...
export type { Translate, TranslateNS } from '@cortex/client-ui-slots'
 ...
 * entry's namespace in the active locale -> that namespace's fallback-locale
 * dictionary -> the shared common namespace (active, then fallback) -> the key
 * itself (missing
...
export class LocaleRuntime {
  ...
  setLocale(id: string): void {
    ...
    // oxlint-disable-next-line typescript/no-unnecessary-condition -- always true with one shipped locale
    if (this.snapshot.active === match.id) return
    /* v8 ignore next 2 -- unreachable while one locale ships: every registered id is the active one. */
  ...
  private adopt(host: SettingsScope<LocaleSettings>): void {
    ...
    /* v8 ignore next -- unreachable while one locale ships: every stored preference resolves to the active locale. */
  ...
  private publish(active: LocaleId, localeChanged: boolean): void {
    ...
    /* v8 ignore next -- a locale change needs a second shipped locale; registrations publish with false. */
 ...
 * `fr-CA` -> fr). `window` is the browser test, not `navigator`: Node exposes
