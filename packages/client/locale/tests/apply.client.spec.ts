...
} from '@cortex/client-locale/client'
import type { LanguageRowInjected, LocaleRuntime } from '@cortex/client-locale/client'
...
describe('locale apply', () => {
  ...
  // pin an unshipped one to keep their en baseline independent of the test
  // environment (the service falls back to en).
  beforeEach(() => {
  ...
  it('projects service snapshots into the row store and routes face writes back', async () => {
    ...
    // A call ahead of any inject must not reach the unbound actions.
    locale.setLocale('en')
    ...
    // The face routes to the service: the only shipped locale is already
    // active, so the write is a no-op (no Host mutation), and an unshipped id
    // is refused by the service.
    face.setLocale('en')
    expect(locale.getLocale().active).toBe('en')
    expect(instance.getSnapshot().active).toBe('en')
    expect(() => { face.setLocale('fr') }).toThrow('not registered')
    expect(locale.bind(SETTINGS_NS)('language.title')).toBe('Language')
    await Promise.resolve()
