 ...
 */
const CSS_VIRTUAL_PREFIX = '\0cortex-css:'
const CSS_VIRTUAL_SUFFIX = '.mjs'

/**
 * Wire/type layers a client bundle may inline: browser-safe contracts
 * with no runtime identity to share (no Symbol/instanceof/singleton state).
 * Everything else under @cortex/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@cortex\/(host-apiproxy|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@cortex\/[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@cortex/client-runtime/client'
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.CORTEX_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.CORTEX_BUILD_FACE) === 'host'
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other @cortex value import is a build error — a
      ...
      name: 'cortex-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@cortex/')) return null
      ...
      name: 'cortex-css-modules-inline',
