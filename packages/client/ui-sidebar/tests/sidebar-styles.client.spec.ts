...
describe('SidebarRoot.module.css', () => {
  it('shares and cancels the wide shell trailing padding structurally', () => {
    ...
    expect(root?.get('--cortex-sidebar-inline-padding')).toBe('12px')
    expect(root?.get('padding')).toBe('6px var(--cortex-sidebar-inline-padding)')
    ...
    expect(declarations('.regionArea')?.get('margin-right')).toBe(
      'calc(-1 * var(--cortex-sidebar-inline-padding))',
    )
  ...
  it('moves the four upper controls while the settings seat only fades', () => {
    const animation = 'rail-in 150ms var(--cortex-ease-in-out) backwards'
    ...
    expect(declarations('.railIn .footArea')?.get('animation')).toBe(
      'rail-fade-in 150ms var(--cortex-ease-in-out) backwards',
    )
    expect(css).toMatch(
