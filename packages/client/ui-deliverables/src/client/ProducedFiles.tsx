...
import type { HostDescriptionSource } from '@cortex/client-connection/client'
import type { InjectFace, PropsLocale } from '@cortex/client-ui-slots'
import type { TurnTailOwnerProps } from '@cortex/client-ui-conversation/client'
import { activityForClosing, basename, type TurnActivity } from './turn-deliverables.ts'
...
export type ProducedFilesProps = Pick<TurnTailOwnerProps, 'openFile'> & {
  ...
  /** Owner Turn/seq for the activity counters; absent renders chips alone. */
  turn?: TurnTailOwnerProps['turn']
  seq?: TurnTailOwnerProps['seq']
} & PropsLocale<typeof NS> & InjectFace<ProducedFilesInjected>

/** Counter order and their locale keys; failed renders last, in its own tone. */
const COUNTERS: ReadonlyArray<[TurnActivityIntent, 'touched.read' | 'touched.search' | 'touched.execute' | 'touched.fetch' | 'touched.delete' | 'touched.move' | 'touched.other']> = [
  ['read', 'touched.read'],
  ['search', 'touched.search'],
  ['execute', 'touched.execute'],
  ['fetch', 'touched.fetch'],
  ['delete', 'touched.delete'],
  ['move', 'touched.move'],
  ['other', 'touched.other'],
]

type TurnActivityIntent = keyof TurnActivity['counts'] & string
...
export function ProducedFiles({
  matched: paths, turn, seq, openFile, isLoopback, useHostDescription, t,
}: ProducedFilesProps) {
  const activity = activityForClosing(turn?.data.get('deliverables'), seq)
  ...
  const counters = COUNTERS
    .map(([intent, key]) => ({ intent, key, count: activity.counts[intent] ?? 0 }))
    .filter(entry => entry.count > 0)
  return (
    <div className={css.root}>
      <span className={css.label}>{paths.length > 0 ? t('produced.label') : t('touched.label')}</span>
      <div ref={rowRef} className={css.row} data-produced-files-row>
        ...
        {counters.map(entry => (
          <span
            key={entry.intent}
            className={css.count}
            // Read counters carry their file list, so a hover answers "read what?".
            title={entry.intent === 'read' && activity.readPaths.length > 0
              ? activity.readPaths.slice(0, 8).map(basename).join('\n')
              : undefined}
          >
            {t(entry.key, { count: String(entry.count) })}
          </span>
        ))}
        {activity.failed > 0 && (
          <span className={`${css.count} ${css.countFailed}`}>
            {t('touched.failed', { count: String(activity.failed) })}
          </span>
        )}
      </div>
