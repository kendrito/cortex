...
import type { ConvViewProps, ViewTab } from '@cortex/client-ui-conversation/client'
import {
  ConversationSession, ConversationSessionHeader,
  type ConversationSessionHeaderProps, type ConversationSessionProps,
} from '@cortex/client-ui-conversation/src/client/skeleton/ConversationSession.tsx'
import { createChatStore } from '@cortex/client-ui-conversation/src/client/stores.ts'
import { apply as localeApply, inject as localeInject } from '@cortex/client-locale/client'
import { stubSettingsScope } from '@cortex/client-test-runtime'
import type { LocaleKeysOf } from '@cortex/client-ui-slots'
import { apply, inject } from '@cortex/client-ui-trajectory/client'
import { apply as nodeApply } from '@cortex/client-ui-trajectory'
...
import { en as conversationEn } from '@cortex/client-ui-conversation/src/client/locales.ts'
...
describe('plugin registration', () => {
  ...
  it('shares one browser-wide duration preference across session injections', async () => {
    ...
    expect(localStorage.getItem('cortex.trajectory.duration')).toBe('true')
    expect(localStorage.getItem(`cortex.trajectory.duration.${SID}`)).toBeNull()
  })
...
describe('tab switching in ConversationRoot', () => {
  ...
  it('labels the trajectory tab in the active locale', async () => {
    ...
    expect(labelOf()).toBe(en['view.trajectory'])
