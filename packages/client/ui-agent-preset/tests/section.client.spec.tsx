...
const READY: AgentPresetSectionState = {
  ...
  hasDocument: true,
  rows: [
    { id: 'standard', trust: 'system', isDefault: true, name: 'Standard mode', description: 'Full coding agent.' },
...
describe('the preset list', () => {
  ...
  it('shows a revealed directory on its row', () => {
    renderSection({ revealedPaths: { mine: '/home/user/.cortex/.agent-presets/mine' } })

    const mine = rowFor('mine')
    expect(within(mine).getByText('/home/user/.cortex/.agent-presets/mine')).toBeTruthy()
...
describe('a long card description', () => {
  ...
  const LONG = 'A friendly general-purpose assistant that keeps a persistent bash session and edits files. '.repeat(8).trim()
  ...
  it('offers the whole description on hover once the card cuts it off', () => {
    ...
    vi.useFakeTimers()
    try {
      renderSection({ rows: [{ id: 'helper', trust: 'user', isDefault: false, name: 'Helper', description: LONG }] })

      fireEvent.mouseEnter(within(rowFor('helper')).getByText(LONG))
  ...
  it('stays quiet when the description already fits', () => {
    ...
    vi.useFakeTimers()
    try {
      renderSection({ rows: [{ id: 'helper', trust: 'user', isDefault: false, name: 'Helper', description: 'Short description.' }] })

      fireEvent.mouseEnter(within(rowFor('helper')).getByText('Short description.'))
      act(() => { vi.advanceTimersByTime(400) })
  ...
  it('renders where the runtime has no ResizeObserver', () => {
    ...
    expect(() => {
      renderSection({ rows: [{ id: 'helper', trust: 'user', isDefault: false, description: LONG }] })
    }).not.toThrow()
    // The first measurement does not depend on the observer.
    expect(within(rowFor('helper')).getByText(LONG).getAttribute('title')).toBe('')
  })
})
