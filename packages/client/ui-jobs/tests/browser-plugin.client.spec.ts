...
describe('ui-job browser half', () => {
  ...
  it('registers its dictionary under its own namespace and releases it with the fiber', async () => {
    ...
    // A withdrawn dictionary leaves the key unresolved rather than translated.
    await fiber.dispose()
    expect(translate('list.aria')).not.toBe(en['list.aria'])
  })

  it('ships a non-empty template for every dictionary key', () => {
    for (const [key, template] of Object.entries(en)) {
      expect(template, key).not.toBe('')
    }
  })
})
