 ...
 */
import type { Context, Fiber } from '@cortex/cordis'
import type {
...
import { SESSION_SEARCH_RESULT_LIMIT } from '@cortex/host-apiproxy/api'
import type {
...
// cortex-scope, keyed by Agent identity); re-exported here so existing
...
/**
 * Increment a trailing parenthesized fork number; an unnumbered title starts
 * with ` (1)`.
...
export class SessionRuntime implements ISessions {
  ...
  constructor(
    ...
    this.selection = createSnapshotStore<SessionSelection>(
      {},
      { persist: { name: 'cortex.sessions.current' } })
