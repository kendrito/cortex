...
import type {} from '@cortex/commands/types'
...
describe('open', () => {
  ...
  it('installs the tail page: cold → loading → open with window and nodes in place', async () => {
    ...
    const page = plainTurn(10, 3, 'ask', 'answer')
  ...
  it('stitches live frames arriving while history is pending, dropping the page overlap', async () => {
    ...
    const page = plainTurn(10, 0, 'good', 'morning')
    session.handleMuxEnvelope('r1' as never, { type: 'session/event', sessionId: SID, event: ev.turnStart(15, 1) })
    session.handleMuxEnvelope('r2' as never, { type: 'session/event', sessionId: SID, event: ev.user(16, 'inserted') })
    gate.resolve(ok({
...
describe('live event path', () => {
  ...
  it('publishes animation-frame Definitions once per frame and lets an immediate event supersede the pending frame', async () => {
    ...
    feed(ev.chunkText(7, 1, 'accumu'))
    feed(ev.chunkText(8, 1, 'lated'))
    ...
    feed(ev.assistant(10, 1, 'accumulated done'))
    await Promise.resolve()
    expect(published).toEqual([
      [0, 1, 2, 3, 4, 5, 6, 7, 8],
...
describe('paging', () => {
  it('prepends an older page and keeps seq continuity', async () => {
    const older = plainTurn(0, 0, 'old ask', 'old answer')
    const newer = plainTurn(6, 1, 'new ask', 'new answer')
    const { api, session } = makeSession()
  ...
  it('installs a page without interpreting business replacement metadata', async () => {
    const { api, session } = makeSession()
    api.onHistory = () => histResponse([
      ev.compactSummary(80, 'summary of the range outside the window', 3, 40),
  ...
  it('drops a discontinuous older page fail-soft (window unchanged, hasMore cleared)', async () => {
    ...
    api.onHistory = payload => payload.beforeSeq === undefined
      ...
      : histResponse(plainTurn(0, 0, 'discontinuous', 'page'), true) // tail seq 5, but baseSeq is 10 → hole
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined)
    try {
      await session.open()
...
describe('remaining branches', () => {
  ...
  it('repairGap failure logs and clears stitching; concurrent gaps coalesce into one repair', async () => {
    ...
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined)
    try {
      session.handleMuxEnvelope('r1' as never, { type: 'session/event', sessionId: SID, event: ev.user(9, 'gap one') })
      session.handleMuxEnvelope('r2' as never, { type: 'session/event', sessionId: SID, event: ev.user(10, 'gap two') }) // stitching: detours, no second repair
  ...
  it('drops a stale doOpen whose history resolved successfully after resync superseded it', async () => {
    ...
    stale.resolve(ok({
      ...
      hasMore: false,
      modelSelection: { provider: 'cortex-official', model: 'stale' },
  ...
  it('drops a gap repair superseded by a full resync while its pull was in flight', async () => {
    ...
    session.handleMuxEnvelope('r' as never, { type: 'session/event', sessionId: SID, event: ev.user(9, 'gap') }) // starts repairGap
  ...
  it('carries history-entry and mux-frame views into the business-neutral Event input', async () => {
    const { api, session } = makeSession()
    const callView = { for: 'call', view: { card: 'generic', title: 'history call card' } }
    api.onHistory = () => Promise.resolve(ok({
      events: [
        ...entries(plainTurn(0, 0, 'a', 'b')),
        { event: ev.toolCall(6, 1, 'h1', 'bash', '{}'), view: callView },
        { event: ev.toolResult(7, 1, 'h1', 'done'), view: { for: 'result', view: { card: 'generic', title: 'history result card' } } },
      ] as never[],
      hasMore: false,
      modelSelection: { provider: 'cortex-official', model: 'cortex-v4-flash' },
    }))
    ...
    expect(chatEvents(session.getSnapshot()).slice(-2).map(item => item.view)).toEqual([
      callView,
      { for: 'result', view: { card: 'generic', title: 'history result card' } },
    ])
    session.handleMuxEnvelope('rv1' as never, {
      type: 'session/event', sessionId: SID, event: ev.toolCall(8, 2, 'l1', 'write', '{}'),
      view: { for: 'call', view: { card: 'generic', title: 'live call card' } },
    } as never)
    expect(chatEvents(session.getSnapshot()).at(-1)?.view).toEqual({
      for: 'call', view: { card: 'generic', title: 'live call card' },
    })
    session.handleMuxEnvelope('rv2' as never, {
      type: 'session/event', sessionId: SID, event: ev.toolResult(9, 2, 'l1', 'ok'),
      view: { for: 'result', view: { card: 'generic', title: 'live result card' } },
    } as never)
    expect(chatEvents(session.getSnapshot()).at(-1)?.view).toEqual({
      for: 'result', view: { card: 'generic', title: 'live result card' },
    })
  })
})
...
describe('reference stability (the memo contract)', () => {
  it('keeps unchanged node references across an append and swaps the snapshot object', async () => {
    const { api, session } = makeSession()
    api.onHistory = () => histResponse(plainTurn(0, 0, 'stable', 'reference'))
    await session.open()
    const before = session.getSnapshot()
  ...
  it('keeps unrelated Session arrays and settled Chat Nodes stable across Event updates', async () => {
    ...
    feed(ev.chunkText(10, 1, 'streaming unrelated to the tool'))
    const after = session.getSnapshot()
    expect(after).not.toBe(before)
