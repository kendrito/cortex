...
import type { StreamChunk } from '@cortex/api-remotes/client'
...
describe('PartialAccumulator', () => {
  ...
  it('accumulates text deltas, starting from empty when prev is missing or another kind', () => {
    ...
    acc.push(chunk({ type: 'reasoning-delta', index: 0, text: 'kind switch restarts' })) // prev is text → restart
    expect(acc.toPartial().blocks).toEqual([{ kind: 'reasoning', text: 'kind switch restarts' }])
  })

  it('accumulates reasoning deltas on the reasoning lane', () => {
    const acc = new PartialAccumulator(1, 0)
    acc.push(chunk({ type: 'block-start', index: 0, blockType: 'reasoning' }))
    acc.push(chunk({ type: 'reasoning-delta', index: 0, text: 'think' }))
    acc.push(chunk({ type: 'reasoning-delta', index: 0, text: 'ing' }))
  ...
  it('continues from a materialized history prefix', () => {
    ...
    acc.push(chunk({ type: 'text-delta', index: 0, text: ' delta' }))
    expect(acc.toPartial().blocks).toEqual([{ kind: 'text', text: 'existing delta' }])
  })
  ...
  it('replaces the accumulated block wholesale on block-end', () => {
    ...
    acc.push(chunk({ type: 'block-end', index: 0, block: { type: 'text', text: 'final full text' } }))
    expect(acc.toPartial().blocks).toEqual([{ kind: 'text', text: 'final full text' }])
  })
