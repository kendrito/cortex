/**
 * Node half of the client module system (`cortex.client` dual-face package): scans
 * the host Loader's entries for packages declaring `cortex.client`, composes the
 * `window.__CORTEX_BOOT__` entry graph (wire single source: {@link WebBootEntry}
...
/** package.json `cortex.client` declaration fields, validated one by one after reading the file. */
interface CortexClientDeclaration {
  inject?: string[]
  platform: string
...
/** Resolved package metadata for one `cortex.client` package (cached per name, never expires). */
interface PkgMeta {
  clientPath: string
  inject?: string[]
  immediately: boolean
...
/** Narrow an unknown parsed JSON value to the `cortex.client` declaration, throwing on malformed fields. */
function parseCortexClient(pkgName: string, value: unknown): CortexClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object cortex.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} cortex.client.platform must be a string`)
  }
  if (decl.inject !== undefined && (!Array.isArray(decl.inject) || decl.inject.some(i => typeof i !== 'string'))) {
    throw new Error(`client-modules: ${pkgName} cortex.client.inject must be a string array`)
  }
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} cortex.client.immediately must be a boolean`)
  }
  return {
...
/**
 * Inject the boot entry graph into index.html: `window.__CORTEX_BOOT__` as the
...
export function injectBootManifest(html: string, graph: WebBootGraph): string {
  ...
  const script = `<script>window.__CORTEX_BOOT__ = ${json}</script>`
...
/**
 * The web plugin table service: incremental `cortex.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `cortex.client` declaration) are
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const cortex = pkg.cortex
    const decl = parseCortexClient(
      pkgName,
      cortex !== null && typeof cortex === 'object' ? (cortex as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares cortex.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
