 ...
 * @module @cortex/client-modules/client
 */
import type { Context } from '@cortex/cordis'
import type { CortexWindow } from './manifest.ts'
...
export type {
  ...
  ClientModuleSystemOptions, ClientPluginHandoff, CortexWindow, WebBootEntry, WebBootGraph,
} from './manifest.ts'
...
export function apply(ctx: Context): void {
  const modules = (globalThis as CortexWindow).__CORTEX_MODULES__
  ...
  if (modules === undefined) {
    throw new Error('client-modules: window.__CORTEX_MODULES__ missing — the shell kernel must construct the module system before plugin boot')
  }
