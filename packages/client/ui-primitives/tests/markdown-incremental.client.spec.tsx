...
import { MarkdownText } from '@cortex/client-ui-primitives'
...
describe('multibyte content', () => {
  const ASTRAL_DOC = [
    '# Résumé 🎉',
    '',
    'A paragraph with **bold**, `inline code`, and the emoji 😀🚀 in an accented café.',
    '',
    '- List item one ✅',
    '- List item two',
    '',
    ...
    '| Alpha | Beta |',
    '',
    'Closing paragraph, with enough blocks to freeze everything before it. 🌊',
  ].join('\n')

  it('code-unit chunking (splitting surrogate pairs mid-stream) matches fresh renders', () => {
    const live = render(<MarkdownText text="" streaming />)
    for (let end = 1; end < ASTRAL_DOC.length + 1; end += 1) {
      const prefix = ASTRAL_DOC.slice(0, Math.min(end, ASTRAL_DOC.length))
      live.rerender(<MarkdownText text={prefix} streaming />)
      const fresh = render(<MarkdownText text={prefix} streaming />)
      expect(live.container.innerHTML).toBe(fresh.container.innerHTML)
      fresh.unmount()
  ...
  it('freeze-cut offsets agree with one-shot parse offsets on astral content', () => {
    const parser = new IncrementalMarkdownParser(parseGfm)
    let result = parser.update(ASTRAL_DOC.slice(0, 3))
    for (let end = 6; end < ASTRAL_DOC.length + 3; end += 3) {
      result = parser.update(ASTRAL_DOC.slice(0, Math.min(end, ASTRAL_DOC.length)))
    }
    const oneShot = parseGfm(ASTRAL_DOC).children.map(node => node.position?.start.offset)
