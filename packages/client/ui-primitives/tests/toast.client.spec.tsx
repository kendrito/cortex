...
describe('Toast', () => {
  it('announces its text and reports done after the hold-and-fade lifetime', () => {
    ...
    try {
      const onDone = vi.fn()
      const view = render(<Toast text="A message can include up to 50 images" icon={<svg data-testid="icon" />} onDone={onDone} />)
      const banner = view.getByRole('alert')
      expect(banner.textContent).toContain('A message can include up to 50 images')
