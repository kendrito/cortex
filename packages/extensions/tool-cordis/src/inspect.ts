 ...
 * `name` adds.
 * @module @cortex/tool-cordis/inspect
 */
...
import type {} from '@cortex/cordis-host-runner'
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  if (rows.length === 0) {
    return ['No dynamic Plugins are defined in this session. Definitions live only in this process\'s memory, so a Cortex restart clears them.']
  }
