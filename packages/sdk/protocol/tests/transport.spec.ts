...
describe('JsonRpcLineTransport', () => {
  ...
  it('preserves multibyte UTF-8 characters split across Buffer chunks', async () => {
    ...
    const frame = Buffer.from(`${JSON.stringify({ jsonrpc: '2.0', method: 'message', params: { text: 'Größe' } })}\n`)
    const character = Buffer.from('ö')
    ...
    expect(notifications).toEqual([{ method: 'message', params: { text: 'Größe' } }])
    transport.close()
  })
