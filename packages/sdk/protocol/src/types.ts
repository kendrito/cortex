/**
 * Named wire types for the Cortex SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@cortex/sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `cortex-sdk-runtime`.
 *
 * @module @cortex/sdk-protocol/types
 */

import type { ContentBlock } from '@cortex/llm'
import type { SessionEvent } from '@cortex/session'
import type { SubagentStopReason } from '@cortex/subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`cortex-sdk-runtime`) and version. */
