/**
 * High-level run API over {@link HarnessClient}: `CortexHarness` owns one
 ...
 * Mirrors the Python SDK's `CortexHarness`/`Session` pair.
 *
 * @module @cortex/sdk-client/api
 */
...
import type { ContentBlock, CortexHarnessOptions, HarnessClientOptions, HarnessNotification, RunResult } from './types.ts'

/**
 * Reusable SDK for running Cortex agent turns in a runtime
 ...
 * child is reaped.
 */
export class CortexHarness implements AsyncDisposable {
  ...
  constructor(options: CortexHarnessOptions) {
    ...
    this.provider = options.provider ?? 'cortex-official'
    this.model = options.model ?? 'cortex-v4-flash'
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: CortexHarness, readonly id: string) {}
