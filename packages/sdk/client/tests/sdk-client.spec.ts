...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): CortexHarness {
  const harness = new CortexHarness({ launch: fakeLaunch(env, extra) })
...
describe('CortexHarness', () => {
  ...
  it('resolves a relative launch cwd to an absolute workspace before the handshake', async () => {
    ...
    const dir = await mkdtemp(join(process.cwd(), '.cortex-sdk-client-relcwd-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const recordFile = join(dir, 'init.jsonl')
  ...
  it('supports await using disposal', async () => {
    let captured: CortexHarness
    {
      await using harness = new CortexHarness({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('fails fast when the command does not exist', async () => {
    const client = new HarnessClient({ command: join(tmpdir(), 'cortex-no-such-runtime-bin') })
    cleanups.push(() => client.close())
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('cortex-sdk-runtime')
