...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@cortex/tools'
import type { WebSearchResult, WebSearchSource } from '@cortex/web'
import type {} from '@cortex/system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `cortex-tool-fs`'s
 ...
 *   `ToolDefinition.timeoutMs` for `@cortex/tool-call-timeout-policy` to enforce.
