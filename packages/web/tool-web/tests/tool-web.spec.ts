...
import type { WebSearchProvider, WebSearchResult } from '@cortex/web'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@cortex/web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by cortex-tool-call-timeout-policy over exec.signal.
