/**
 * Integration: the real fetch backend (`cortex-web-fetch-http`) + a real search provider
 * (`cortex-web-search-exa`) + the real seam (`cortex-web`) + the model tool (`cortex-tool-web`) + the
 * tool-call timeout policy (`cortex-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@cortex/tools'
...
import * as TimeoutPolicy from '@cortex/tool-call-timeout-policy'
...
describe('web_search integration over the real Exa provider', () => {
  it('runs web_search end-to-end and formats the provider result', async () => {
    ...
    const out = await call('web_search', { query: 'cortex-official' })
    expect(out.isError).toBe(false)
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by cortex-tool-call-timeout-policy,
