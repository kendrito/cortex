 ...
 * (`cortex-spill-local` backend + `cortex-spill-policy`), exercised through
...
import LocalSpillStore from '@cortex/spill-local'
...
beforeEach(async () => {
  ...
  spillRoot = mkdtempSync(join(tmpdir(), 'cortex-spill-web-'))
