...
export class WebRuntime extends Service {
   ...
   * `$CORTEX_WEB_SEARCH_PROVIDER` / `$CORTEX_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.CORTEX_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.CORTEX_WEB_FETCH_PROVIDER
  }
