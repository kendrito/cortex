/**
 * Profile machinery of `cortex-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'cortex-profile-'))
...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { cortex: { bundle: { patch: './cordis.patch.yml' } } },
    }))
    if (spec.patch !== undefined) writeFileSync(join(dir, 'cordis.patch.yml'), spec.patch)
  }
  writeFileSync(join(appDir, 'package.json'), JSON.stringify({ name: 'cortex-app', dependencies: appDeps }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    expect(readProfileManifest('t', dir).cortex?.profile?.bundles).toEqual(['@cortex/base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', cortex: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).cortex?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each cortex.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or cortex section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @cortex/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@cortex/base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).cortex?.profile?.bundles)
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@cortex/base': { patch: '[]\n' },
      '@cortex/web-app': { patch: '[]\n' },
      '@cortex/headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@cortex/base', '@cortex/web-app', '@cortex/headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).cortex?.profile?.bundles)
      .toEqual(['@cortex/base', '@cortex/headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@cortex/base', '@cortex/web-app', '@cortex/headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).cortex?.profile?.bundles).toEqual([
      '@cortex/base', '@cortex/web-app', '@cortex/headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no cortex.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no cortex.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', () => {
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'cortex-app']) {
  ...
  it('throws when a fallback entry is a real directory', () => {
    const anchor = stageInstallation({})
    const home = tmp()
    mkdirSync(join(home, 'profiles', 'node_modules', 'cortex-app'), { recursive: true })
  ...
  it('replaces a wrong symlink', () => {
    ...
    symlinkSync(tmp(), join(fallback, 'cortex-app'), 'junction')
    healProfilesModuleFallback(anchor, home)
    expect(readlinkSync(join(fallback, 'cortex-app'))).toContain('app')
  })
  ...
  it('tolerates losing the concurrent-heal race to an identical link and rejects a different one', () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    expect(lstatSync(join(fallback, 'cortex-app')).isSymbolicLink()).toBe(true)
  })
})
