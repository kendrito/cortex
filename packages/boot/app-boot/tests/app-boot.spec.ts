...
const NAME = 'cortex-test-bin'

const tmp = (): string => mkdtempSync(join(tmpdir(), 'cortex-app-boot-'))
...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'CORTEX_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['CORTEX_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['CORTEX_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'CORTEX_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['CORTEX_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['CORTEX_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'CORTEX_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'CORTEX_AGENTS_HOME=/tmp/injected\n'],
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@cortex', 'cortex-system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@cortex', 'cortex-system-prompt')
  ...
  it('exposes cortexHomePath to Loader config expressions', async () => {
    const dir = tmp()
    const cortexHome = join(dir, 'home')
    vi.stubEnv('CORTEX_HOME', cortexHome)
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js cortexHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(cortexHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      const identityAt = rendered.indexOf('You are an AI agent powered by Cortex.')
