 ...
 * `cortex --profile` launcher family.
 *
 * A profile is a directory under `$CORTEX_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `cortex.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"cortex": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `cortex.profile.bundles` order over
 ...
 * first from the cortex installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$CORTEX_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @cortex/app-boot/profile
 */
...
/** The bundle half of the `cortex` manifest section: what a bundle package exports. */
export interface CortexBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `cortex` manifest section: what a profile directory composes. */
export interface CortexProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `cortex`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface CortexManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: CortexBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: CortexProfileManifest
}
...
export interface ProfileManifest {
  ...
  cortex?: CortexManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `cortex.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `cortex.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`cortex --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveCortexHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveCortexHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`cortex: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@cortex/base', '@cortex/web-app'],
  headless: ['@cortex/base', '@cortex/headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@cortex/base', '@cortex/web-app', '@cortex/headless'],
}

/** The bundle list a `cortex plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@cortex/base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this cortex profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `cortex.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `cortex-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      cortex: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`cortex: ${link} exists and is not a symlink; remove it so cortex can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$CORTEX_HOME/profiles/node_modules`: one
 * symlink per package in the cortex app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`cortex-compaction`, `cortex-invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the cortex app's package.json.
 * @param home - the Harness home; defaults to {@link resolveCortexHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveCortexHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (cortex-subprocess,
    // cortex-compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.cortex?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    cortex: {
      ...manifest.cortex,
      profile: { ...manifest.cortex?.profile, bundles: [...current] },
    },
  }
 ...
 * `@cortex/base` (and every other in-box bundle) always comes from
 * the same installation as the running cortex, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `cortex.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the cortex app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the cortex installation or ${profileDir}; `
    + `run 'cortex plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `cortex.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `cortex.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the cortex app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveCortexHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveCortexHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'cortex plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the cortex section entirely.
  const bundles = manifest.cortex?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.cortex?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no cortex.bundle in its package.json`)
    }
