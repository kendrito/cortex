/**
 * Shared boot glue for the app bins (`cortex`, `cortex-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.cortex`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @cortex/app-boot
 */
...
import { Context, type FiberState } from '@cortex/cordis'
import Loader, { type Entry, type EntryOptions } from '@cortex/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@cortex/cordis-plugin-include'
import Group from '@cortex/cordis-plugin-group'
import { cortexHomePath, resolveCortexHome } from '@cortex/home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@cortex/launch-environment'
import type {} from '@cortex/cordis-plugin-hmr'
...
declare module '@cortex/cordis' {
  interface Context {
    ...
    cortexHomePath?: typeof cortexHomePath
  }
}
...
export {
  ...
  type CortexBundleManifest,
  type CortexManifestSection,
  type CortexProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$CORTEX_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_NAMES = new Set([
  ...
  'CORTEX_BASE_URL', 'CORTEX_SEARCH_BASE_URL',
...
const BOOTSTRAP_PREFIXES = ['CORTEX_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * entries (`@cortex/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@cortex/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@cortex/cordis-plugin-group`
 ...
 * directory. The self-referential `cortex-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The Cortex implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend Cortex itself.`,
  })
}
