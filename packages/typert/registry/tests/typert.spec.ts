...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@cortex/tools', 'ToolInput')).toBe('@cortex/tools#ToolInput')
    expect(typertPackageKey('@cortex/tools', 'host')).toBe('@cortex/tools#host')
    expect(ctx.typert.get('@cortex/tools#ToolInput')).toMatchObject({
      package: '@cortex/tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@cortex/tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@cortex/tools', 'host')).toMatchObject({
      key: '@cortex/tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@cortex/tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@cortex/tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@cortex/tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@cortex/tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@cortex/tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@cortex/tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@cortex/tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@cortex/tools#ToolInput')).not.toBe(projected)
  })
