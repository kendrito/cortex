...
`)
  ...
  it('validates Remote artifacts only on the host face of a dual-face package', () => {
    ...
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as {
      cortex?: { client?: object }
      exports: Record<string, unknown>
      files: string[]
    }
    manifest.cortex = { client: {} }
...
function copyFixture(sourceRoot = fixtureRoot): string {
  const root = mkdtempSync(join(tmpdir(), 'cortex-typert-remote-model-'))
...
`
  ...
  writeFileSync(configPath, JSON.stringify({
    ...
    compilerOptions: {
      ...
      skipLibCheck: false,
      paths: {
        '@cortex/typert-protocol': ['./typert-protocol.d.ts'],
...
import type { TypertRemoteNamespaceMap } from '@cortex/typert-protocol'
declare const ctx: { remote: TypertRemoteNamespaceMap }
