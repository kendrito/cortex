...
import type { TypertContribution } from '@cortex/typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@cortex/tool-cordis/src/api-catalog.ts'
...
describe('model-driven cortex-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@cortex/tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('cortex-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@cortex/tools')
    ...
    const record = ctx.typert.getPackage('@cortex/tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@cortex/tools', 'host')).toBeUndefined()
  })
})
