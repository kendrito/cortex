...
import * as typertLoader from '@cortex/typert-loader'
import { validateTypertManifest } from '@cortex/typert-loader'
import { z } from 'zod'
...
afterEach(async () => {
  ...
  context = undefined
  Reflect.deleteProperty(globalThis, '__cortexTypertLoaderGate')
...
describe('typert loader', () => {
  it('registers an explicit package without a Loader entry and withdraws it with the loader', LOADER_TEST_TIMEOUT, async () => {
    root = await mkdtemp(join(tmpdir(), 'cortex-typert-loader-'))
    await linkZod(root)
  ...
  it('drops an in-flight manifest when the loader is disposed before import settles', LOADER_TEST_TIMEOUT, async () => {
    ...
    Reflect.set(globalThis, '__cortexTypertLoaderGate', {
    ...
    await writePackage(root, '@fixture/pending', {
      typertSource: [
        'import { z } from \'zod\'',
        'globalThis.__cortexTypertLoaderGate.started()',
        'await globalThis.__cortexTypertLoaderGate.wait',
