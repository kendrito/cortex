...
import { createScope, type ScopeKey } from '@cortex/scope'
import { describe, expect, it } from 'vitest'
import * as Persona from '@cortex/persona'
import { PERSONA_SECTION } from '@cortex/persona'
...
describe('the persona row', () => {
  ...
  it('interpolates prompt variables strictly, like any other section', async () => {
    const ctx = await harness('')
    const key: ScopeKey = { agent: 'a1' }
    ctx.systemPrompt.variable('model', () => 'cortex-v4-pro')
    ...
    expect(renderPrompt(await ctx.systemPrompt.assemble({ scope: key })))
      .toContain('You run on cortex-v4-pro.')
  })
