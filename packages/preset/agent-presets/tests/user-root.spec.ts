 ...
 * authors into `<cortexHome>/.agent-presets`, the way `cortex-skill-filesystem` owns
 * `<cortexHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$CORTEX_HOME` is repointed per test because the derived root is resolved in the
...
import AgentPresets, { COMPOSITION_FILE, type Config } from '@cortex/agent-presets'
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'cortex-preset-home-'))
...
describe('the harness-home preset root', () => {
  ...
  it('yields to a configured user root for authoring, which writableRoot takes first', async () => {
    const explicit = await mkdtemp(join(tmpdir(), 'cortex-preset-explicit-'))
    const ctx = await roster({
      roots: [
