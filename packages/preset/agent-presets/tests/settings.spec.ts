...
import AgentPresets, { COMPOSITION_FILE, SETTINGS_NAMESPACE } from '@cortex/agent-presets'
...
async function harness(
  ...
  const home = await mkdtemp(join(tmpdir(), 'cortex-preset-settings-'))
...
describe('the default preset as a user setting', () => {
  ...
  it('clears a user default it has just deleted', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-preset-authored-'))
