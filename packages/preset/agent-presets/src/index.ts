 ...
 * @module @cortex/agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context, Service } from '@cortex/cordis'
import z from '@cortex/schemastery'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@cortex/scope'
// Type-only: resolves the `agent/created` lifecycle event this service watches.
import type {} from '@cortex/agent'
import { settingsNamespace, type SettingsScope, type default as SettingsService } from '@cortex/settings'
...
export class AgentPresets extends Service {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = config.includeUserRoot
      ? [...config.roots, { path: cortexHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...config.roots]
   ...
   * scope key. The binding is cortex-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `cortex-skill-filesystem` watches its
