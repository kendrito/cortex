...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@cortex/shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@cortex/hook-protocol'
import type { RunHookOptions } from '@cortex/hook-protocol'
 ...
 * executor (cortex-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what cortex-bash-local's resolve does for the fields runHook sets.
      return {
