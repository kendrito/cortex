...
The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@cortex/hooks-claude-code`, `@cortex/hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`cortex-hook-protocol`) | The bridge (`cortex-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `cortex-hooks-claude-code` and `cortex-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
