...
import SubagentRuntime, { SubagentRunId } from '@cortex/subagent'
import * as HooksClaude from '@cortex/hooks-claude-code'
import { MockAdapter, textResponse, toolCallResponse } from '../../../core/agent-loop/tests/mock-adapter.ts'

/**
 * Full-loop bridge tests: a scripted mock MODEL drives the REAL agent loop + REAL
 * bash executor, and the REAL `cortex-hooks-claude-code` bridge runs REAL shell hook
...
function writeConfig(hooks: unknown, scripts: Record<string, string> = {}): string {
  const dir = mkdtempSync(join(tmpdir(), 'cortex-hooks-claude-'))
  dirs.push(dir)
