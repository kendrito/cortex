/**
 * File-backed credentials provider over `$CORTEX_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $CORTEX_HOME/.credentials.yaml      (provider-managed, writable)
 * > <invocation cwd>/.env            (read-only fallback)
 * > $CORTEX_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `CORTEX_API_KEY=… cortex`, a CI
 ...
 * @module @cortex/credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@cortex/atomic-write'
import { canonicalizeWatchPath, resolveCortexHome } from '@cortex/home-paths'
import { launchEnvironmentOf } from '@cortex/launch-environment'
import { CredentialProvider, credentialRef } from '@cortex/credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@cortex/credentials'
import type { LaunchEnvironmentEntry } from '@cortex/launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveCortexHome(config.cortexHome), CREDENTIALS_FILENAME)),
...
/** File-backed credentials provider (`$CORTEX_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start cortex from instead',
      )
    }
  }
