...
const KEY = credentialRef('CORTEX_CRED_PIPE')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-credentials-watch-'))
...
describe('watcher pipeline', () => {
  ...
  it('survives a watcher error and keeps publishing later edits', async () => {
    ...
    await writeCredentials(path, 'CORTEX_CRED_PIPE: arrived\n')
  ...
  it('keeps the last good snapshot when the file turns unreadable at runtime', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'CORTEX_CRED_PIPE: good\n')
    const ctx = await boot({ path, debounceMs: 5 })
  ...
  it('keeps the reload queue alive after an invariant violation escapes the fan-out', async () => {
    ...
    await writeCredentials(path, 'CORTEX_CRED_PIPE: first\n')
    instance!.watcher.emit('all', 'change', path)
    ...
    arm = false
    await writeCredentials(path, 'CORTEX_CRED_PIPE: second\n')
    instance!.watcher.emit('all', 'change', path)
    await vi.waitFor(async () => {
  ...
  it('quiesces the refresh pipeline before dispose completes', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'CORTEX_CRED_PIPE: initial\n')
    const ctx = new Context()
    ...
    await writeCredentials(path, 'CORTEX_CRED_PIPE: changed\n')
    const [instance] = await fakeInstances()
  ...
  it('empties the snapshot when the document is deleted and emits the removals', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'CORTEX_CRED_PIPE: doomed\n')
    const ctx = await boot({ path, debounceMs: 5 })
    const seen: string[] = []
  ...
  it('keeps the last good snapshot when an external edit makes the document invalid', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'CORTEX_CRED_PIPE: a\n')
    const ctx = await boot({ path, debounceMs: 5 })
    const seen: string[] = []
    ctx.on('credentials/updated', (ref) => {
      seen.push(ref)
    ...
    await writeCredentials(path, 'CORTEX_CRED_PIPE: b\n')
    instance!.watcher.emit('all', 'change', path)
    await vi.waitFor(async () => {
