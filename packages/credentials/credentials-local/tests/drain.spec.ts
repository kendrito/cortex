...
vi.mock('@cortex/atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@cortex/atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@cortex/atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
  mocked.__setGate(next)
}

const KEY = credentialRef('CORTEX_CRED_DRAIN_A')
const OTHER = credentialRef('CORTEX_CRED_DRAIN_B')
...
describe('write-drain teardown', () => {
  it('lets the in-flight write land and fails the queued one after disposal', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'cortex-credentials-drain-'))
