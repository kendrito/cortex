...
`resolveCortexHome()` resolves the single-root Cortex home. Precedence, highest first: an explicit configured path, `$CORTEX_HOME`, then `~/.cortex`. The harness keeps all user data under one root.

`cortexHomePath(...segments)` joins child segments onto that resolved home with Node's platform path rules. With no segments it returns the home itself.

`cortexHomeDisplay()` names an active root symbolically for user-facing paths: `~/.cortex` for the default home, `$CORTEX_HOME` for any configured home. It never leaks an absolute machine path.

`CORTEX_HOME_DIR_NAME` owns the default user-data directory name: `.cortex`.

`defaultCortexHome()` returns the default Cortex home by joining the operating-system home directory with `.cortex`, using Node's platform path rules.
