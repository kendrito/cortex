 ...
 *
 * @module @cortex/acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `cortex-llm-replay` via `$CORTEX_SNAPSHOT_CHILD_FILES` so each child
...
export function snapshotSpillRoot(
  ...
  return `${root}/cortex-acp-snap-${key}`
}
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      CORTEX_SNAPSHOT: opts.mode,
      CORTEX_SNAPSHOT_FILE: opts.fixtureFile,
      CORTEX_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      CORTEX_SNAPSHOT_SPILL_ROOT: spillRoot,
      CORTEX_HOME: join(cwd, '.cortex'),
      CORTEX_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { CORTEX_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { CORTEX_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in cortex-llm-replay's loadSessionScripts
