...
function snapshotMode(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    default: throw new Error(`unknown CORTEX_SNAPSHOT mode: ${value}`)
  }
}
...
defineAcpSnapshotSuite({
  ...
  mode: snapshotMode(process.env.CORTEX_SNAPSHOT),
})
```
...
- **Built mode requires current artifacts** — run `pnpm run build` before selecting `CORTEX_EXAMPLE_MODE=lib`; source mode remains the zero-build path.
