 ...
 * zero-build dev path, resolving `@cortex/*` / `@cordisjs/*` through the tsconfig `paths`
...
export const EXAMPLE_MODE_ENV = 'CORTEX_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.CORTEX_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated Cortex homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { CORTEX_HOME: join(cwd, '.cortex'), CORTEX_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
