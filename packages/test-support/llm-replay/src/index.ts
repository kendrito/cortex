 ...
 * @module @cortex/llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@cortex/cordis'
import type {} from '@cortex/compaction'
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@cortex/llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.CORTEX_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $CORTEX_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.CORTEX_SNAPSHOT_OVERRIDE
  const childEnv = process.env.CORTEX_SNAPSHOT_CHILD_FILES
