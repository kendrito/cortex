...
|---|---|---|---|
| `file` | string | `$CORTEX_SNAPSHOT_FILE` | Path to the primary (parent) `session.jsonl` fixture. Required (config or env). |
| `overrideFile` | string | `$CORTEX_SNAPSHOT_OVERRIDE` | Optional `ReplayOverrideDoc` sidecar for the primary session: a bare `ReplayEntry[]` replaces its derived script, while `{ patches }` augments it by call index. |
| `childFiles` | string[] | `$CORTEX_SNAPSHOT_CHILD_FILES` (path-delimited) | Recorded subagent child-session logs for a nested scenario; empty for a single-session scenario. |
    ...
    providers:
      - id: cortex-official
        ...
        models:
          ...
          - id: cortex-v4-pro
  # file/overrideFile/childFiles default to $CORTEX_SNAPSHOT_FILE /
  # $CORTEX_SNAPSHOT_OVERRIDE / $CORTEX_SNAPSHOT_CHILD_FILES, set by the snapshot
