...
Point the shipping Cortex adapter at the server; it appends `/chat/completions` to the configured base:

```sh
CORTEX_BASE_URL=http://127.0.0.1:8000/v1 \
CORTEX_API_KEY=mock-key \
pnpm cortex --profile headless "test provider recovery"
```
