...
- id: command-compact
  name: '@cortex/command-compact'
```

The shipped `cortex` base mounts it beside `compaction-basic`, and the Web client provides the command adapter. Automation surfaces that compose no command adapter keep automatic compaction only.
