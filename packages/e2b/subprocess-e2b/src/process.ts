...
import type { CommandHandle, CommandResult, Sandbox } from '@cortex/e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$cortex_e2b_env_bin" -i "$cortex_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$cortex_e2b_tee" --output-error=warn-nopipe >("$cortex_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$cortex_e2b_tee" --output-error=warn-nopipe >("$cortex_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
    : `2> >(${encoder} >&2 2>/dev/null)`
  const inner = [
    'set +e',
    'cortex_e2b_env_bin=$1',
    'cortex_e2b_node=$2',
    'cortex_e2b_ps=$3',
    'cortex_e2b_tr=$4',
    'cortex_e2b_tee=$5',
    ...
    'cortex_e2b_rm=$7',
    'shift 7',
    'cortex_e2b_pgid="$("$cortex_e2b_ps" -o pgid= -p "$$" | "$cortex_e2b_tr" -d " ")"',
    `printf '%s\\n' "$cortex_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t cortex_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$cortex_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$cortex_e2b_env_bin" -i -- "\${cortex_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'cortex_e2b_status=$?',
    `printf '%s\\n' "$cortex_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$cortex_e2b_status"',
  ].join('\n')
  const argv = spec.argv.map(quoteE2BShellArg).join(' ')
  const bootstrap = [
    `mapfile -d '' -t cortex_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'cortex_e2b_env_bin="$(command -v env)"',
    'cortex_e2b_setsid="$(command -v setsid)"',
    'cortex_e2b_bash="$(command -v bash)"',
    'cortex_e2b_node="$(command -v node)"',
    'cortex_e2b_ps="$(command -v ps)"',
    'cortex_e2b_tr="$(command -v tr)"',
    'cortex_e2b_tee="$(command -v tee)"',
    'cortex_e2b_head="$(command -v head)"',
    'cortex_e2b_rm="$(command -v rm)"',
    'for cortex_e2b_tool in "$cortex_e2b_env_bin" "$cortex_e2b_setsid" "$cortex_e2b_bash" "$cortex_e2b_node" "$cortex_e2b_ps" "$cortex_e2b_tr" "$cortex_e2b_tee" "$cortex_e2b_head" "$cortex_e2b_rm"; do',
    '  [[ "$cortex_e2b_tool" == /* && -x "$cortex_e2b_tool" ]] || exit 125',
    'done',
    `exec "$cortex_e2b_env_bin" -i -- "\${cortex_e2b_env[@]}" "$cortex_e2b_setsid" --wait -- "$cortex_e2b_bash" -c ${quoteE2BShellArg(inner)} cortex-e2b "$cortex_e2b_env_bin" "$cortex_e2b_node" "$cortex_e2b_ps" "$cortex_e2b_tr" "$cortex_e2b_tee" "$cortex_e2b_head" "$cortex_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
