...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'cortex_state=$1',
  'mapfile -d \'\' -t cortex_env < "$cortex_state/environment"',
  'mapfile -d \'\' -t cortex_argv < "$cortex_state/argv"',
  'cortex_output_marker=$(<"$cortex_state/output-marker")',
  'rm -f -- "$cortex_state/environment" "$cortex_state/argv" "$cortex_state/output-marker" "$cortex_state/runner.bash"',
  'if (( ${#cortex_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$cortex_output_marker"',
  'exec env -i -- "${cortex_env[@]}" "${cortex_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`cortex-e2b-bootstrap:${randomUUID()}`)
