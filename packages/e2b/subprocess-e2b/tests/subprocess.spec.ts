...
import type E2BRuntime from '@cortex/e2b'
import type { SubprocessSpawnSpec } from '@cortex/subprocess'
import E2BSubprocessRuntime from '@cortex/subprocess-e2b'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=Größe\0NPM_TOKEN=secret\0CORTEX_STALE=old\0BROKEN\0=bad\0'
...
function runtime(fake: FakeSandbox, getSandbox: () => Promise<Sandbox> = async () => fake.sandbox): E2BRuntime {
  return {
    cwd: '/workspace',
    runtimeRoot: '/workspace/.cortex-e2b',
    getSandbox,
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.cortex-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$cortex_e2b_env_bin" -i'))!
    expect(command).toContain('"$cortex_e2b_setsid" --wait -- "$cortex_e2b_bash" -c')
    expect(command).not.toContain('CORTEX_API_KEY')
    expect(command).not.toContain('CORTEX_MODE')
    ...
    expect(command).not.toContain('${!cortex_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$cortex_e2b_home" -a -d "$cortex_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('cortex_e2b_node="$(command -v node)"')
    expect(command).toContain('"$cortex_e2b_env_bin" -i "$cortex_e2b_node" -e')
    expect(command).toContain('"$cortex_e2b_env_bin" -i -- "${cortex_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$cortex_e2b_env_bin" -i -- "${cortex_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.cortex-e2b/processes/one/pid',
      '/workspace/.cortex-e2b/processes/one/exit-code',
      '/workspace/.cortex-e2b/processes/one/environment',
      '/workspace/.cortex-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.cortex-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=Größe\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0CORTEX_API_KEY=explicit-secret\0CORTEX_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.cortex-e2b/processes/one/stderr.log')
  ...
  it('preserves UTF-8 bytes when the ASCII transport is split across callbacks', async () => {
    ...
    for (const character of `${Buffer.from('AGrößeB').toString('base64')}\n`) {
      await fake.stdoutWire(character)
    }
    fake.finish()
    await expect(handle.done).resolves.toEqual({ exitCode: 0, signal: null })
    expect(Buffer.concat(chunks).toString('utf8')).toBe('AGrößeB')
  })
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('cortex_e2b_tee='))!
    expect(command).toContain('"$cortex_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$cortex_e2b_tee" --output-error=warn-nopipe')
