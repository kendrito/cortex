...
import { Inbox } from '@cortex/agent'
...
import TerminalSessionService, { TerminalSessionId } from '@cortex/terminal'
import { BashTerminalBackend } from '@cortex/terminal-bash'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', CORTEX_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/cortex-e2b-bootstrap-profile-leak'
      ...
      ctx.provide('e2b', {
        cwd: '/home/user',
        runtimeRoot: '/home/user/.cortex-e2b',
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'cortex_leak=0',
          'for cortex_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$cortex_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$cortex_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then cortex_leak=1; fi',
          'done',
          'printf "DIRECT=<%s> LEAK=<%s>\\n" "${NPM_TOKEN-}" "$cortex_leak"',
        ].join('\n')],
        cwd: '/home/user',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> Cortex=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$CORTEX_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> Cortex=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'cortex-e2b-composition-',
      binScript,
    ...
    expect(output).toMatchObject({
      ...
      splitUtf8Output: 'Größe',
      hover: {
        kind: 'hover',
        hover: { contents: '**remote hover** naïve café' },
      },
      definition: {
        kind: 'locations',
    ...
    expect(terminalMotd).not.toContain('.cortex-e2b/terminals/')
    expect((output.terminal as { echo: { viewport: string } }).echo.viewport).toContain('PTY-Größe')
    expect((output.terminal as { scrollback: string }).scrollback).toContain('PTY-Größe')
