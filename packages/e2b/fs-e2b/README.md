...
E2B implementation of the [`@cortex/fs`](../../fs/fs/README.md) provider contract. It has no config: load [`@cortex/e2b`](../e2b/README.md) first, then this service in place of `cortex-fs-local`. The provider uses the owner's remote cwd and SDK handle, so file tools observe the same world as E2B-backed Bash processes.
...
Indirectly, through [`cortex-tool-fs`](../../fs/tool-fs/README.md), which renders remote UTF-8 content, directory results, mutation acknowledgements, and provider errors while E2B identity and transport remain internal.
