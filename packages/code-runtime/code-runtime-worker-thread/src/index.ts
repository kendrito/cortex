 ...
 * @module @cortex/code-runtime-worker-thread
 */
...
import { CodeRuntime, DUNDER_MEMBER, PORTABLE_RESERVED_WORDS, RESERVED_BINDING_GLOBALS, RESERVED_ERROR_MEMBERS } from '@cortex/code-runtime'
import type { CodeBindingNamespace, CodeJsonValue, CodeRunFailure, CodeRunRequest, CodeRunResult } from '@cortex/code-runtime'
import { snapshotJsonValue } from '@cortex/session'
 ...
 */
const STRIP_WRAP = { prefix: 'async function __cortex_program__() {\n', suffix: '\n}' } as const
...
export class WorkerThreadCodeRuntime extends CodeRuntime {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    for (const [key, value] of Object.entries(this.config)) {
      if (!(Number.isFinite(value) && value > 0)) throw new Error(`cortex-code-runtime-worker-thread: config.${key} must be a positive number, got ${String(value)}`)
    }
    if (!Number.isSafeInteger(this.config.maxOutputBytes) || this.config.maxOutputBytes < MIN_OUTPUT_BYTES) {
      throw new Error(`cortex-code-runtime-worker-thread: config.maxOutputBytes must be a safe integer of at least ${MIN_OUTPUT_BYTES}, got ${String(this.config.maxOutputBytes)}`)
    }
    ...
    if (this.config.maxWallMs > MAX_TIMER_DELAY_MS) {
      throw new Error(`cortex-code-runtime-worker-thread: config.maxWallMs must be at most ${MAX_TIMER_DELAY_MS} (Node clamps a longer setTimeout delay to 1ms), got ${String(this.config.maxWallMs)}`)
    }
  ...
  async run(request: CodeRunRequest): Promise<CodeRunResult> {
    if (this.disposed) throw new Error('cortex-code-runtime-worker-thread: run() after disposal')
  ...
  private validateBindings(request: CodeRunRequest): Map<string, CodeBindingNamespace> {
    ...
    for (const namespace of request.bindings) {
      if (!IDENTIFIER.test(namespace.global) || PORTABLE_RESERVED_WORDS.has(namespace.global)) {
        throw new Error(`cortex-code-runtime-worker-thread: binding global ${JSON.stringify(namespace.global)} is not a usable identifier`)
      }
      ...
      if (RESERVED_BINDING_GLOBALS.has(namespace.global)) {
        throw new Error(`cortex-code-runtime-worker-thread: reserved binding global ${JSON.stringify(namespace.global)}`)
      }
      if (bindings.has(namespace.global)) {
        throw new Error(`cortex-code-runtime-worker-thread: duplicate binding global ${JSON.stringify(namespace.global)}`)
      }
    ...
    for (const namespace of request.bindings) {
      ...
      if (!IDENTIFIER.test(descriptor.name) || PORTABLE_RESERVED_WORDS.has(descriptor.name)) {
        throw new Error(`cortex-code-runtime-worker-thread: binding error class ${JSON.stringify(descriptor.name)} is not a usable identifier`)
      }
      if (RESERVED_BINDING_GLOBALS.has(descriptor.name)) {
        throw new Error(`cortex-code-runtime-worker-thread: reserved binding global ${JSON.stringify(descriptor.name)}`)
      }
      if (bindings.has(descriptor.name) || errorClassNames.has(descriptor.name)) {
        throw new Error(`cortex-code-runtime-worker-thread: duplicate injected global ${JSON.stringify(descriptor.name)}`)
      }
      const member = descriptor.memberNameProperty
      if (member.length === 0 || RESERVED_ERROR_MEMBERS.has(member) || DUNDER_MEMBER.test(member)) {
        throw new Error(`cortex-code-runtime-worker-thread: binding error member property ${JSON.stringify(descriptor.memberNameProperty)} is not usable`)
      }
