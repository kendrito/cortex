 ...
 * @module @cortex/subagent-acp/run
 */
...
import { AssistantOutputFold } from '@cortex/subagent'
...
export interface AcpRunSpec {
   ...
   * `CORTEX_API_KEY`). Merged on top of the subprocess seam's scrubbed
   ...
   * Explicit `CORTEX_*` entries are deployment-owned facts for the child harness
   * (e.g. `CORTEX_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and CORTEX_* names
