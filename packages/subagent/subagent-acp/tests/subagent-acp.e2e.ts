 ...
 * spawn coverage and self-skips without `CORTEX_API_KEY`.
 */

// The real acp-agent example: its bin + cordis.yml (the live Cortex config).
...
// How to launch the child acp-agent (src via tsx / lib via plain node, per CORTEX_EXAMPLE_MODE).
...
const childLaunch = resolveExampleLaunch({
  ...
  env: {
    ...process.env.CORTEX_API_KEY !== undefined ? { CORTEX_API_KEY: process.env.CORTEX_API_KEY } : {},
    ...process.env.CORTEX_BASE_URL !== undefined ? { CORTEX_BASE_URL: process.env.CORTEX_BASE_URL } : {},
...
describe.skipIf(!process.env.CORTEX_API_KEY)('ACP backend with-key e2e (drive our own acp-agent)', () => {
  it('drives the real acp-agent example process to answer a prompt', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'cortex-subagent-acp-e2e-'))
    ctx = new Context()
