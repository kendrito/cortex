...
const settingsModel = 'cortex-settings-inheritance-marker'
const fakeKey = 'cortex-fake-anthropic-key'
...
async function realHarness(behavior: MessagesBehavior): Promise<{
  ...
  const root = mkdtempSync(join(tmpdir(), 'cortex-claude-code-real-'))
  roots.push(root)
  ...
  it('inherits host settings and sends the exact task and fake key to local Messages', async () => {
    ...
    if (process.platform === 'win32') {
      expect(harness.spawnSpecs[0]?.argv.slice(0, 6)).toEqual([
        'cmd.exe', '/d', '/v:off', '/s', '/c', '%CORTEX_CLAUDE_CODE_EXECUTABLE%',
      ])
      const batchExecutable = harness.spawnSpecs[0]?.env?.CORTEX_CLAUDE_CODE_EXECUTABLE
