...
import type { SubprocessHandle } from '@cortex/subprocess'
...
function cortexBaseUrl(): string {
  ...
  if (configured !== OFFICIAL_CORTEX_BASE_URL) {
    throw new Error('Claude Code Cortex e2e requires the official Cortex base URL')
  }
  return configured
}
...
describe.skipIf(!process.env.CORTEX_API_KEY)(
  'Claude Code provider with real Cortex API',
  () => {
    it('returns one unique nonce through the production provider and real SDK/CLI', async () => {
      ...
      if (apiKey === undefined) throw new Error('e2e ran without CORTEX_API_KEY')
      const root = mkdtempSync(join(tmpdir(), 'cortex-claude-cortex-e2e-'))
      roots.push(root)
      ...
      const env = {
        ...
        ANTHROPIC_BASE_URL: `${cortexBaseUrl()}/anthropic`,
        ANTHROPIC_MODEL: 'cortex-v4-pro[1m]',
        ANTHROPIC_DEFAULT_OPUS_MODEL: 'cortex-v4-pro[1m]',
        ANTHROPIC_DEFAULT_SONNET_MODEL: 'cortex-v4-pro[1m]',
        ANTHROPIC_DEFAULT_HAIKU_MODEL: 'cortex-v4-flash',
        CLAUDE_CODE_SUBAGENT_MODEL: 'cortex-v4-flash',
      ...
      const nonce = `CORTEX_CLAUDE_CORTEX_${randomUUID()}`
      const parent = {
        id: 'cortex-e2e-parent',
