...
import * as control from '@cortex/tool-subagent-control'
...
async function setup(options: { load?: boolean; config?: tool.Config } = {}) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'cortex-tool-subagent-report-'))
...
describe('cortex-tool-subagent-report', () => {
...
describe('cortex-tool-subagent-report result independence', () => {
