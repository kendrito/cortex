/**
 * Out-of-process SDK subagent backend. Each child is a complete Cortex
 ...
 * @module @cortex/subagent-cortex-sdk
 */

import type { Context } from '@cortex/cordis'
import z from '@cortex/schemastery'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@cortex/subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@cortex/subagent'
import {
...
export const name = 'subagent-cortex-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('cortex-sdk'),
  command: z.string().required(),
  args: z.array(z.string()).default([]),
  cwd: z.string(),
  provider: z.string().default('cortex-official'),
  model: z.string().default('cortex-v4-flash'),
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    const spec: SdkRunSpec = {
      ...
      cwd: resolveChildCwd('subagent-cortex-sdk', this.config.cwd, request.parent.session.header.cwd),
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-cortex-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-cortex-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-cortex-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-cortex-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-cortex-sdk maxTokens must be a positive safe integer')
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-cortex-sdk', resolved.cwd)
