/**
 * Fresh-process SDK subagent client. Drives one child Cortex
 * runtime over stdio JSON-RPC through `@cortex/sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@cortex/subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @cortex/subagent-cortex-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { CortexHarness, type HarnessNotification } from '@cortex/sdk-client'
import type { ContentBlock } from '@cortex/llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@cortex/session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@cortex/subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@cortex/subagent'
...
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `cortex-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `CORTEX_API_KEY`, or `CORTEX_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `CORTEX_*` fact survives while ambient namesakes never leak.
   */
