...
import { resolveExampleLaunch, runLoaderSmoke } from '@cortex/loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-cortex-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'cortex-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'cortex-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        CORTEX_TEST_CHILD_COMMAND: childLaunch.command,
        CORTEX_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        CORTEX_TEST_CHILD_ENV: JSON.stringify({
