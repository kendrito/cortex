...
import type { SubagentListEntry } from '@cortex/subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'cortex-tool-list-agents-'))
  roots.push(root)
...
describe('cortex-tool-subagent-control/list-agents', () => {
