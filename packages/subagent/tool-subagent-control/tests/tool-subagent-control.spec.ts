...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'cortex-tool-subagent-control-'))
  roots.push(root)
...
describe('cortex-tool-subagent-control', () => {
...
describe('cortex-tool-subagent-control interrupt_agent', () => {
