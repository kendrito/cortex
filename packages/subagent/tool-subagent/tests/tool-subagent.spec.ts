...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@cortex/tools'
import { assembleContextFor, type Agent } from '@cortex/agent'
...
import type { SubagentStartRequest } from '@cortex/subagent'
...
/**
 * Drives the REAL plugin body: mounts `cortex-tool-subagent` on a real
...
describe('cortex-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @cortex/jobs')
  })
...
describe('cortex-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'cortex-tool-subagent-continuable-'))
    roots.push(root)
