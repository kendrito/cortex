...
import {
  startCortexResponsesBridge,
  type CortexResponsesBridge,
} from './cortex-responses-bridge.ts'
...
const bridges: CortexResponsesBridge[] = []
...
describe.skipIf(!process.env.CORTEX_API_KEY)(
  'Codex provider with real Cortex API',
  () => {
    it('returns one unique nonce through the production provider and real Codex', async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), 'cortex-codex-cortex-e2e-'))
      roots.push(root)
      const workspace = join(root, 'workspace')
      const codexHome = join(root, 'codex-home')
      mkdirSync(workspace)
      mkdirSync(codexHome)
      const nonce = `CORTEX_CODEX_CORTEX_${randomUUID()}`
      const bridge = await startCortexResponsesBridge(nonce)
      bridges.push(bridge)
      writeFileSync(join(codexHome, 'config.toml'), [
        'model = "cortex-v4-flash"',
        'model_provider = "cortex-e2e"',
        'approval_policy = "never"',
        'sandbox_mode = "read-only"',
        'disable_response_storage = true',
        'check_for_update_on_startup = false',
        '',
        '[model_providers.cortex-e2e]',
        'name = "Cortex E2E bridge"',
