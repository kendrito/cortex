...
import SandboxedFileSystem from '@cortex/fs-sandbox'
...
import { snapshotSubagentDescriptor } from '@cortex/subagent'
...
beforeEach(async () => {
  workspace = await realpath(await mkdtemp(join(tmpdir(), 'cortex-inherit-')))
})
  ...
  it('records the parent sandbox override and the approval pin before publishing a spawn child', async () => {
    ...
    try {
      ...
      const contextText = runtimeContext.data.content
        ...
        .map(block => block.text)
        .join('\n')
      expect(contextText).toContain('Current Cortex file policy: read-only')
