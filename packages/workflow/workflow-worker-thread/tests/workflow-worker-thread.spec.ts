...
import type { SubagentCapabilities, SubagentProvider, SubagentResult, SubagentRun, SubagentStartRequest } from '@cortex/subagent'
import type { WorkflowMeta, WorkflowResult, WorkflowResultInfo, WorkflowRun, WorkflowRunInfo } from '@cortex/workflow'
...
describe('cortex-workflow-worker-thread', () => {
  describe('script execution over a real worker thread', () => {
    ...
    it('agent({schema, model}) forwards outputSchema and agentOptions to the provider across the thread', async () => {
      ...
      const result = await run(ctx, parent, scripted(`
        const found = await agent('list files', { model: 'cortex-v4-pro', schema: { type: 'object', properties: { files: { type: 'array', items: { type: 'string' } } }, required: ['files'] } })
      ...
      expect(provider.runs[0]!.request.agentOptions).toEqual({ model: 'cortex-v4-pro' })
    ...
    it('the worker spawns with a scrubbed environment: an escaped script finds no ambient credentials', async () => {
      ...
      // the escape below would read it back (exactly how CORTEX_API_KEY
    ...
    it('the unbuilt worker forwards exactly TSX_TSCONFIG_PATH through the scrub: the paths-map pin survives, secrets do not', async () => {
      ...
      // must inherit the pin (or its cortex-* imports silently resolve to
