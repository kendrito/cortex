...
import * as StorageDomain from '@cortex/storage-domain'
import * as StorageJson from '@cortex/storage-json'
import { remoteMethods } from '@cortex/typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@cortex/session-persistence-jsonl', JsonlSessionPersistence],
    ['@cortex/storage', Storage],
    ['@cortex/storage-json', StorageJson],
    ['@cortex/storage-domain', StorageDomain],
    ['@cortex/message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'cortex-message-feedback-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "- name: '@cortex/storage'",
      "- name: '@cortex/storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@cortex/storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@cortex/message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
