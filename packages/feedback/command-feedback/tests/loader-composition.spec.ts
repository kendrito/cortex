...
import type { Agent, AgentStatus } from '@cortex/agent'
import CommandRuntime from '@cortex/commands'
import SessionStore, { SessionId } from '@cortex/session'
import * as CommandFeedback from '@cortex/command-feedback'
import { getOrCreateAnonymousUserId } from '@cortex/anonymous-user-id'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'cortex-command-feedback-loader-'))
    vi.stubEnv('CORTEX_HOME', root)
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@cortex/commands', CommandRuntime],
      ['@cortex/command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { CORTEX_HOME: root } })
