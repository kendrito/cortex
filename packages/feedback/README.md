...
A command feedback remark is log-only: it never enters model context or derived history. A telemetry backend, when a deployment mounts one, may observe `feedback/record`; capture itself remains independent of that policy.
