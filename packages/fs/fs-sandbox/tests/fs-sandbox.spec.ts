...
import { FsError, FsTargetKey } from '@cortex/fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.cortex-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'cortex-fssbx-tmp-')), 'temp.txt')
