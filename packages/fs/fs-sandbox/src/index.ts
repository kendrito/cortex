 ...
 * `@cortex/fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@cortex/bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@cortex/tool-fs`),
 * exactly as bash's does.
 *
 * @module @cortex/fs-sandbox
 */

import { Context } from '@cortex/cordis'
import { LocalFileSystem } from '@cortex/fs-local'
import type { Config as LocalConfig } from '@cortex/fs-local'
import { FsError } from '@cortex/fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@cortex/fs'
import { writableRoots } from '@cortex/sandbox'
 ...
 * INSTEAD OF `cortex-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `cortex-tool-fs` resolves
