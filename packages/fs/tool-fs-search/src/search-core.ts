 ...
 *
 * @module @cortex/tool-fs-search/search-core
 */

import { isAbsolute, relative, sep } from 'node:path'
import type { Context } from '@cortex/cordis'
import { HarnessError } from '@cortex/llm'
import { ItemRetainer, TextRetainer } from '@cortex/output-retention'
import type { RetainedItems } from '@cortex/output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@cortex/subprocess'
import type { SaveTextSpill, SpillRef } from '@cortex/spill'
 ...
 * `@cortex/tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`cortex-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@cortex/tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@cortex/tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@cortex/tool-fs-search/presentation} `grepSearchMeta`)
