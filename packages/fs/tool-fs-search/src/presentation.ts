 ...
 * model-facing render consumes ({@link module:@cortex/tool-fs-search/search-core}
 ...
 * final output budget (`cortex-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @cortex/tool-fs-search/presentation
 */
 ...
 * ({@link module:@cortex/tool-fs-search/grep} `formatGrepMatches`), so
