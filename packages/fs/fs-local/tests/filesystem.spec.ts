 ...
 * `cortex-fs-observation-policy`, so it is not exercised here.
 */
...
import { FsVersion } from '@cortex/fs'
import type { FsTarget } from '@cortex/fs'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'cortex-fs-'))
  ctx = new Context()
...
describe('resolve', () => {
  it('resolves a relative path against opts.cwd, not config.cwd', async () => {
    ...
    const other = await mkdtemp(join(tmpdir(), 'cortex-fs-other-'))
    try {
...
describe('writeText', () => {
  ...
  it('gates the NEW content by UTF-8 byte length, not character count', async () => {
    // Seven accented-Latin characters are 9 UTF-8 bytes: below an 8-byte bound
    // by characters but at/above it by bytes, so the basis must be declined.
    await remountWithDiffLimit(8)
    await writeFile(join(dir, 'multibyte.txt'), 'tiny')
    const target = await fs.resolve('multibyte.txt')
    const outcome = await fs.writeText(target, 'naïveté')
    expect(outcome.operation).toBe('update')
    expect(outcome.before).toBeNull()
    expect(outcome.after).toBe('naïveté')
  })
