...
import { CodeRuntime } from '@cortex/code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@cortex/code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@cortex/llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from '@cortex/llm'
import SystemPrompt from '@cortex/system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@cortex/tools'
import type { Config as ToolConfig } from '@cortex/tools'
import LocalFileSystem from '@cortex/fs-local'
import * as FsPolicy from '@cortex/fs-observation-policy'
import LocalAttachmentStore from '@cortex/attachment-local'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'cortex-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'cortex-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { cortexHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { cortexHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { cortexHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
