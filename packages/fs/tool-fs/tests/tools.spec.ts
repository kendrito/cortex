...
import ToolRuntime, { type ToolResult } from '@cortex/tools'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@cortex/fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'cortex-tool-fs-session-cwd-'))
