...
// --------------------------------------------------------------------------
describe('default deployment (with cortex-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'cortex-tool-fs-'))
    ctx = new Context()
...
// --------------------------------------------------------------------------
describe('bare provider (no cortex-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'cortex-tool-fs-bare-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(LocalFileSystem, { cwd: dir })
...
// caller-selected session workspace wins, matching cortex-tool-bash.
describe('per-session cwd', () => {
  let sessionDir: string
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'cortex-tool-fs-cfg-'))
    sessionDir = await mkdtemp(join(tmpdir(), 'cortex-tool-fs-session-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
