 ...
 *
 * `@cortex/tools`' `ToolExecution` satisfies this shape, so the policy
 * reads `exec` straight through without importing `cortex-tools` or `cortex-agent`.
 * Only the session HEADER id is read — the same identity every other subsystem
 * keys off (see `cortex-tool-bash`'s owner derivation).
 *
 * @module @cortex/spill-policy/types
 */
