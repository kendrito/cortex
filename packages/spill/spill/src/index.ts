 ...
 * `@cortex/spill-local` (host filesystem) is the first.
 *
 * The Service Definition is deliberately minimal: `saveText` and nothing else. It owns NO
 * retention policy (that is `@cortex/output-retention`), NO tool-result
 * replacement (that is `@cortex/spill-policy`), and NO retrieval or
