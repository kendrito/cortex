 ...
 * (like `cortex-bash-local`'s `run.ts`) so the filesystem behavior is unit-testable
 * without a `ctx` and without the OS temp dir.
 *
 * @module @cortex/spill-local/store
 */
...
export function privateRoot(): string {
  defaultRoot ??= mkdtempSync(join(tmpdir(), 'cortex-spill-'))
  return defaultRoot
}
