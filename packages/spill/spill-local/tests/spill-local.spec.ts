...
import type { SaveTextSpill } from '@cortex/spill'
import LocalSpillStore, { encodeSegment, privateRoot, saveTextFile, sessionDir } from '@cortex/spill-local'

let root: string

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'cortex-spill-test-'))
})
afterEach(() => {
