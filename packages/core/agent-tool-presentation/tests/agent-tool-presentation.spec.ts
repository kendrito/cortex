...
import ToolRuntime, { RUN_CODE_NAME, defineTool } from '@cortex/tools'
import type { Agent } from '@cortex/agent'
import { SessionId } from '@cortex/session'
import { apply, Config, inject, name } from '@cortex/agent-tool-presentation'
...
describe('the tool-presentation row', () => {
  ...
  it('waits for a code runtime the deployment does not compose', async () => {
    ...
    // Pending, not applied: `cortex-agent-presets` rejects a mount holding a row
