...
import { Context, FiberState, getTraceable, Service, symbols } from '@cortex/cordis'
...
import type { TypertContext, TypertLookup } from '@cortex/typert-protocol'
...
declare module '@cortex/cordis' {
  interface Context {
     ...
     * `cortex-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in cortex-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `cortex-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `cortex-agent-loop` package.
 */
 ...
 * (`@cortex/agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@cortex/agent#Agent',
        wireTypeSymbol: '@cortex/session/types#SessionId',
