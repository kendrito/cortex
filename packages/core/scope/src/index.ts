...
import { Context as CordisContext } from '@cortex/cordis'
...
const kScope = Symbol('cortex.scope')
...
function linkScopeParent(key: ScopeKey, parent: ScopeKey): void {
  for (let cursor: ScopeKey | undefined = parent; cursor !== undefined; cursor = scopeParents.get(cursor)) {
    if (cursor === key) throw new Error('cortex-scope: scope parent link would form a cycle')
  }
...
export function bindScopeParent(key: ScopeKey, parent: ScopeKey): ScopeParentBinding {
  if (scopeParents.has(key)) {
    throw new Error('cortex-scope: scope key is already bound to a parent; re-linking requires the binding returned by the original bind')
  }
