...
import LlmRuntime, { createUserMessage, LlmError, ReasoningEffortId  } from '@cortex/llm'
import type { GenerateOptions, LlmModelReasoningInfo, LlmResolvedModelInfo, StreamChunk } from '@cortex/llm'
import SessionStore, { Session, SessionId, foldRequestHeader } from '@cortex/session'
...
describe('request stability across the loop', () => {
  ...
  it('rematerializes the selected adapter maxTokens default after a provider switch', async () => {
    const cortex = new MockAdapter([textResponse('cortex')], undefined, 256_000)
    ...
    expect(cortex.requests[0]?.maxTokens).toBe(256_000)
  ...
  it('preserves an explicit agent maxTokens cap across a provider switch', async () => {
    ...
    expect(cortex.requests[0]?.maxTokens).toBe(4_096)
