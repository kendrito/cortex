 ...
 *
 * @module @cortex/agent-loop
 */

import { Context, FiberState, Service } from '@cortex/cordis'
import { randomUUID } from 'node:crypto'
import z from '@cortex/schemastery'
import { emitAgentEvent } from '@cortex/agent'
import type {
  Agent,
  AgentFactory,
  AgentHandle,
...
import { SessionId, SessionPreparation } from '@cortex/session'
import type { Session, SessionHeader } from '@cortex/session'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    ...
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a cortex-session-persistence backend)')
    }
