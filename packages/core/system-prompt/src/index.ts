 ...
 *
 * @module @cortex/system-prompt
 */

import { Context, Service } from '@cortex/cordis'
import z from '@cortex/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@cortex/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@cortex/scope'
import type { ContextSnapshotSection, ToolSchema } from '@cortex/llm'
...
declare module '@cortex/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@cortex/scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by Cortex.',
      })
    }
    this.section({
