...
import { createUserMessage, CallId  } from '@cortex/llm'
import { createScope } from '@cortex/scope'
import type { Scope } from '@cortex/scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@cortex/tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@cortex/tools'
import type { Agent } from '@cortex/agent'
import { Session, SessionId } from '@cortex/session'
import type { JsonValue, SessionEventMap } from '@cortex/session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@cortex/scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `cortex-agent-tool-presentation` produces.
