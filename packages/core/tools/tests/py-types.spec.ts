...
import { jsonSchemaToPy, renderToolsSdkPy } from '@cortex/tools/src/py-types.ts'
import { parameterSchemaSpecToJsonSchema } from '@cortex/tools'
import type { ToolSdkSchema } from '@cortex/tools/src/ts-types.ts'
...
describe('renderToolsSdkPy', () => {
  ...
  it('keeps a non-ASCII field name as a TypedDict field and derives its class name from it', () => {
    // `größe` satisfies `xid_start xid_continue*`, so CPython accepts it as an
    ...
    const tool: ToolSdkSchema = {
      ...
      parameters: {
        ...
        properties: {
          größe: { type: 'string' },
          opts: { type: 'object', additionalProperties: false, properties: { höhe: { type: 'number' } } },
        },
        required: ['größe'],
      },
      output: { type: 'string' },
    ...
    const text = renderToolsSdkPy([tool])
    expect(text).toContain('async def übersetze(self, args: ÜbersetzeArgs) -> str:')
    expect(text).toContain('class ÜbersetzeArgs(TypedDict):')
    expect(text).toContain('    größe: str')
    expect(text).toContain('class ÜbersetzeArgsOpts(TypedDict):')
    expect(text).toContain('    höhe: NotRequired[float]')
    expect(text).not.toContain('dict[str, Any]')
  })
  ...
  it('subscripts a tool name that NFKC-normalizes to something else, while declaring a plain Unicode one', () => {
    // Same split at the tool-name site: `größe` becomes an `async def`, the
    ...
    const text = renderToolsSdkPy([of('größe'), of('ﬁnd')])
    expect(text).toContain('async def größe(self, args: GrößeArgs) -> str:')
