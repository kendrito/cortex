...
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@cortex/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@cortex/scope'
import type { CallId, ContentBlock, ToolSchema } from '@cortex/llm'
import { assertNever, deepFreeze, HarnessError } from '@cortex/llm'
import type { Agent } from '@cortex/agent'
import { snapshotJsonValue } from '@cortex/session'
import type { JsonValue, UserMessage } from '@cortex/session'
import type { ToolProviderResult } from '@cortex/system-prompt'
import type { CodeRuntime } from '@cortex/code-runtime'
 ...
 * instead of deriving them: the seam's `cortex-code-runtime` README, its
 * `CodeRuntime.language` JSDoc, `docs/subsystems/code-runtime.md`, this
 * package's own README, and the {@link Config.mode} JSDoc.
 */
/**
...
export type { JsonValue } from '@cortex/session'
...
// lives in its own UI-facing module; re-export it so `@cortex/tools`
...
declare module '@cortex/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@cortex/scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@cortex/scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@cortex/scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@cortex/tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@cortex/tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `cortex-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`cortex-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`cortex-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @cortex/code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`cortex-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `cortex-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('cortex.tool.execution') as ToolExecutionToken
}
