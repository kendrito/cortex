 ...
 * @module @cortex/tools/src/code-mode
 */

import { CallId, HarnessError } from '@cortex/llm'
import type { ContentBlock } from '@cortex/llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@cortex/code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`cortex-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
