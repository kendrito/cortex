...
Bin-only app that boots an external `cordis.yml`; its [`jsonrpc`](../../sdk/server/README.md) entry serves SDK clients over newline-delimited stdio. The config composes the spine, backends, and serving plugin. The published `cortex-jsonrpc-agent` bin resolves bare plugins from the configuration project. The Python SDK's `cortex-jsonrpc-agent-pkg` [single-executable runtime](../../../.agents/notes/implemented/architecture/2026-07-10-single-file-executable-sdk-runtime-distribution.md) uses `lib/packaged-bin.js` instead: packaged bare plugins resolve from its closed runtime tree, while relative plugins remain configuration-relative.

## Config discovery

The first non-empty channel wins: `$CORTEX_CORDIS_CONFIG`, then positional `argv[2]`. If neither names an existing file, the bin prints one-line usage to stderr and exits 1; there is no working-directory or built-in fallback. [`cortex-app-boot`](../../boot/app-boot/README.md) makes plugin load failures fatal. This protocol does not use `CORTEX_SNAPSHOT`.

A config without `cortex-sdk-jsonrpc-server` is valid and serves nothing; the bin does not designate a server plugin.
...
- **The bin cannot prove that the config serves JSON-RPC** — a valid config with no `cortex-sdk-jsonrpc-server` entry boots successfully and serves nothing.
- **No built-in or default config exists** — every launch must provide `CORTEX_CORDIS_CONFIG` or a positional path, and deployment owns the complete plugin tree and stdout discipline.
