...
import { renderPrompt, TOOL_ORDER_REST } from '@cortex/system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@cortex/agent'
...
import * as sessionInvariant from '@cortex/session/invariant'
import * as agentInvariant from '@cortex/agent/invariant'
import * as scopeInvariant from '@cortex/scope/invariant'
import * as agentLoopInvariant from '@cortex/agent-loop/invariant'

const testToolSignal = new AbortController().signal

declare module '@cortex/jobs' {
...
/**
 * Unit coverage for the @cortex/agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldCortexHome = process.env.CORTEX_HOME
  const oldAgentsHome = process.env.CORTEX_AGENTS_HOME
  process.env.CORTEX_HOME = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-home-'))
  process.env.CORTEX_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldCortexHome === undefined) {
      delete process.env.CORTEX_HOME
    } else {
      process.env.CORTEX_HOME = oldCortexHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.CORTEX_AGENTS_HOME
    } else {
      process.env.CORTEX_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('cortex-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@cortex/agent$'] },
      { package_blocklist: ['^@cortex/session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by Cortex.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-skill-custom-'))
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-skill-refresh-'))
    const home = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-skill-refresh-home-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('shares top-level cortexHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'cortex-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'cortex-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-cortex-home-test') as ToolExecution['token'],
      callId: CallId('agent-core-cortex-home'),
      rootCallId: CallId('agent-core-cortex-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ CORTEX_HOME: home, CORTEX_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested Cortex home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        cortexHome: '/global-cortex-home',
        workspaceContext: false,
        skills: { filesystem: { cortexHome: '/nested-cortex-home' } },
      })
    }).toThrow('agent-spine-demo: cortexHome and skills.filesystem.cortexHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('picks shared spine config without leaking entry-point fields', () => {
    ...
    expect(agentCore.pickSpineConfig(appConfig)).toEqual({
      ...
      cortexHome: appConfig.cortexHome,
