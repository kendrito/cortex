...
```
@cortex/cordis-plugin-timer  timer service (writes nothing to stdout)
@cortex/llm              abstract LLM service + content-block vocabulary
@cortex/session          event-sourced session log + store
@cortex/session-title    log-backed title service + deterministic fallback
@cortex/system-prompt    prompt-section + tool-schema assembly
@cortex/tools            registry + guarded pre/around/post/final-result pipeline
@cortex/skill            skill provider registry
@cortex/skill-filesystem      local filesystem skill provider
@cortex/agent            agent registry + initiator scope + agent/* events
@cortex/goal             optional persisted same-session goal domain
@cortex/tool-goal        optional model-facing goal controls
@cortex/goal-round-driver     optional same-session goal-round driver
@cortex/llm-retry        provider-routed request retry policy
@cortex/jobs-local      generic background-job registry
@cortex/invariants       configurable invariant registry service
@cortex/session/invariant
@cortex/agent/invariant
@cortex/scope/invariant
@cortex/agent-loop/invariant
                                  package-owned relational checks
@cortex/tool-bash        the model-facing bash schema (unless toolBash=false)
@cortex/agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@cortex/tool-skill       session-prefix skill catalog + model-facing loader schema
@cortex/tool-jobs       job_output/job_list/job_kill schemas + completion notices
@cortex/agent-loop       THE concrete loop (gets the forwarded `agents`)
                                  (cortex-system-prompt gets the forwarded `persona`)
```
...
- **the LLM adapter** — the bundle ships the abstract `llm` service; the leaf registers a concrete adapter on `ctx.llm` (`llm-pi-ai`, or `llm-replay` for keyless replay).
...
Indirectly, through `cortex-system-prompt`, `cortex-tool-skill`, `cortex-tool-bash`, `cortex-tools`, and `cortex-llm-retry`, plus `cortex-tool-goal` and goal-round prompts when `goals` is enabled. The bundle adds no model-bound wrapper content of its own.
