 ...
 * @module @cortex/agent-spine-demo
 */

import type { Context } from '@cortex/cordis'
import Timer from '@cortex/cordis-plugin-timer'
import z from '@cortex/schemastery'
import LlmRuntime from '@cortex/llm'
import SessionStore from '@cortex/session'
import SessionTitleService, { type Config as SessionTitleConfig } from '@cortex/session-title'
import SystemPrompt, { type Config as SystemPromptConfig } from '@cortex/system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@cortex/tools'
import SkillRegistry, { type Config as SkillRegistryConfig } from '@cortex/skill'
import * as SkillFileSystem from '@cortex/skill-filesystem'
import AgentRegistry from '@cortex/agent'
import GoalService, { type Config as GoalDomainConfig } from '@cortex/goal'
import * as goalSession from '@cortex/goal-round-driver'
import * as toolGoal from '@cortex/tool-goal'
import LocalJobRegistry, { type Config as JobsConfig } from '@cortex/jobs-local'
import InvariantRegistry, { type Config as InvariantConfig } from '@cortex/invariants'
...
import * as bashEnv from '@cortex/shell-env'
import * as workspaceContext from '@cortex/agent-instructions'
import * as toolSkill from '@cortex/tool-skill'
import * as toolJobs from '@cortex/tool-jobs'
import AgentLoop, { type Config as AgentLoopConfig } from '@cortex/agent-loop'
import * as llmRetry from '@cortex/llm-retry'
...
export const Config = z.intersect([
  ...
  }) as unknown as z<Pick<Config, 'tools' | 'cortexHome' | 'sessionTitle' | 'skills' | 'workspaceContext' | 'toolBash' | 'jobs' | 'toolJobs' | 'invariants' | 'goals'>>,
...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.cortexHome !== undefined ? { cortexHome: config.cortexHome } : {},
...
export function apply(ctx: Context, config: Config): void {
  const nestedCortexHome = config.skills?.filesystem?.cortexHome
  if (config.cortexHome !== undefined && nestedCortexHome !== undefined
    && resolveCortexHome(config.cortexHome) !== resolveCortexHome(nestedCortexHome)) {
    throw new Error('agent-spine-demo: cortexHome and skills.filesystem.cortexHome must resolve to the same directory')
  }
  const cortexHome = resolveCortexHome(config.cortexHome ?? nestedCortexHome)
  ...
  if (skillsEnabled) {
    ...
    ctx.plugin(SkillFileSystem, Object.assign({}, config.skills?.filesystem, { cortexHome }))
  }
