 ...
 * `cortex-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in cortex-app-boot. Replay skips `.env` and selects sibling
 ...
 * @module @cortex/acp-demo/bin
 */

import { parseArgs } from 'node:util'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@cortex/app-boot'

const NAME = 'cortex-acp-demo'

/* v8 ignore start -- thin self-executing composition over the unit-tested
   cortex-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
installFailLoud(NAME)
const snapshotMode = process.env['CORTEX_SNAPSHOT']
