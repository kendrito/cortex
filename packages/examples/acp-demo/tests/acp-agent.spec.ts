...
import { agentEvents } from '@cortex/agent'
import { TOOL_ORDER_REST } from '@cortex/system-prompt'
import type { Message } from '@cortex/llm'
import { SessionId } from '@cortex/session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @cortex/acp-demo composition:
...
async function mount(config: acpAgent.Config, withBash = false): Promise<Context> {
  ...
  config.persistenceRoot ??= await mkdtemp(join(tmpdir(), 'cortex-acp-demo-persistence-'))
...
async function isolatedSkillsConfig(catalogDescriptionMaxLength?: number): Promise<NonNullable<acpAgent.Config['skills']>> {
  const home = await mkdtemp(join(tmpdir(), 'cortex-acp-demo-skills-'))
  return {
    filesystem: { cortexHome: join(home, '.cortex'), agentsHome: join(home, '.agents') },
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  const home = await mkdtemp(join(tmpdir(), 'cortex-acp-demo-default-skills-'))
  process.env.CORTEX_HOME = join(home, '.cortex')
  process.env.CORTEX_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
...
describe('cortex-acp-demo composition', () => {
  it('brings up the spine + persistence + the ACP bridge', async () => {
    const ctx = await mount({
      provider: 'mock',
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/cortex-acp-demo-test',
  ...
  it('forwards explicit project-instruction controls to the bundled spine', async () => {
    const ctx = await mount({
      ...
      model: 'mock',
      persona: 'hi',
      persistenceRoot: '/tmp/cortex-acp-demo-workspace-context',
      workspaceContext: false,
    })
    expect(ctx.get('agents')).toBeDefined()
    expect(ctx.get('agentLoop')).toBeDefined()
  ...
  it('forwards skill config and cortexHome into agent-spine-demo', async () => {
    const skills = await isolatedSkillsConfig(6)
    const ctx = await mount({ provider: 'mock', model: 'mock', persona: 'hi', cortexHome: skills.filesystem!.cortexHome!, skills, workspaceContext: false })
  ...
  it('forwards maxParallelToolCalls to the bundled agent loop', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/cortex-acp-demo-test-parallel',
      skills: await isolatedSkillsConfig(),
      workspaceContext: false,
    })
  ...
  it('forwards toolOrder through agent-spine-demo to the system-prompt assembly', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: '/tmp/cortex-acp-demo-test-tool-order',
      workspaceContext: false,
    })
