...
async function makeConsumer(): Promise<string> {
  ...
  for (const rel of cortexPackages) {
...
describe.skipIf(!existsSync(acpBin))('cortex-acp-demo BUILT bin (node lib/bin.js, no tsx)', () => {
  it('boots the published bin, completes a turn, and writes default Zstandard persistence', async () => {
    ...
    child = spawn(process.execPath, [acpBin, '--config', './cordis.yml'], {
      ...
      env: {
        ...process.env,
        CORTEX_HOME: join(consumer, '.cortex'),
        CORTEX_AGENTS_HOME: join(consumer, '.agents'),
      },
