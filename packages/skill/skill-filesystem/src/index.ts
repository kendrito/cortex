 ...
 *
 * @module @cortex/skill-filesystem
 */
...
import type Schema from '@cortex/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@cortex/fs'
...
const PROJECT_CORTEX_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_CORTEX_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.CORTEX_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.CORTEX_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.cortex/skills'), source: 'project-cortex', rank: PROJECT_CORTEX_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.cortexHome, 'skills'), source: 'user-cortex', rank: USER_CORTEX_RANK, skipSystem: true },
