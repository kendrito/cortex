...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@cortex/fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `cortex-${name}-`)))
}
...
describe('cortex-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.cortex/skills'), 'same', 'user cortex skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.cortex/skills'), 'same', 'project cortex skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.cortex/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project cortex skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-cortex')
    ...
    await writeSkill(join(noGit, '.cortex/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.cortex/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.cortex/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.cortex/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.cortex/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.cortex/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.cortex/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.cortex/skills/broken-link'))
    await symlink('/dev/null', join(home, '.cortex/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { cortexHome: join(home, '.cortex'), agentsHome: join(home, '.agents'), watch: false })

    expect((await ctx.skills.list({ cwd: nestedCwd })).map(skill => [skill.name, skill.source])).toEqual([
      ['backend-root', 'project-agents'],
      ['text-skill', 'user-cortex'],
    ])
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.cortex/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.cortex/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    ...
    const previousAgentsHome = process.env.CORTEX_AGENTS_HOME
    const previousBundledSkillDir = process.env.CORTEX_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.CORTEX_HOME = join(envHome, '.cortex')
      process.env.CORTEX_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.CORTEX_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.cortex/skills'), 'env-skill', 'Env skill')
      ...
      process.env.CORTEX_HOME = join(envHome, 'empty-cortex')
      delete process.env.CORTEX_BUNDLED_SKILL_DIR
      process.env.CORTEX_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { cortexHome: join(envHome, 'empty-cortex') }).name).toBe('filesystem')
    } finally {
      ...
      if (previousAgentsHome === undefined) {
        ...
        process.env.CORTEX_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.CORTEX_BUNDLED_SKILL_DIR
      } else {
        process.env.CORTEX_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
