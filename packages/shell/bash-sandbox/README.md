...
Sandbox-consuming Service Provider for the [`@cortex/shell`](../shell/) executor seam. Load it **instead of** `@cortex/bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@cortex/sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `cortex-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
...
- Process mechanics (spawn, process-group kills, output collection/spill, background handles, credential scrub) are inherited from [`cortex-bash-local`](../bash-local/); runner selection lives in [`cortex-sandbox-local`](../../sandbox/sandbox-local/).

Deny-only at the seam: a denial is a reported fact, and this executor never negotiates permissions itself — the approval question lives in the tool layer (`cortex-tool-bash`), which drives the override this package honors.

```yaml
- id: sandbox
  name: '@cortex/sandbox-local'
- id: sandbox-policy
...
The generated [`cortex-tool-bash` schemas](../../../docs/tool-catalog.md#cortextool-bash) are the baseline. By advertising a confining `sandboxMode`, this backend augments `bash` with `sandbox_permissions` using enum `workspace-write` | `danger-full-access` and with `justification`. The policy owner separately contributes the current capability-neutral `sandbox:policy` context.

#### Token effect

Small fixed schema increment on requests where `bash` is visible, plus the current-policy clause owned by `cortex-sandbox-policy`.
