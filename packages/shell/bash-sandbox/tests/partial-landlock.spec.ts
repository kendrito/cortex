...
import { LAUNCHER_FAILURE_EXIT } from '@cortex/node-addon-landlock-run'
import { SANDBOX_UNAVAILABLE, SandboxUnavailableError } from '@cortex/sandbox'
import { LocalSandboxProvider } from '@cortex/sandbox-local'
...
async function fakeLauncher(fatalExit?: number): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-partial-landlock-'))
  tempDirs.push(dir)
...
describe('partial Landlock runner-failure classification', () => {
  it.each(['missing', 'unexecutable', 'missing-interpreter'] as const)('classifies a %s configured runner through the direct spawn error channel', async (kind) => {
    const dir = await mkdtemp(join(tmpdir(), 'cortex-unusable-sandbox-runner-'))
    tempDirs.push(dir)
    const runner = join(dir, `${kind}-runner`)
    if (kind === 'unexecutable') await writeFile(runner, '#!/bin/sh\nexit 0\n', { mode: 0o644 })
    if (kind === 'missing-interpreter') {
      await writeFile(runner, '#!/cortex-definitely-missing-sandbox-interpreter\nexit 0\n', { mode: 0o755 })
    }
  ...
  it.each(['bare-name', 'relative'] as const)(
    ...
    async (form) => {
      const dir = await mkdtemp(join(tmpdir(), 'cortex-argv-form-sandbox-runner-'))
      tempDirs.push(dir)
      const filename = 'missing-interpreter-runner'
      const runner = form === 'bare-name' ? filename : `./${filename}`
      await writeFile(join(dir, filename), '#!/cortex-definitely-missing-sandbox-interpreter\nexit 0\n', { mode: 0o755 })
      const bash = await setupConfiguredRunner(runner)
  ...
  it('keeps a real malformed executable ordinary across no-shebang spawn behavior', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'cortex-malformed-sandbox-runner-'))
    tempDirs.push(dir)
