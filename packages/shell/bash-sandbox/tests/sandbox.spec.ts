...
import type { ShellRunResult, CollectedOutput } from '@cortex/shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@cortex/sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@cortex/sandbox'
import { SandboxPolicyService } from '@cortex/sandbox-policy'
import { SandboxBashExecutor } from '@cortex/bash-sandbox'
...
import type { Config } from '@cortex/bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'cortex-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'CORTEX_WRAP=1', 'bash', '-c', 'printf "%s" "$CORTEX_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$CORTEX_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'cortex-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.CORTEX_ORDER_FILE, "runner\\n");',
    ...
    try {
      const result = await bash.run(bash.resolve({
        command: 'true',
        env: { BASH_ENV: hook },
        cortexEnv: { CORTEX_ORDER_FILE: order },
      }))
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'cortex-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'cortex-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
