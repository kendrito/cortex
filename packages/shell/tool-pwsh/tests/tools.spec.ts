 ...
 * managed `CORTEX_*` collection, abort translation, canonical result projection,
...
import SystemPrompt, { renderPrompt } from '@cortex/system-prompt'
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@cortex/shell'
...
import type { ShellProcessRead } from '@cortex/shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, cortexHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, cortexHome === undefined ? {} : { cortexHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, cortexHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed CORTEX_* environment', async () => {
    const cortexHome = mkdtempSync(join(tmpdir(), 'cortex-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, cortexHome)
    ...
    expect(request?.cortexEnv).toEqual({
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const cortexEnv = bash.requests[0]?.cortexEnv
    expect(cortexEnv).toBeDefined()
    expect(cortexEnv?.['CORTEX_SHELL']).toBe('1')
    expect(cortexEnv?.['CORTEX_HOME']).toEqual(expect.any(String))
    expect(cortexEnv).not.toHaveProperty('CORTEX_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'cortex-tool-pwsh-policy-'))
