/**
 * Integration tests: the REAL `@cortex/pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@cortex/tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@cortex/pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'cortex-tool-pwsh-'))
