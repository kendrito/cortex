/**
 * Real-process tests for `@cortex/pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@cortex/pwsh-local'
import LocalSubprocessRuntime from '@cortex/subprocess-local'
import SubprocessRuntime from '@cortex/subprocess'
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@cortex/subprocess'
import { MAX_TIMER_DELAY_MS } from '@cortex/timeout'
import type { ShellProcess } from '@cortex/shell'

const spillDir = mkdtempSync(join(tmpdir(), 'cortex-pwsh-exec-spec-'))
...
describe('resolvePwshPath and candidatePwshPaths (pure, every platform)', () => {
  ...
  it('returns the first EXISTING win32 candidate, else pwsh', () => {
    const dir = mkdtempSync(join(tmpdir(), 'cortex-pwsh-resolve-'))
  ...
  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'cortex-pwsh-resolve-link-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'cortex-pwsh-resolve-dir-'))
    const store = join(dir, 'store')
...
describe('spawn construction (pure, every platform)', () => {
  ...
  it('runs every command as ONE argv element under the UTF-8 encoding preamble', async () => {
    ...
    await ctx.shell.run(ctx.shell.resolve({ command: 'Write-Output Größe' }))
    ...
    expect(argv[5]).toBe(`${ENCODING_PREAMBLE}Write-Output Größe`)
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'cortex-pwsh-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'cortex-pwsh-cwd-b-'))
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-cortex' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/cortexEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:CORTEX_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      cortexEnv: { CORTEX_SEAM_VAR: 'cortex-ok' },
    })
    ...
    expect(spec.cortexEnv).toEqual({ CORTEX_SEAM_VAR: 'cortex-ok' })
    const result = await bash.run(spec)
    expect(lf(result.stdout.text)).toBe('piped\n[env-ok][cortex-ok]\n')
  })

  it('resolve() omits stdin/env/cortexEnv when the request supplies none', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({ command: 'Write-Output ok' })
    expect('stdin' in spec).toBe(false)
    expect('env' in spec).toBe(false)
    expect('cortexEnv' in spec).toBe(false)
  })
})
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:CORTEX_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      cortexEnv: { CORTEX_BG_VAR: 'bg-cortex-env' },
    }))
    const partialOutput = await readUntil(proc, '[bg-env][bg-cortex-env]')
    await proc.done
    const output = partialOutput + lf(proc.readOutput().delta)
    expect(output).toBe('bg-stdin\n[bg-env][bg-cortex-env]\n')
    expect(proc.exitCode).toBe(0)
  })
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-cortex' }))
