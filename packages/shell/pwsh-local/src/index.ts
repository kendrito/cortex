 ...
 *
 * @module @cortex/pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors cortex-bash-local call-for-call by
...
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@cortex/shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@cortex/shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@cortex/subprocess'
import { installSettingsSection } from '@cortex/settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@cortex/timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of cortex-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `cortex-bash-local`'s `runArgv`/`startArgv` hooks; see
   * `@cortex/pwsh-sandbox`).
   */
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.cortexEnv },
    }
  }
   ...
   * `cortex-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@cortex/pwsh-sandbox`.
