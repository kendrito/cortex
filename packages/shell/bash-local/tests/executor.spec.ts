...
const spillDir = mkdtempSync(join(tmpdir(), 'cortex-bash-exec-spec-'))
...
describe('LocalBashExecutor.run', () => {
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'true', workdir: '/nonexistent-cortex' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/cortexEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: 'cat; echo "[$SEAM_VAR][$CORTEX_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
...
describe('LocalBashExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: 'cat; echo "[$BG_VAR][$CORTEX_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      cortexEnv: { CORTEX_BG_VAR: 'bg-cortex-env' },
    }))
    const output = await readUntil(proc, '[bg-env][bg-cortex-env]')
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'true', workdir: '/nonexistent-cortex' }))
