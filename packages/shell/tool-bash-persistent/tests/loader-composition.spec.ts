...
suite('persistent Bash through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    root = await mkdtemp(join(tmpdir(), 'cortex-persistent-bash-loader-'))
    ...
    await writeFile(configPath, [
      ...
      '    timeoutMs: 2000',
      '    disposeGraceMs: 500',
      "- name: '@cortex/tool-bash-persistent'",
      '  config:',
      '    timeoutMs: 5000',
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@cortex/tool-bash-persistent', ToolBashPersistent],
    ])
    ...
    expect(observed).not.toContain('CORTEX_PERSISTENT_BASH')
