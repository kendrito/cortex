...
class StubPtySession implements TerminalBackendSession {
  readonly motd = '__CORTEX_PERSISTENT_BASH_PROMPT__ '
  readonly pid = 123
  ...
  startSend(request: TerminalSendRequest): TerminalSendOperation {
    ...
    if (this.mode === 'wait-for-abort' || this.mode === 'end-on-abort') {
      const done = new Promise<ReturnType<StubPtySession['result']>>((resolve) => {
        request.signal?.addEventListener('abort', () => {
          const start = /__CORTEX_PERSISTENT_BASH_START_[^_]+(?:-[^_]+)*__/.exec(request.text)?.[0]
          const end = /__CORTEX_PERSISTENT_BASH_END_[^:]+:/.exec(request.text)?.[0]
    ...
    this.pendingText = ''
    const start = /__CORTEX_PERSISTENT_BASH_START_[^_]+(?:-[^_]+)*__/.exec(sent)?.[0]
    const end = /__CORTEX_PERSISTENT_BASH_END_[^:]+:/.exec(sent)?.[0]
...
describe('tool-bash-persistent', () => {
  ...
  it('handles inferred idle, prompt fallback, shell exit, clipping, and cleanup', async () => {
    ...
    expect(promptFallback).not.toContain('CORTEX_PERSISTENT_BASH_PROMPT')
    ...
    expect(crlfPromptFallback).not.toContain('CORTEX_PERSISTENT_BASH_PROMPT')
  ...
  it('sanitizes a prompt fallback reached after multiple polling rounds', async () => {
    ...
    expect(result).not.toContain('CORTEX_PERSISTENT_BASH_PROMPT')
    expect(result).not.toContain('CORTEX_PERSISTENT_BASH_START')
  })
