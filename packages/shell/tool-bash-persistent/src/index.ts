 ...
 * @module @cortex/tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSendResult, TerminalSessionId } from '@cortex/terminal'
...
const SHELL_PROMPT = '__CORTEX_PERSISTENT_BASH_PROMPT__ '
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__CORTEX_PERSISTENT_BASH_START_${nonce}__`,
    end: `__CORTEX_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __cortex_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__cortex_persistent_bash_status"`
}
