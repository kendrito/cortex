...
import { ShellExecutor } from '@cortex/shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@cortex/shell'
import SystemPrompt from '@cortex/system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@cortex/tools'
...
import ApprovalService from '@cortex/user-approval'
import type { ApprovalOutcome } from '@cortex/user-approval'
...
const spillDir = mkdtempSync(join(tmpdir(), 'cortex-tool-bash-spec-'))
...
describe('bash tool', () => {
  ...
  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'bash', { command: 'true', description: 'test command', workdir: '/nonexistent-cortex' })
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @cortex/jobs and @cortex/tool-jobs')
  })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingCortexHome = join(spillDir, 'cortex-home')
   ...
   * (the credential scrub in cortex-bash-local is the security control; see the
  ...
  class RecordingBashExecutor extends ShellExecutor {
    ...
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      return {
        ...
        ...request.cortexEnv !== undefined ? { cortexEnv: request.cortexEnv } : {},
        sandboxPolicy: request.sandboxPolicy,
      }
    }
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { cortexHome: recordingCortexHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$CORTEX_*')
    expect(description).not.toContain('CORTEX_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.cortexEnv).toEqual({
      CORTEX_HOME: recordingCortexHome,
      CORTEX_SESSION_ID: 'request-fg',
      CORTEX_SESSION_JSONL: path,
      CORTEX_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { CORTEX_SESSION_ID: 'spoofed', CORTEX_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.CORTEX_SESSION_ID
    ...
    expect(bash.requests[0]?.cortexEnv).toEqual({
      CORTEX_HOME: recordingCortexHome,
      CORTEX_SESSION_ID: 'request-id-only',
      CORTEX_SHELL: '1',
    })
    expect(process.env.CORTEX_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.cortexEnv)).toEqual([
      {
        CORTEX_HOME: recordingCortexHome,
        CORTEX_SESSION_ID: 'request-parent',
        CORTEX_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        CORTEX_SHELL: '1',
      },
      {
        CORTEX_HOME: recordingCortexHome,
        CORTEX_SESSION_ID: 'request-child',
        CORTEX_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        CORTEX_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.cortexEnv?.CORTEX_SESSION_JSONL).not.toBe(bash.requests[1]?.cortexEnv?.CORTEX_SESSION_JSONL)
  })
