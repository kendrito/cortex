 ...
 * harness home resolved by {@link resolveCortexHome} (`$CORTEX_HOME` > `~/.cortex`),
 ...
 * machine: every process sharing one `$CORTEX_HOME` reports the same id, and
 ...
 *
 * @module @cortex/anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `CORTEX_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveCortexHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
