...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from '@cortex/settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveCortexHome(config.cortexHome), 'settings.yaml'))
