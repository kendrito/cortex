...
import { writeFileAtomic } from '@cortex/atomic-write'
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'cortex-settings-local-'))
...
describe('boot and reads', () => {
  ...
  it('defaults the file location under the configured harness home', async () => {
    const dir = await tempDir()
    const ctx = await boot({ cortexHome: dir, watch: false })
