 ...
 * technique as cortex-session-persistence-jsonl's win32 suite): a small in-memory
...
describe('loadWin32DialogBindings over the fake COM world', () => {
  it('drives the full selection conversation with memory hygiene', async () => {
    ...
    expect(runFolderDialog(bindings, 'Choose a workspace directory', showing)).toBe('C:\\Zürich\\directory')
  ...
  it('cascades DPI contexts to the first the host accepts', async () => {
    ...
    expect(runFolderDialog(bindings, 'Pick', vi.fn())).toBe('C:\\Zürich\\directory')
...
describe('the worker entry over a mocked process boundary', () => {
  ...
  afterEach(() => {
    ...
    if (originalTitle === undefined) delete process.env.CORTEX_DIALOG_TITLE
    else process.env.CORTEX_DIALOG_TITLE = originalTitle
    vi.doUnmock('../src/win32-dialog-bindings.ts')
    vi.resetModules()
  })
  ...
  it('refuses to run without the dialog title', async () => {
    delete process.env.CORTEX_DIALOG_TITLE
    ;(process as { send?: unknown }).send = () => true
    await expect(import('../src/win32-dialog-worker.ts')).rejects.toThrow('CORTEX_DIALOG_TITLE is required')
  })
