...
import { TypertLookupFailure } from '@cortex/typert-protocol'
import TypertRegistry from '@cortex/typert-registry'
import { createUserMessage, MessageId } from '@cortex/llm'
import type { Agent } from '@cortex/agent'
import UserQuestionService from '@cortex/user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@cortex/session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@cortex/session-persistence'
import type { RpcRequest } from '@cortex/host-apiproxy/api/rpc'
import { RpcId } from '@cortex/host-apiproxy/api/rpc'
import { createApiProxy } from '@cortex/host-apiproxy'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'cortex-cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'cortex-cold-race-'))
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@cortex/session').Session
