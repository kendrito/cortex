...
describe('sessions domain schemas', () => {
  ...
  it('validates the per-method request/value pairs', () => {
    ...
    expect(sessionModelsValueSchema.parse({
      current: { provider: 'cortex-official', model: 'cortex-v4-flash', reasoningEffort: 'max' },
      routable: true,
      groups: [{
        id: 'cortex-official',
        name: 'Cortex',
    ...
    }).groups[0]?.models[0]?.id).toBe('cortex-v4-flash')
    ...
    expect(sessionSelectModelValueSchema.parse({
      selected: { provider: 'cortex-official', model: 'cortex-v4-pro', reasoningEffort: 'max' },
    ...
    expect(() => sessionModelsValueSchema.parse({
      current: { provider: 'cortex-official', model: 'm' },
      groups: [{
        id: 'cortex-official',
        name: 'Cortex',
    ...
    expect(prompt.clientTimeZone).toBe('Asia/Singapore')
...
describe('subagent domain schemas', () => {
  it('carries optional request-local browser-zone provenance on prompts', () => {
    expect(subagentPromptRequestSchema.parse({
      ...
      clientTimeZone: 'Asia/Singapore',
    }).clientTimeZone).toBe('Asia/Singapore')
    expect(subagentPromptRequestSchema.parse({
      parentSessionId: 'parent',
      childSessionId: 'child',
      mode: 'continuable',
