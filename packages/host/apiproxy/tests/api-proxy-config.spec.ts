...
import LlmRuntime, { LlmAdapter } from '@cortex/llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@cortex/llm'
import { SettingsProvider, settingsNamespace } from '@cortex/settings'
import type { SettingsNamespace } from '@cortex/settings'
import { CredentialProvider } from '@cortex/credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@cortex/credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@cortex/agent-default-model'
...
const NS = settingsNamespace('llm-acme')

const AdapterConfig = z.object({
  apiKey: z.string().role('secret'),
  apiKeyEnv: z.string().default('CORTEX_API_KEY'),
  baseURL: z.string(),
})
...
async function harness(options?: {
  ...
  if (options?.configurableProviders !== false) {
    ctx.llm.registerConfigurableProviders([
      { provider: 'cortex-official', displayName: 'Cortex', settingsNs: 'llm-acme', settingsPath: [] },
    ])
  }
...
describe('settings domain', () => {
  ...
  it('describes layered redacted namespaces with their secret slots', async () => {
    ...
    expect(view.ns).toBe('llm-acme')
    expect(view.applies).toBe('live')
    expect((view.schema as { refs?: unknown }).refs).toBeDefined()
    expect(view.value).toEqual({ apiKeyEnv: 'CORTEX_API_KEY', baseURL: 'https://user' })
  ...
  it('serves model-provider and explicitly allowlisted Web namespaces only', async () => {
    ...
    ctx.settings.register(settingsNamespace('locale'), z.object({
      preference: z.union(['fr', 'en']).required(false),
    }))
    ...
    expect(value.namespaces.map(view => view.ns)).toEqual([
      'llm-acme', 'permission', 'ui-theme', 'locale', 'ui-conversation',
      'shell', 'agent-loop',
    ])
  ...
  it('refuses even a model-provider namespace once its directory entry is gone', async () => {
    ...
    expect(expectErr(await api.settings.update(request({ ns: 'llm-acme', patch: { baseURL: 'https://x' } }))).code)
  ...
  it('forwards a provider settings change for model-catalog consumers', async () => {
    ...
    const frames = await collectHost(api, ['host/remote-event'], 1, async () => {
      await api.settings.update(request({ ns: 'llm-acme', patch: { baseURL: 'https://base' } }))
    })
    expect(frames).toEqual([forwardedSettings('llm-acme')])
    ...
    expect(expectOk(await api.settings.describe(request({}))).namespaces[0]!.value)
      .toEqual({ apiKeyEnv: 'CORTEX_API_KEY', baseURL: 'https://base' })
  })
  ...
  it('forwards an Agent-default settings change for model-catalog consumers', async () => {
    ...
    }), { base: { provider: 'cortex-official', model: 'cortex-v4-flash' } })
    const api = createApiProxy(ctx, DEFAULTS)
    ...
    const frames = await collectHost(api, ['host/remote-event'], 1, async () => {
      await defaultModel.replace({ provider: 'cortex-official', model: 'cortex-reasoner' })
    })
  ...
  it('maps a stale expectedRevision to settings-conflict carrying both revisions', async () => {
    ...
    expect(expectOk(await api.settings.update(request({ ns: 'llm-acme', patch: { baseURL: 'https://first' }, expectedRevision: opened })))
      .revision).toBe(opened + 1)
    const error = expectErr(await api.settings.update(request({ ns: 'llm-acme', patch: { baseURL: 'https://second' }, expectedRevision: opened })))
    expect(error.code).toBe('settings-conflict')
    expect(error.details).toEqual({ ns: 'llm-acme', expected: opened, actual: opened + 1 })
  ...
  it('updates the user layer, answers with the new redacted view, and broadcasts the frame', async () => {
    ...
    const api = createApiProxy(ctx, DEFAULTS)
    const frames = await collectHost(api, ['host/remote-event'], 1, async () => {
      const view = expectOk(await api.settings.update(request({ ns: 'llm-acme', patch: { apiKey: 'sk-new', baseURL: 'https://next' } })))
      expect(view.value).toEqual({ apiKeyEnv: 'CORTEX_API_KEY', baseURL: 'https://next' })
  ...
  it('replace resets the user layer wholesale', async () => {
    const ctx = await harness({ settings: { doc: { 'llm-acme': { baseURL: 'https://user' } } } })
    ctx.settings.register(NS, AdapterConfig)
    const api = createApiProxy(ctx, DEFAULTS)
    const view = expectOk(await api.settings.replace(request({ ns: 'llm-acme', section: {} })))
    expect(view.value).toEqual({ apiKeyEnv: 'CORTEX_API_KEY' })
    expect(view.user).toEqual({})
  })

  it.each([
    ['an invalid namespace name', 'Not A Namespace', {}],
    ['a schema-invalid patch', 'llm-acme', { baseURL: 42 }],
  ...
  it('maps a read-only provider refusal onto the same rejection', async () => {
    ...
    const error = expectErr(await api.settings.update(request({ ns: 'llm-acme', patch: {} })))
    expect(error.code).toBe('settings-rejected')
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('@cortex/credentials-local')
  })
  ...
  it('maps a shadowed write onto credential-rejected for set and unset alike', async () => {
    const ctx = await harness({ credentials: { shadowed: ['CORTEX_API_KEY'] } })
    const api = createApiProxy(ctx, DEFAULTS)
    const described = expectOk(await api.credentials.describe(request({ refs: ['CORTEX_API_KEY'] })))
    expect(described.credentials['CORTEX_API_KEY']).toEqual({ configured: true, source: 'env', writable: false })
    const setError = expectErr(await api.credentials.set(request({ ref: 'CORTEX_API_KEY', value: 'x' })))
    expect(setError.code).toBe('credential-rejected')
    expect(setError.details).toEqual({ ref: 'CORTEX_API_KEY' })
    const unsetError = expectErr(await api.credentials.unset(request({ ref: 'CORTEX_API_KEY' })))
...
describe('llm domain', () => {
  it('merges the configurable directory with live routes and appends undeclared ones', async () => {
    ...
    ctx.llm.registerAdapter(['cortex-official'], new CatalogAdapter('Cortex', ['cortex-v4-flash']))
    ...
    expect(value.providers).toEqual([
      { provider: 'cortex-official', displayName: 'Cortex', settingsNs: 'llm-acme', settingsPath: [], active: true },
  ...
  it('serves the host-scoped catalog with per-provider failures contained', async () => {
    const ctx = await harness()
    ctx.llm.registerAdapter(['cortex-official'], new CatalogAdapter('Cortex', ['cortex-v4-flash', 'cortex-v4-pro']))
    ...
    expect(value.groups).toEqual([{
      ...
      name: 'Cortex',
      models: [
        { id: 'cortex-v4-flash', name: 'cortex-v4-flash' },
  ...
  it('forwards llm/adapters-updated at every topology commit point', async () => {
    ...
    const api = createApiProxy(ctx, DEFAULTS)
    const frames = await collectHost(api, ['host/remote-event'], 2, async () => {
      const dispose = ctx.llm.registerAdapter(['cortex-official'], new CatalogAdapter('Cortex', []))
      dispose()
      return Promise.resolve()
    })
    expect(frames).toEqual([
...
describe('llm.discoverModels', () => {
  ...
  it('carries the route being edited so an adapter can answer from its own registry', async () => {
    ...
    expect(probe).toEqual({ provider: 'openai' })
