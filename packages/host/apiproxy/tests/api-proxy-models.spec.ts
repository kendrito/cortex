...
import AgentRegistry, { agentEvents } from '@cortex/agent'
import type { Agent } from '@cortex/agent'
import LlmRuntime, { LlmAdapter, ReasoningEffortId } from '@cortex/llm'
import type {
...
import type { RpcRequest } from '@cortex/host-apiproxy/api/rpc'
import { RpcId } from '@cortex/host-apiproxy/api/rpc'
...
async function harness(logged?: {
  ...
  ctx.llm.registerAdapter(['cortex-official'], new CatalogAdapter('Cortex', [
    { provider: 'cortex-official', id: 'cortex-chat', name: 'Cortex Chat' },
    { provider: 'cortex-official', id: 'cortex-reasoner', name: 'Cortex Reasoner', description: 'Reasoning model' },
  ], REASONING))
...
describe('Web session model selection', () => {
  it('validates an ordered image batch before persisting any member', async () => {
    ...
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'cortex-official', model: 'cortex-chat' }),
      cwd: '/tmp',
    })
  ...
  it('groups successful providers and leaves an unlisted current selection out of the catalog', async () => {
    ...
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'cortex-official', model: 'cortex-chat' }), cwd: '/tmp' })
    ...
    expect(catalog.groups).toEqual([{
      ...
      name: 'Cortex',
      models: [
        { id: 'cortex-chat', name: 'Cortex Chat', reasoning: REASONING },
        {
          id: 'cortex-reasoner',
          name: 'Cortex Reasoner',
  ...
  it('accepts an advisory-unlisted model, rejects an unavailable provider, and switches only after the next assembly', async () => {
    ...
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'cortex-official', model: 'cortex-chat' })
    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'cortex-official', model: 'cortex-chat' })
    ...
    )).resolves.toMatchObject({ provider: 'cortex-official', model: 'cortex-chat' })

    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'cortex-official', model: 'private-preview' })
    await expect(agentEvents(ctx, agent).waterfall(
    ...
    expect(unsupported.result).toMatchObject({
      ...
      error: {
        ...
        message: 'provider "cortex-official" model "private-preview" does not support reasoning effort "medium"',
      },
    })
    ...
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'cortex-official', model: 'private-preview', reasoningEffort: 'max' })
    await ctx.fiber.dispose()
  })

  it('reads the Agent default live for a session whose log names no selection', async () => {
    const { ctx, sessionId } = await harness()
    let stored = { provider: 'cortex-official', model: 'cortex-chat' }
    const api = createApiProxy(ctx, {
    ...
    stored = { provider: 'cortex-official', model: 'cortex-reasoner' }
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'cortex-official', model: 'cortex-reasoner' })
    expect(expectValue(await api.host.describe(request({}))))
      .toMatchObject({ provider: 'cortex-official', model: 'cortex-reasoner' })
    await ctx.fiber.dispose()
  })
  ...
  it('saves an accepted selection as the default and survives a storage failure', async () => {
    ...
    expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'cortex-official', model: 'cortex-reasoner', reasoningEffort: 'max',
    })))
    expect(saved).toEqual([
      { provider: 'cortex-official', model: 'cortex-reasoner', reasoningEffort: 'max' },
    ])
    ...
    const stillAccepted = expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'cortex-official', model: 'cortex-chat',
    })))
    expect(stillAccepted.selected).toEqual({ provider: 'cortex-official', model: 'cortex-chat', reasoningEffort: 'high' })
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'cortex-official', model: 'cortex-chat', reasoningEffort: 'high' })
    await ctx.fiber.dispose()
  })
  ...
  it('refuses a prompt no adapter can route, and reports it on the directory', async () => {
    ...
    expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'cortex-official', model: 'unlisted-but-served',
    })))
    const catalog = expectValue(await api.sessions.models(request({ sessionId })))
