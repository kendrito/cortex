...
import type { SessionHeader, SessionId } from '@cortex/session'
import type { SessionLineageNode } from '@cortex/session-query'
import type { SessionRawArtifact } from '@cortex/session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from '@cortex/host-apiproxy'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root artifact verbatim under its original filename', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('cortex-session-session-root.zip')
