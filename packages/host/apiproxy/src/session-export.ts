...
import type { AttachmentStore, ImageAttachmentRef } from '@cortex/attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@cortex/session-query'
import type { SessionId, SessionStore } from '@cortex/session'
import type { SessionPersistence, SessionRawArtifact } from '@cortex/session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `cortex-session-${safeSessionIdSegment(sessionId)}.zip`
}
