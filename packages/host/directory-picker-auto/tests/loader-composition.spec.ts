...
import HttpServer from '@cortex/host-webserver'
import type { DirectoryPicker } from '@cortex/host-directory-picker'
import BrowseDirectoryPicker from '@cortex/host-directory-picker-browse'
import NativeDirectoryPicker from '@cortex/host-directory-picker-native'
...
const AUTO = '@cortex/host-directory-picker-auto'
const NATIVE = '@cortex/host-directory-picker-native'
const BROWSE = '@cortex/host-directory-picker-browse'
const NATIVE_SURFACE = '@cortex/client-ui-directory-picker-native'
const BROWSE_SURFACE = '@cortex/client-ui-directory-picker-browse'
...
async function loadComposition(
  ...
  root = await mkdtemp(join(tmpdir(), 'cortex-directory-picker-auto-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@cortex/host-webserver'",
    '  config:',
    `    host: '${bindHost}'`,
    '    port: 0',
    `- name: '${AUTO}'`,
  ...
  const modules = new Map<string, unknown>([
    ['@cortex/host-webserver', HttpServer],
...
function stubAttendedHost(): void {
  fakeBin = mkdtempSync(join(tmpdir(), 'cortex-picker-bin-'))
