/**
 * Package-owned invariant companion for `@cortex/host-webserver`.
 * @module @cortex/host-webserver/invariant
 */
...
const PACKAGE_NAME = '@cortex/host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__cortex_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__cortex_invariant_upgrade_probe__', handler: () => {} }
