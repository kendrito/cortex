 ...
 * kernel denial matches the advertised dialect; consumer coverage lives in cortex-bash-sandbox.
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'cortex-bwrap-e2e-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(!bwrapUsable)('sandbox-local: real bwrap confinement', () => {
  ...
  it('workspace-write mounts an EPHEMERAL /tmp: the write succeeds inside, the host /tmp stays untouched', async () => {
    ...
    const workdir = await tempDir(homedir())
    const target = `/tmp/cortex-bwrap-e2e-ephemeral-${process.pid}.txt`
