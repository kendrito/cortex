...
const platformPackageName = `@cortex/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @cortex peer (koffi arrives
  // from the registry).
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'cortex-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'cortex-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'cortex-packed-work-'))
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@cortex/node-addon-landlock-run'
