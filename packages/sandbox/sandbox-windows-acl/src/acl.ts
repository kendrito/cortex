 ...
 * @module @cortex/sandbox-windows-acl/acl
 */
...
/**
 * One lock file per protected path: `<GetTempPathW()>\cortex-acl-locks\<first 16
 * hex of sha256(lowercased path)>.lock`. The lock root derives from
 * GetTempPathW (never from runner argv or CORTEX_HOME), and the lowercasing
...
export function lockFilePath(api: Win32Bindings, path: string): string {
  ...
  return join(getTempPath(api), 'cortex-acl-locks', `${digest}.lock`)
}
