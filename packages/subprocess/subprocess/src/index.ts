 ...
 * `@cortex/subprocess-local`.
 * @module @cortex/subprocess
 */

import { Context, Service } from '@cortex/cordis'
import { CORTEX_ENV_PREFIX } from './types.ts'
import type { SubprocessHandle, SubprocessSpawnSpec } from './types.ts'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from './types.ts'

export { CORTEX_ENV_PREFIX } from './types.ts'
export type {
  CollectedOutput,
  CortexEnvironment,
  CortexEnvironmentKey,
  SubprocessCollect,
 ...
 * harness's own `CORTEX_API_KEY`/secrets must not leak into a spawned
 ...
 * `CORTEX_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `CORTEX_*` fact goes through the spec's explicit `env`,
 ...
 * would otherwise survive and read back as `$env:CORTEX_*` in the child;
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(CORTEX_ENV_PREFIX)) env[key] = value
  }
  return env
}
