 ...
 * @module cortex-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for Cortex-managed child environment facts. */
export const CORTEX_ENV_PREFIX = 'CORTEX_' as const

/** One environment key inside the managed {@link CORTEX_ENV_PREFIX} namespace. */
export type CortexEnvironmentKey = `${typeof CORTEX_ENV_PREFIX}${string}`

/** Trusted Cortex variables for one child-process execution. */
export type CortexEnvironment = Readonly<Record<CortexEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@cortex/terminal` without a cross-seam dependency;
