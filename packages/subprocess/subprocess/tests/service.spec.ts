...
import { scrubbedParentEnv, SubprocessRuntime } from '@cortex/subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and CORTEX_ names (case-insensitively) but keeps PATH', () => {
    process.env.CORTEX_SCRUB_PROBE = 'stale'
    process.env.cortex_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.CORTEX_SCRUB_PROBE).toBeUndefined()
      expect(env.cortex_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.CORTEX_SCRUB_PROBE
      delete process.env.cortex_scrub_probe_lower
