...
- A Cortex-compatible API endpoint and credential
...
Set the credential in the environment. Set `CORTEX_BASE_URL` as well when the model is served by an OpenAI-compatible proxy rather than the default Cortex endpoint.

```sh
export CORTEX_API_KEY=sk-your-key-here
# export CORTEX_BASE_URL=http://127.0.0.1:8000/v1
# export CORTEX_MODEL=cortex-v4-flash
# export CORTEX_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Session persistence | Uncompressed JSONL under `CORTEX_SESSION_ROOT` |
