...
This tutorial creates a minimal Harness plugin and loads it into the Web UI. Start from a repository checkout that has completed the [run-from-source path](../../../../README.md#run).
...
Run `pwd` from the repository root, then create `scratch-plugin/cordis.yml` as a Web overlay that inserts the local plugin. Replace `/absolute/path/to/cortex` below with the printed path:

```yaml
- insert:
    - id: hello
      name: '/absolute/path/to/cortex/scratch-plugin/src/my-plugin.ts'
```
...
```sh
pnpm cortex web --patch ./scratch-plugin/cordis.yml
```
