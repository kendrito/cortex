...
- **Service Definition** (`cortex-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`cortex-bash-local`) — executes commands on the local machine
- **Consumer** (`cortex-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  cortex-shell   │────▶│  cortex-bash-local  │     │ cortex-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@cortex/my-cap'
...
```yaml
- name: '@cortex/my-cap-local'
- name: '@cortex/tool-my-cap'
```
