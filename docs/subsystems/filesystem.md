...
The optional filesystem capability has four parts: [cortex-fs](../../packages/fs/fs) owns `ctx.fs` and atomic text operations with optional guards, [cortex-fs-local](../../packages/fs/fs-local) implements local disk, [cortex-fs-observation-policy](../../packages/fs/fs-observation-policy) records observed presence or absence and adds freshness rules through events rather than a service, and [cortex-tool-fs](../../packages/fs/tool-fs) directly executes model-facing read/write/edit calls and renders windows. It is outside the agent-loop spine; alternate backends do not change policy or tool schemas.

`cortex-fs-observation-policy` is optional. Without it, the `FileSystem` Service Definition, a provider, and the `cortex-tool-fs` Consumer form the complete, unconstrained filesystem seam: `write` unconditionally creates or overwrites, and `edit` unconditionally replaces literal text. The policy plugin changes these operations by deciding the `fs/*` waterfalls. Removing it does not break the tool because the tool calls `ctx.fs` and dispatches events; it does not call policy methods. A deployment that loads `cortex-tool-fs` is expected to also load `cortex-fs-observation-policy` so the default behavior is read-before-write/edit.
...
`cortex-fs` owns three events the tool dispatches and the policy plugin listens for, so the emitter (`cortex-tool-fs`) and the listener (`cortex-fs-observation-policy`) share a vocabulary without the emitter depending on the policy plugin. They carry only `cortex-fs` vocabulary plus an opaque `object` actor — no model-facing concepts and no agent/session owner structure.
 ...
 * `FsObservationActor` without importing `cortex-tools`, `cortex-agent`, or `cortex-session`.
 *
