 ...
 * metadata (the authoritative edges live in each package's `cortex.client`
...
```ts type-equiv
/** The composed client entry graph the host injects as `window.__CORTEX_BOOT__`. */
