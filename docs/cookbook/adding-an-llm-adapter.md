...
How to connect a new model provider. Reference implementation: `packages/llm/llm-pi-ai` (wrapping the `@earendil-works/pi-ai` library, one adapter instance serving a dict of provider routes). Read the `StreamChunk` doc in `packages/llm/llm/src/types.ts` first — it records the protocol conventions the adapter was verified against.
...
## Protocol obligations (the contract the shipped implementation verified)
...
Keep wire types, request serialization, transport parsing, chunk translation, and the adapter class as separate responsibilities; [`llm-pi-ai`](../../packages/llm/llm-pi-ai/README.md) is the reference layout — `config.ts` (profile schema and resolution), `catalog.ts` (model catalog materialization), `provider.ts` (per-route provider and wire-protocol construction), `context.ts` (request-history conversion), `stream.ts` (chunk translation), `replay.ts` (native replay metadata), and `adapter.ts` (the `LlmAdapter` subclass) are separate modules behind one plugin entry.
