...
export interface PackageGraphNode {
  /** Package name with the `@cortex/` prefix removed. */
  short: string
...
export function collectPackageGraph(root: string, groupOrder: readonly string[], gate: string): PackageGraphNode[] {
  ...
  // Only edges to packages this scan found are graph edges: the vendored
  // framework packages share the scope but live outside `packages/`.
  const known = new Set(packages.map(pkg => pkg.short))
  for (const pkg of packages) pkg.deps = pkg.deps.filter(dep => known.has(dep))
