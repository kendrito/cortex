...
const SESSION_PACKAGE = '@cortex/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@cortex/session/types'
 ...
 * top-level declaration (in `@cortex/session`) and any declaration
 * merge inside a `declare module '@cortex/session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @cortex/session/known-event-types',
    ' */',
    '',
    '/**',
