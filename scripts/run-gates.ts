...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.CORTEX_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('CORTEX_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$CORTEX_GATE_CONCURRENCY'
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { CORTEX_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('cortex-package-licenses', 'verify-cortex-package-licenses', { label: 'Cortex package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('CORTEX_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('cortex-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'cortex source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { CORTEX_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { CORTEX_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[]): Gate {
  return pnpmScript('web-snapshot', 'test:web:built', {
    ...
    displayCommand: 'CORTEX_SNAPSHOT=replay pnpm run test:web:built',
    env: { CORTEX_SNAPSHOT: 'replay' },
    needs,
  })
}
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.CORTEX_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `CORTEX_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// CORTEX_COVERAGE_MAX_WORKERS is the lane's worker budget, so the two parallel
...
// CORTEX_GATE_CONCURRENCY=1, which keeps the gates from overlapping at all.
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('CORTEX_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.CORTEX_GATE_VERBOSE === '1'
