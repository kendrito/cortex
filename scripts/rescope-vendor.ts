/**
 * Rescope the vendored Cordis packages into the `@cortex` scope, and undo
 ...
 * `@cortex/tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@cortex/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@cortex/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@cortex/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@cortex/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@cortex/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@cortex/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@cortex/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@cortex/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@cortex/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@cortex/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@cortex/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@cortex\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@cortex/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@cortex/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@cortex/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@cortex/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@cortex/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@cortex/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@cortex` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@cortex\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@cortex/cordis']
    const dev = manifest.devDependencies?.['@cortex/cordis']

    if (!peer) errors.push(\`\${label}: @cortex/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @cortex/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @cortex/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    // The rescoped name is already covered by the `@cortex/.+` pattern beside it.
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@cortex/\')) {',
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@cortex/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@cortex/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@cortex/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@cortex/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @cortex, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@cortex\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@cortex/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@cortex\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@cortex/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @cortex, so the consumer installs this repository's
  ...
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'cortex-packed-consumer', private: true, type: 'module' }))
