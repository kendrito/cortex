 ...
 * silently changes composition. Example configs and the cortex Web composition
...
// These example files are overlays consumed by the built cortex app, so their bare
...
const CHOOSER_PACKAGE = '@cortex/host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@cortex/host-directory-picker-native',
  '@cortex/host-directory-picker-browse',
  '@cortex/client-ui-directory-picker-browse',
  '@cortex/client-ui-directory-picker-native',
]
 ...
 * `cortex.client` block, and the node half of a surface plugin is an empty
 ...
 * `cortex.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    const manifest = readManifest(manifestPath) as PackageManifest & {
      ...
      cortex?: { client?: unknown }
    }
    const shipsClient = manifest.exports !== undefined && Object.hasOwn(manifest.exports, './client')
    const declaresClient = manifest.cortex?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no cortex.client, so its browser half is never served`
      : `${manifestPath}: declares cortex.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `CORTEX_WEB_URL` reaching
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@cortex/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@cortex/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // cortex app's own dependency surface — the profile module fallback mirrors it.
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `cortex` source
