...
const MERGE_HEAD = /declare module ['"](?:@cortex\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@cortex/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@cortex/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@cortex/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@cortex/cordis'` Context merge
...
/**
 * Every event name a `declare module '@cortex/cordis'` Events merge declares in one
