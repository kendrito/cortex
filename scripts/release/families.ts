 ...
 * owns: `cortex` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@cortex/root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@cortex/')) throw new Error(`${normalized} must name an @cortex package`)
...
class CortexFamily extends ReleaseFamily {
  readonly id = 'cortex'
  readonly patterns = ['packages/*/*/package.json', 'apps/*/package.json'] as const
  readonly tagPrefix = 'cortex-v'
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`cortex release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@cortex/cortex', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@cortex/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new CortexFamily(), new VendorFamily()]
}
