...
/** Drop the `@cortex/` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@cortex/', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @cortex/cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
