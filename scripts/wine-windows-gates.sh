...
#
# Environment: CORTEX_WINE_NODE_MAJOR (default $PRIMARY_NODE_VERSION, then 24)
# picks the Windows Node line; CORTEX_WINE_GATE_CACHE_DIR relocates the cache;
# CORTEX_WINE_GATE_KEEP=1 preserves the scratch tree for inspection.

set -euo pipefail

repo_root="$(git rev-parse --show-toplevel)"
node_major="${CORTEX_WINE_NODE_MAJOR:-${PRIMARY_NODE_VERSION:-24}}"
cache_dir="${CORTEX_WINE_GATE_CACHE_DIR:-$repo_root/.cache/wine-windows}"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/cortex-wine-gates.XXXXXX")"
cleanup() {
  wineserver -k > /dev/null 2>&1 || true
  if [ "${CORTEX_WINE_GATE_KEEP:-0}" = '1' ]; then
...
download_node_archive() {
  ...
  local mirror_url="https://nodejs.org/download/release/$version/$archive"
  ...
  echo 'wine-windows-gates: nodejs.org dist transfer stalled; resuming from the release download path' >&2
...
build_gate() {
  ...
  wine_node "$scratch/logs/host-tsdown.log" "$tsdown_js" --env.CORTEX_BUILD_FACE host || return $?
  wine_node "$scratch/logs/client-tsc.log" "$tsc_js" -b tsconfig.client.json --pretty false || return $?
  wine_node "$scratch/logs/client-tsdown.log" "$tsdown_js" --env.CORTEX_BUILD_FACE client
}
site_gate() {
  cd website
