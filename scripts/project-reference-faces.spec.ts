...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'cortex-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@cortex/shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@cortex/split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@cortex/host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@cortex/client-consumer' })
