/**
 * Watch-build for client-plugin HMR: runs every `cortex.client` plugin package
 ...
 * broadcasts `rebuilt` frames itself (`cortex web`), so any process that
 ...
 * whose package.json carries `cortex.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      cortex?: { client?: { platform?: unknown } }
    }
    if (manifest.cortex?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
...
if (isMain) {
  ...
  if (pluginDirs.length === 0) {
    console.error('dev-web: no cortex.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  console.log(
    `dev-web: watching ${String(pluginDirs.length)} cortex.client plugin packages`
