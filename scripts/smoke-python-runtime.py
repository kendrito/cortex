...
if TYPE_CHECKING:
    from cortex_harness import RunResult
...
- id: sdk-jsonrpc-server
  name: '@cortex/sdk-jsonrpc-server'
- id: agent-core
  name: '@cortex/agent-spine-demo'
  config:
    ...
    tools:
      mode: both
- id: sessions
  name: '@cortex/session-persistence-jsonl'
  config:
    root: !!js process.env.CORTEX_SESSION_ROOT
    compression: 'none'
- id: code-runtime
  name: '@cortex/code-runtime-worker-thread'
- id: subagents
  name: '@cortex/subagent'
- id: subagent-spawn-in-process
  name: '@cortex/subagent-spawn-in-process'
  config:
    providerName: spawn
- id: subagent-tool
  ...
  config:
    provider: spawn
- id: workflow-engine
  name: '@cortex/workflow-worker-thread'
  config:
    provider: spawn
- id: workflow-tool
  name: '@cortex/tool-workflow'
- id: cordis-host-runner
  name: '@cortex/cordis-host-runner'
- id: cordis-tool
  name: '@cortex/tool-cordis'
"""
...
def smoke_sdk_default(base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cortex-sdk-default-") as temporary:
...
def smoke_sdk_custom(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cortex-sdk-custom-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
...
def smoke_sdk_minimal(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cortex-sdk-minimal-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_snapshot(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cortex-sdk-snapshot-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="cortex-direct-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
        environment = {
            **os.environ,
            "CORTEX_CORDIS_CONFIG": str(cordis),
            "CORTEX_SESSION_ROOT": str(sessions),
            "CORTEX_CWD": str(root),
            "CORTEX_API_KEY": "sk-keyless-smoke",
            "CORTEX_BASE_URL": base_url,
        }
        peer = RuntimePeer([str(executable)], root, environment)
        try:
            peer.send({"jsonrpc": "2.0", "id": "initialize", "method": "initialize", "params": {"cwd": str(root), "provider": "cortex-official", "model": "smoke-model"}})
