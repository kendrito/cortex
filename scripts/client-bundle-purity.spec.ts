...
function clientConfigs(id = '@cortex/client-test') {
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@cortex/client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { CORTEX_BUILD_FACE: 'client' } })
...
function purityResolveId(): ResolveId {
  ...
  const gate = plugins.find(p => p.name === 'cortex-client-bundle-purity')
...
function cssModulePlugin(): CssModulePlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'cortex-css-modules-inline')
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@cortex/client-ui-slots')).toBeNull()
    expect(resolveId('@cortex/client-web-react')).toBeNull()
    expect(resolveId('@cortex/client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@cortex/client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@cortex/host-apiproxy/api')).toBeNull()
    expect(resolveId('@cortex/session/surface')).toBeNull()
    expect(resolveId('@cortex/brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@cortex/goal/remote')).toBeNull()
    expect(() => resolveId('@cortex/goal')).toThrow(/purity/)
    expect(() => resolveId('@cortex/goal/client')).toThrow(/purity/)
    expect(() => resolveId('@cortex/goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @cortex leak', () => {
    expect(() => resolveId('@cortex/agent')).toThrow(/purity/)
    expect(() => resolveId('@cortex/client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@cortex/client-connection')).toThrow(/purity/)
    expect(() => resolveId('@cortex/client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@cortex/client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@cortex/client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@cortex/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@cortex/client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@cortex/client-ui-goal')
    ...
    const resolved = new URL(source, 'https://cortex.test/plugins/@cortex/client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@cortex/host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@cortex/client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://cortex.test/plugins/@cortex/client-connection/client.js.map')
