...
const vendoredPackages = new Set([
  ...
  '@cortex/cosmokit',
  '@cortex/schemastery',
  '@cortex/cordis-plugin-loader',
  '@cortex/cordis-plugin-include',
  '@cortex/cordis-plugin-group',
  '@cortex/cordis-plugin-timer',
  '@cortex/cordis-plugin-hmr',
  '@cortex/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@cortex/node-addon-landlock-run',
  '@cortex/node-addon-landlock-run-linux-arm64',
  '@cortex/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@cortex/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/local/cortex.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/local/cortex.git'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/

const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@cortex/cortex': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@cortex/web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  // Profile bundles publish their cortex.bundle.patch layer beside the lib.
  '@cortex/base': ['cordis.patch.yml'],
  '@cortex/web-app': ['cordis.patch.yml'],
  '@cortex/headless': ['cordis.patch.yml'],
  '@cortex/client-ui-theme': ['lib/styles'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@cortex/sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@cortex/sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@cortex/subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function expectedCortexPackageFiles(manifest: PackageManifest): readonly string[] {
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the cortex family stays restricted until its own sequence goes
  ...
  if (manifest.name?.startsWith('@cortex/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@cortex/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@cortex/')) {
    const peer = manifest.peerDependencies?.['@cortex/cordis']
    ...
    const expectedFiles = expectedCortexPackageFiles(manifest)
...
function checkRepositoryVersion(): string[] {
  // The root carries the cortex release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:cortex` and its publication.
