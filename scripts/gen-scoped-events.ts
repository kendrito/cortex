/**
 * Generate cortex-scope's invariant resolver map from the repository TypeScript
 * Program.
 *
 ...
 * `Events` parameter tuple. Zero matches require `@cortexScopeScan unsupported`;
...
class ScopedEventGenerator {
  ...
  render(): string {
    ...
    return [
      '/**',
      ' * Generated scoped-event routing-subject resolvers for cortex-scope invariants.',
      ' * Do not edit by hand; run `pnpm run gen-scoped-events`.',
      ' *',
      ' * @module @cortex/scope/scoped-events.generated',
      ' */',
      '',
        ...
        if (ts.isInterfaceDeclaration(node) && node.name.text === 'Events' && isCordisModuleInterface(node)) {
          for (const member of node.members) {
            ...
            if (!scopedBase) {
              ...
              if (tag.present) {
                this.violations.push(`${where} has @cortexScopeScan metadata but is not a Scoped event`)
              }
              continue
            }
            if (!raw.includes(SCOPE_DOC_MARKER)) {
            ...
            if (candidates.length === 0) {
              if (!tag.unsupported) {
                ...
                this.violations.push(
                  ...
                  + 'add @cortexScopeScan unsupported only when the key is intentionally absent from the payload',
                )
              }
            ...
            if (tag.unsupported) {
              this.violations.push(
                `${where} has unnecessary @cortexScopeScan unsupported; ${candidates[0]?.path} exposes the routing key`,
              )
              continue
            }
...
/** Return whether an Events interface is inside declare module '@cortex/cordis'. */
function isCordisModuleInterface(node: ts.InterfaceDeclaration): boolean {
  ...
  return ts.isModuleBlock(block)
    ...
    && declaration.name.text === '@cortex/cordis'
}
...
/** Parse and validate the optional @cortexScopeScan unsupported tag. */
function parseScopeTag(raw: string, where: string, violations: string[]): ScopeTag {
  const tags = raw
    ...
    .filter(line => line.startsWith('@cortexScopeScan'))
  if (tags.length > 1) violations.push(`${where} has multiple @cortexScopeScan tags`)
  if (tags.length === 0) return { present: false, unsupported: false }
  const unsupported = tags[0] === '@cortexScopeScan unsupported'
  if (!unsupported) {
    violations.push(
      `${where} has invalid scoped-event scan metadata '${tags[0]}'; expected '@cortexScopeScan unsupported'`,
    )
  }
...
/** Generate or freshness-check the fixed cortex-scope source file. */
function main(): void {
