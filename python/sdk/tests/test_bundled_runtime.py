...
from cortex_harness import CortexHarness, HarnessClient, HarnessConfig
from cortex_harness.errors import TransportClosedError
...
- id: session-checkpoints
  name: '@cortex/session-checkpoint-policy'
- id: subprocess
...
def _client(tmp_path: Path, launch_args: tuple[str, ...]) -> HarnessClient:
    return HarnessClient(
        HarnessConfig(
            ...
            cwd=str(tmp_path),
            env={
                "CORTEX_CORDIS_CONFIG": "./cordis.yml",
                "CORTEX_SESSION_ROOT": str(tmp_path / "sessions"),
                "CORTEX_CWD": str(tmp_path),
                # The lazily mounted adapter requires a key even without a model call.
                "CORTEX_API_KEY": "sk-dummy-for-boot",
                "CORTEX_BASE_URL": "http://127.0.0.1:9",
            },
...
def test_bundled_runtime_boots_a_cordis_config(tmp_path: Path, mode: str) -> None:
    ...
    with _client(tmp_path, launch_args) as client:
        init = client.initialize(provider="cortex-official", cwd=str(tmp_path), model="cortex-v4-pro")

    assert init.serverInfo is not None
    assert init.serverInfo.name == "cortex-sdk-runtime"
...
def test_python_sdk_boots_minimal_jsonrpc_config(tmp_path: Path, mode: str) -> None:
    ...
    harness = CortexHarness(
        ...
        env={
            "CORTEX_MODEL": model,
            "CORTEX_CONTEXT_WINDOW": "1000000",
            "CORTEX_SYSTEM_PROMPT": "You are the Python SDK minimal boot test agent.",
        },
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    try:
        with pytest.raises((TransportClosedError, TimeoutError)) as excinfo:
            client.initialize(provider="cortex-official", cwd=str(tmp_path), model="cortex-v4-pro")
    finally:
        client.close()

    assert "@cortex/does-not-exist" in str(excinfo.value)
...
def test_zero_config_run_injects_bundled_default_cordis_config(
    ...
    monkeypatch.setenv("CORTEX_RUNTIME_MODE", mode)
