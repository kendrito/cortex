...
json.dump({
    ...
    "CORTEX_CWD": os.environ.get("CORTEX_CWD"),
    "CORTEX_SESSION_ROOT": os.environ.get("CORTEX_SESSION_ROOT"),
...
""".strip()
    ...
    with CortexHarness(
        ...
        env={
            ...
            "CORTEX_BASE_URL": "http://127.0.0.1:4321",
        },
    ) as harness:
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("CORTEX_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from cortex_harness import CortexHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(CortexHarness.run).parameters
