...
class CortexHarnessConfig:
    ...
    CORTEX_API_KEY and CORTEX_BASE_URL settings keep working. Use ``env`` to
...
class CortexHarness:
    """Reusable synchronous SDK for running Cortex agent turns.
    ...
    def __init__(self, config: CortexHarnessConfig | None = None, **kwargs: object) -> None:
        ...
        if self.config.api_key is not None:
            env["CORTEX_API_KEY"] = self.config.api_key
    ...
    def __enter__(self) -> "CortexHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: CortexHarness, session_id: str) -> None:
