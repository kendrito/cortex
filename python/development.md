...
An interactive smoke test needs `CORTEX_API_KEY` in the environment or repository-root `.env`:
...
- Set `CORTEX_RUNTIME_MODE=node` to use the built Node carrier on system Node `>=22.19`. The build script refreshes this carrier, but distributions never include or auto-select it.
...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `cortex-runtime-bin` version.
...
python scripts/build-python-release.py --package runtime --platform macos-arm64 --runtime-exe dist-exe/cortex-jsonrpc-agent-pkg-macos-arm64 --output-dir dist-python
pip install --find-links dist-python cortex-sdk=="$version"
```
