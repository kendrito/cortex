...
def _host_platform_tag() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"unsupported cortex-runtime-bin build platform: {key}") from exc
...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        if self.target_name == "sdist":
            raise RuntimeError(
                "cortex-runtime-bin is wheel-only; build and publish platform wheels only."
            )

        platform_tag = os.environ.get("CORTEX_RUNTIME_PLATFORM_TAG") or _host_platform_tag()
        ...
        if len(matches) != 1:
            ...
            raise RuntimeError(
                f"unsupported CORTEX_RUNTIME_PLATFORM_TAG {platform_tag!r}; expected one of {supported}"
            )
        expected_executable = matches[0][1]
        runtime_dir = Path(self.root) / "src" / "cortex_harness_runtime" / "runtime"
        runtime_files = sorted(runtime_dir.glob("cortex-jsonrpc-agent-pkg-*") if runtime_dir.is_dir() else [])
