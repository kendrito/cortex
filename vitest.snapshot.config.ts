...
const snapshotMaxConcurrency = positiveIntFromEnv(
  'CORTEX_SNAPSHOT_MAX_CONCURRENCY',
...
if (process.env.CORTEX_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.CORTEX_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.CORTEX_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,
