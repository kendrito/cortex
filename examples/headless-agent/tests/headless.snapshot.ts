...
} from '@cortex/acp-snapshot'
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@cortex/loader-smoke'
import {
  decompressZstdFrame,
  scanZstdFrames,
} from '@cortex/session-persistence-jsonl/src/zstd.ts'
...
const cortexBinScript = fileURLToPath(new URL('../../../apps/cli/src/bin.ts', import.meta.url))
...
const refreshing = process.env.CORTEX_SNAPSHOT === 'refresh'
...
async function prepareCliMockFixture(cwd: string): Promise<void> {
  const fixtureDir = join(cwd, '.cortex', 'profiles', 'headless', 'snapshot-fixtures')
...
describe('headless stream-json snapshots', () => {
  ...
  it('recovers from context overflow through an assembled compaction', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        CORTEX_SNAPSHOT: 'replay',
        CORTEX_SNAPSHOT_FILE: compactionSessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('logs actionable missing-credential guidance through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tsconfigPath,
      env: {
        // First-run posture: no key in the environment, none under ./.cortex.
        CORTEX_API_KEY: '',
        CORTEX_BASE_URL: '',
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
    ...
    if (refreshing) await writeFile(streamExpected, normalized)
    expect(normalized).toBe(await readFile(streamExpected, 'utf8'))
    // The durable failure names the reference the route resolves, leads with
    // the credential store — the path that keeps the secret out of
    // configuration files — then the environment export, and stops there:
    // configuration carries the reference, so there is no literal-key escape
    // hatch left to offer.
    expect(normalized).toContain('its profile resolves CORTEX_API_KEY, which is not set')
    expect(normalized).toContain(
      'store CORTEX_API_KEY through the credentials service (the web Models page writes it) or export it',
    )
  ...
  it('logs actionable invalid-credential guidance through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        CORTEX_API_KEY: 'sk-\u{1F600}pasted-from-a-chat-window',
        CORTEX_BASE_URL: '',
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
    ...
    // page at all.
    expect(normalized).toContain('the API key resolved from CORTEX_API_KEY contains characters')
  ...
  it('replays the advanced toolchain through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        CORTEX_SNAPSHOT: 'replay',
        CORTEX_SNAPSHOT_FILE: advancedSessionFixture,
        CORTEX_SNAPSHOT_CHILD_FILES: [
          join(advancedScenarioDir, 'session.1.jsonl'),
          join(advancedScenarioDir, 'session.2.jsonl'),
        ].join(delimiter),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
  ...
  it('replays persisted goal tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        CORTEX_SNAPSHOT: 'replay',
        CORTEX_SNAPSHOT_FILE: join(goalScenarioDir, 'session.jsonl'),
        CORTEX_SNAPSHOT_OVERRIDE: join(goalScenarioDir, 'replay.override.json'),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('replays two fresh Ralph rounds through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        CORTEX_SNAPSHOT: 'replay',
        CORTEX_SNAPSHOT_FILE: join(ralphScenarioDir, 'session.jsonl'),
        CORTEX_SNAPSHOT_OVERRIDE: join(ralphScenarioDir, 'replay.override.json'),
        CORTEX_SNAPSHOT_CHILD_FILES: [
  ...
  it('delivers a continuable child result without parent polling', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        CORTEX_SNAPSHOT_FILE: parentReplay,
        CORTEX_SNAPSHOT_OVERRIDE: parentOverride,
        CORTEX_SNAPSHOT_CHILD_FILES: childReplay,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('replays persistent PTY tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        CORTEX_SNAPSHOT: 'replay',
        CORTEX_SNAPSHOT_FILE: ptySessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
