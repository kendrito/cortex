...
import { boot } from '@cortex/app-boot'
import { Inbox } from '@cortex/agent'
import type { Agent } from '@cortex/agent'
import { Session, SessionId } from '@cortex/session'
import type {} from '@cortex/fs-e2b'
import type {} from '@cortex/bash-local'
import type {} from '@cortex/lsp-stdio'
import type {} from '@cortex/terminal-bash'
...
try {
  ...
  const splitUtf8Handle = ctx.subprocess.spawn({
    argv: ['bash', '-c', "printf 'Gr'; sleep 0.05; printf '\\303'; sleep 0.05; printf '\\266'; sleep 0.05; printf '\\303'; sleep 0.05; printf '\\237'; sleep 0.05; printf 'e'"],
    cwd: process.cwd(),
  ...
  if (splitUtf8Outcome.exitCode !== 0 || splitUtf8Output !== 'Größe') {
  ...
  const outputDrainHandle = ctx.subprocess.spawn({
    argv: ['bash', '-c', "bash -c 'exec -a cortex-output-drain-descendant sleep 30' & printf 'leader-done\\n'"],
    cwd: process.cwd(),
  ...
  const outputDrainClean = !outputDrainProcesses.some(processInfo =>
    JSON.stringify([processInfo.cmd, processInfo.args]).includes('cortex-output-drain-descendant'),
  )
  ...
  await ctx.fs.writeText(remoteSource, 'const café = "naïve"\nconsole.log(café)\n', { kind: 'createIfAbsent' })
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'CORTEX_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /CORTEX_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
