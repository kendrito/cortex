...
import { normalizeSessionLog, scrubRequestHeaders, type NormalizeContext } from '@cortex/acp-snapshot'
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@cortex/loader-smoke'
import { createUserMessage } from '@cortex/llm'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@cortex/session'
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'cortex-subagent-inherit-',
      binScript,
      ...
      env: {
        ...
        CORTEX_SNAPSHOT_FILE: replayOverride,
        CORTEX_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@cortex/system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the Cortex file sandbox cannot modify files in the standing mode.')
