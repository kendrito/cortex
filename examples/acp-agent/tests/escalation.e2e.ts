 ...
 *
 * Keyless smoke: boot the REAL `cordis.yml` through the `cortex-acp-agent` bin as
 ...
 *
 * With-key escalation flow (self-skips without CORTEX_API_KEY or a usable
...
function launchExampleAcpAgent(
  ...
  const launched = launchAcpTestAgent({
    ...
    env: {
      CORTEX_API_KEY: process.env.CORTEX_API_KEY ?? 'sk-dummy-for-boot',
      CORTEX_PERMISSION_MODE: sandboxMode,
    },
...
describe.skipIf(!process.env.CORTEX_API_KEY || !hasRunner)('default sandbox composition e2e: the live approval loop', () => {
