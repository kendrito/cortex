...
const DANGER_FULL_ACCESS_ENV = { CORTEX_PERMISSION_MODE: 'danger-full-access' }
...
describe('acp-agent over real stdio (no key required)', () => {
  it('emits only framed JSON-RPC on stdout', async () => {
    ...
    // A dummy key lets the cortex adapter APPLY (it only checks the key is
  ...
  it('session/new succeeds over real stdio (no model call)', async () => {
    ...
    workdir = await mkdtemp(join(tmpdir(), 'acp-e2e-'))
    // A dummy key lets the cortex adapter boot (it only checks presence, not
...
describe.skipIf(!process.env.CORTEX_API_KEY)('acp-agent e2e: real prompt over ACP', () => {
