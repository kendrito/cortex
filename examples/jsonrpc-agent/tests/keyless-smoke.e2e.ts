...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'cortex-jsonrpc-agent-smoke-'))
    ...
    const child = execa(process.execPath, [
      ...
      cwd: repoRoot,
      env: {
        CORTEX_API_KEY: 'keyless-smoke-no-call',
        CORTEX_BASE_URL: `http://127.0.0.1:${address.port}`,
        CORTEX_CWD: root,
        CORTEX_SESSION_ROOT: join(root, '.sessions'),
        ...(envValue === undefined ? {} : { CORTEX_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
    ...
    try {
      child.stdin.write(`${JSON.stringify({
        ...
        id: 1,
        method: 'initialize',
        params: { cwd: root, provider: 'cortex-official', model: 'cortex-v4-pro', maxTokens: 1234 },
      })}\n`)
  ...
  it('rejects an invalid max-token success env value', async () => {
    const { exitCode, stdout, stderr } = await execa(process.execPath, [
      ...
      env: {
        CORTEX_API_KEY: 'keyless-smoke-no-call',
        CORTEX_MAX_TOKENS_AS_SUCCESS: 'sometimes',
      },
      stdin: 'ignore',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@cortex/sdk-jsonrpc-server)')
