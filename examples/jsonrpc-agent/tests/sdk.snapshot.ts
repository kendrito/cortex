 ...
 * the REAL `cortex-jsonrpc-agent` runtime (per `CORTEX_EXAMPLE_MODE`) through the
 * REAL `@cortex/sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `CORTEX_SNAPSHOT=record`
 * re-records against the live API; `CORTEX_SNAPSHOT=refresh` replays committed
...
import { CortexHarness, type HarnessNotification, type RunResult } from '@cortex/sdk-client'
...
const mode = process.env.CORTEX_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  {
    name: 'bash-tool',
    prompt: 'Run this exact command with your bash tool, then reply with its stdout only: echo cortex-sdk-proof-7391',
  ...
  },
  {
    ...
    environment: { CORTEX_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    CORTEX_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    CORTEX_SESSION_ROOT: sessionsRoot,
    CORTEX_CWD: cwd,
    CORTEX_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      CORTEX_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { CORTEX_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
    ...scenario.environment,
  }

  const harness = new CortexHarness({
    launch: {
