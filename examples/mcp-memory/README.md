...
These three **default-off reference configurations** connect one memory system to Cortex through [`@cortex/mcp-client`](../../packages/mcp/mcp-client/README.md). Pick one, or copy the same generic MCP row for another server.

These third-party configurations are provided as interoperability examples only. Their inclusion does not imply endorsement, recommendation, partnership, or ongoing support by Cortex.
...
```sh
cortex web --patch "$PWD/examples/mcp-memory/memorix.cordis.yml"
```
...
cortex web --patch "$PWD/examples/mcp-memory/mcp-reference-memory.cordis.yml"
```
...
cortex web --patch "$PWD/examples/mcp-memory/engram.cordis.yml"
```
...
A new Cortex session is required; a Host restart is not. Restart or HMR is needed only after an MCP child crashes because the current generic client does not auto-reconnect; its tool registrations remain until plugin disposal or a successful re-sync, and calls can fail against the closed transport. Initial discovery is asynchronous, so wait for the provider's `mcp__...` tools before sending the first validation prompt.
